...
public:
    ...
    struct fsio {
        ...
        cvec_ptr<physical_io_device> nvram{};
    ...
    void setup_persistent_store(persistent_store &ps, const char *path, bool primary = true);
    // After the primary cart/memcard store is set up, discover and set up any additional
    // persistent stores the system exposes (HID_NVRAM PIOs), each in its own <stem>.<ext> file.
    void setup_aux_persistent_stores();
    // force=true flushes regardless of the throttle interval (exit, close, pause, save-state).
    void sync_persistent_storage(bool force = false);
    ...
    // All persistent stores currently open (cart SRAM / memcard + any HID_NVRAM stores).
    // The "primary" store (cart SRAM / memcard) is the one bundled into save states as sram.bin.
    struct ManagedStore { persistent_store *ps{}; bool primary{}; };
    std::vector<ManagedStore> active_stores;
    persistent_store *primary_store();
    double sram_last_sync{-1e9};        // ImGui time of the last disk flush (throttle reference)
    int  sram_sync_interval_ms{10000};  // min ms between periodic disk flushes (from settings)
    bool sram_autosync_on_pause{true};  // flush SRAM on play->pause (from settings)

    // --- RTC bridge (HID_RTC PIO) ------------------------------------------------------------
    struct JSM_RTC *find_rtc();         // the present HID_RTC PIO's data, or null
    void update_rtc(bool ff_active);    // per-frame: refresh wall-clock, handle boot/FF (re)seed
    void render_rtc_window();           // ImGui window; no-op unless an RTC PIO is present
    bool rtc_default_user{false};       // settings: start in user-time instead of wall-clock
    bool rtc_ff_uses_user{true};        // settings: fast-forward switches to user-time
    bool rtc_booted{false};             // initial seed done this session
    bool rtc_ff_prev{false};            // FF edge detection
    bool rtc_window_open{true};         // window visibility toggle
