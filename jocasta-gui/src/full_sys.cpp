...
static u32 mfs_add_path(multi_file_set* ROMs, const char* path, bool is_folder)
{
    ...
    if (str_ends_with_ci(path, ".chd"))
        return mfs_add_chd_path(ROMs, path);
    if (str_ends_with_ci(path, ".ccd"))
        return mfs_add_ccd_path(ROMs, path);
...
[[maybe_unused]]
static u32 grab_BIOSes_UNUSED(multi_file_set* BIOSes, jsm::systems which, const char* bios_dir_override)
{
    char BIOS_PATH[255];
    char BASE_PATH[255];
    ...
    switch(which) {
        ...
        case jsm::systems::TURBOGRAFX16_CD:
            ...
            if (using_override)
                ...
                snprintf(BIOS_PATH, sizeof(BIOS_PATH), "%s/tg16", BASE_PATH);
            BIOSes->add("syscard3.pce", BIOS_PATH);
            break;
        case jsm::systems::TURBOGRAFX16_ARCADE_CD:
            ...
            if (using_override)
                ...
                snprintf(BIOS_PATH, sizeof(BIOS_PATH), "%s/tg16", BASE_PATH);
            BIOSes->add("arcade_card_pro.pce", BIOS_PATH);
            break;
...
u32 grab_gdi(multi_file_set* ROMs, jsm::systems which, const char* fname, const char* sec_path) {
    ...
    // First line = track count (informational) -- read and skipped
...
void full_system::setup_bios()
{
    ...
    const BIOSEntry* entry = sysdb_find_bios(which);
    if (!entry) return;

    std::string dir = bios_override_dir[0]
        ? std::string(bios_override_dir)
        : bios_default_dir(entry->dir_slug);

    std::vector<std::string> working_paths;
    bios_load(which, dir, BIOSes, working_paths);
    ...
    if (ps.persistent) {
        ...
        if (!ps.fno) {
            ...
            if (ps.fno) {
                ...
                if (ps.init_data && ps.init_data_size) // seed a fresh file from the cart image
                    memcpy(a, ps.init_data, ps.init_data_size < ps.requested_size ? ps.init_data_size : ps.requested_size);
        ...
        if (ps.fno) {
            ...
            // Couldn't open file — fall back to volatile RAM (still seed it if provided)
            ps.data = malloc(ps.requested_size);
            memset(ps.data, ps.fill_value, ps.requested_size);
            if (ps.init_data && ps.init_data_size)
                memcpy(ps.data, ps.init_data, ps.init_data_size < ps.requested_size ? ps.init_data_size : ps.requested_size);
        }
...
persistent_store *full_system::primary_store()
{
    for (auto &m : active_stores)
        if (m.primary && m.ps) return m.ps;
    return nullptr;
}
...
void full_system::setup_persistent_store(persistent_store &ps, multi_file_set &mfs)
{
    ...
    char path[600];
    snprintf(path, sizeof(path), "%s/%s.sram", rfb->path, rfb->name);
    setup_persistent_store(ps, path, true);
}

void full_system::setup_persistent_store(persistent_store &ps, const char *path, bool primary)
{
    printf("\nSETTING UP PERSISTENT STORE: %s", path);
    snprintf(ps.filename, sizeof(ps.filename), "%s", path);
    finish_setup_ps(ps);   // opens/creates the file, allocates + loads ps.data, marks ready

    // Register (dedupe by pointer; promote to primary if asked).
    for (auto &m : active_stores)
        if (m.ps == &ps) { m.primary = m.primary || primary; return; }
    active_stores.push_back({ &ps, primary });
}

// Discover and open every additional persistent store the system exposes (HID_NVRAM PIOs),
// each beside the primary save as <stem>.<ext>. Called right after the primary cart/memcard
// store is set up in apply_core_options (so sram_save_dir is resolved).
void full_system::setup_aux_persistent_stores()
{
    if (!sys) return;

    std::string stem = "bios";
    if (!ROMs.files.empty()) {
    ...
    for (auto &pio : sys->IOs) {
        if (pio.kind != HID_NVRAM) continue;
        persistent_store &ps = pio.nvram.store;
        if (ps.requested_size == 0 || ps.ready_to_use) continue;
        const char *ext = ps.ext[0] ? ps.ext : "sram";
        char path[700];
        snprintf(path, sizeof(path), "%s/%s.%s", sram_save_dir.c_str(), stem.c_str(), ext);
        setup_persistent_store(ps, path, /*primary=*/false);
    }
}

void full_system::sync_persistent_storage(bool force)
{
    if (active_stores.empty()) return;

    // Throttle disk writes: a frequently-dirtied store (e.g. CPU work RAM) would otherwise hit
    // the disk every frame and wear an SSD. force=true bypasses the throttle (exit / close /
    // pause / save-state). Periodic syncs only happen while the core is running (this is called
    // from the per-frame path), so a paused emulator never writes on the timer.
    if (!force) {
        double now = ImGui::GetTime();
        double interval = (double)sram_sync_interval_ms / 1000.0;
        if (now - sram_last_sync < interval) return;
    }

    for (auto &m : active_stores) {
        persistent_store *ps = m.ps;
        if (!ps || !ps->persistent || !ps->dirty || !ps->fno || !ps->data) continue;
        fseek(ps->fno, 0, SEEK_SET);
        fwrite(ps->data, 1, ps->actual_size, ps->fno);
        fflush(ps->fno);
        ps->dirty = false;
    }
    sram_last_sync = ImGui::GetTime();
}

void full_system::close_persistent_storage()
{
    sync_persistent_storage(true);
    for (auto &m : active_stores) {
        persistent_store *ps = m.ps;
        if (!ps) continue;
        if (ps->fno)  { fclose(ps->fno); ps->fno = nullptr; }
        if (ps->data) { free(ps->data);  ps->data = nullptr; }
        ps->actual_size = 0;
        ps->old_requested_size = 0;
...
JSM_RTC *full_system::find_rtc()
{
    if (!sys) return nullptr;
    for (auto &pio : sys->IOs)
        if (pio.kind == HID_RTC && pio.rtc.present) return &pio.rtc;
    return nullptr;
}

void full_system::update_rtc(bool ff_active)
{
    JSM_RTC *r = find_rtc();
    if (!r) return;

    // Refresh the wall-clock the core reads/displays.
    time_t now = time(nullptr);
    struct tm lt = *localtime(&now);
    r->wall.year    = (u16)(lt.tm_year + 1900);
    r->wall.month   = (u8)(lt.tm_mon + 1);
    r->wall.day     = (u8)lt.tm_mday;
    r->wall.hour    = (u8)lt.tm_hour;
    r->wall.minute  = (u8)lt.tm_min;
    r->wall.second  = (u8)lt.tm_sec;
    r->wall.weekday = (u8)lt.tm_wday;
    r->wall_valid   = true;

    // Initial seed: pick the source from settings and adopt the base time once.
    if (!rtc_booted) {
        rtc_booted = true;
        r->source = rtc_default_user ? RTC_SOURCE_USER : RTC_SOURCE_WALLCLOCK;
        if (r->source == RTC_SOURCE_USER && r->user.year < 1900) r->user = r->wall;
        r->reseed = true;
    }

    // Fast-forward: on the rising edge, relabel to user-time (the clock free-runs on emulated
    // time, so it has diverged from the wall clock -- it's no longer "wall" once fast-forwarded).
    // No reseed: the running clock continues uninterrupted ("proceed from there").
    if (rtc_ff_uses_user && ff_active && !rtc_ff_prev && r->source == RTC_SOURCE_WALLCLOCK) {
        r->user   = r->game.year >= 1900 ? r->game : r->wall;
        r->source = RTC_SOURCE_USER;
    }
    rtc_ff_prev = ff_active;
}

void full_system::render_rtc_window()
{
    JSM_RTC *r = find_rtc();
    if (!r || !rtc_window_open) return;

    if (!ImGui::Begin("RTC", &rtc_window_open, ImGuiWindowFlags_AlwaysAutoResize)) {
        ImGui::End();
        return;
    }

    // Source selection.
    int src = (int)r->source;
    ImGui::TextUnformatted("Clock source:");
    if (ImGui::RadioButton("Wall clock", &src, RTC_SOURCE_WALLCLOCK)) {
        r->source = RTC_SOURCE_WALLCLOCK;
        r->reseed = true; // re-sync to real time
    }
    ImGui::SameLine();
    if (ImGui::RadioButton("User", &src, RTC_SOURCE_USER)) {
        if (r->user.year < 1900) r->user = r->game; // start from the current game time
        r->source = RTC_SOURCE_USER;
        r->reseed = true;
    }

    // Game-visible time (what the running game reads).
    ImGui::Separator();
    if (r->has_date)
        ImGui::Text("Game time: %04u-%02u-%02u  %02u:%02u:%02u",
                    r->game.year, r->game.month, r->game.day,
                    r->game.hour, r->game.minute, r->game.second);
    else
        ImGui::Text("Game time: %02u:%02u:%02u", r->game.hour, r->game.minute, r->game.second);
    ImGui::Text("Wall clock: %04u-%02u-%02u  %02u:%02u:%02u",
                r->wall.year, r->wall.month, r->wall.day,
                r->wall.hour, r->wall.minute, r->wall.second);

    // User-time editor (only meaningful in user mode, and only while paused so edits aren't
    // fighting the running clock).
    ImGui::Separator();
    const bool can_edit = (r->source == RTC_SOURCE_USER) && (run_state != FSS_play);
    if (r->source == RTC_SOURCE_USER && run_state == FSS_play)
        ImGui::TextDisabled("(pause to edit the time)");
    ImGui::BeginDisabled(!can_edit);
    int y = r->user.year, mo = r->user.month, d = r->user.day;
    int h = r->user.hour, mi = r->user.minute, s = r->user.second;
    bool changed = false;
    if (r->has_date) {
        changed |= ImGui::InputInt("Year",  &y);
        changed |= ImGui::InputInt("Month", &mo);
        changed |= ImGui::InputInt("Day",   &d);
    }
    changed |= ImGui::InputInt("Hour",   &h);
    changed |= ImGui::InputInt("Minute", &mi);
    changed |= ImGui::InputInt("Second", &s);
    if (ImGui::Button("Apply / set now")) { r->user = r->wall; changed = true; }
    if (changed) {
        auto clamp = [](int v, int lo, int hi) { return v < lo ? lo : v > hi ? hi : v; };
        r->user.year    = (u16)clamp(y, 1900, 2099);
        r->user.month   = (u8)clamp(mo, 1, 12);
        r->user.day     = (u8)clamp(d, 1, 31);
        r->user.hour    = (u8)clamp(h, 0, 23);
        r->user.minute  = (u8)clamp(mi, 0, 59);
        r->user.second  = (u8)clamp(s, 0, 59);
        r->source       = RTC_SOURCE_USER;
        r->reseed       = true;
    }
    ImGui::EndDisabled();
...
{
    // Force a final flush and close/free every open store (cart SRAM + aux NVRAM).
    close_persistent_storage();
    rtc_booted = false;   // next system re-seeds its RTC from scratch
    rtc_ff_prev = false;
...
void full_system::apply_core_options(AppSettings& settings, const char* core_key)
{
    ...
    // Open any additional per-system persistent stores (e.g. NGP work RAM) beside the primary.
    setup_aux_persistent_stores();

    // Cache disk-flush pacing settings (read live in sync_persistent_storage / on pause).
    sram_sync_interval_ms   = settings.get_sram_sync_interval_ms();
    sram_autosync_on_pause  = settings.get_sram_autosync_on_pause();

    // RTC: global default source + per-game fast-forward behavior.
    rtc_default_user = settings.get_rtc_default_user();
    rtc_ff_uses_user = settings.get_core_option(core_key, "rtc_ff_uses_user", 1) != 0;
    if (io.nvram.vec) {
        persistent_store &ps = io.nvram.get().nvram.store;
        if (!ps.ready_to_use && ps.requested_size > 0) {
            char sram_path[600];
            snprintf(sram_path, sizeof(sram_path), "%s/%s.sram",
                     core_saves_dir.c_str(), io.nvram.get().nvram.label);
            setup_persistent_store(ps, sram_path);
        }
    }
...
void full_system::save_state_to_slot(int slot)
{
    ...
    // Always bundle the primary store's SRAM so it can be restored even if the option changes.
    if (persistent_store *pps = primary_store(); pps && pps->data && pps->actual_size > 0)
        mz_zip_writer_add_mem(&zip, "sram.bin",
                              pps->data, pps->actual_size,
                              MZ_BEST_COMPRESSION);

    // RTC config is owned by the PIO and (de)serialized here by the host, independent of the
    // core's own state. On load the source mode decides behavior: WALLCLOCK keeps tracking real
    // time (the saved time is ignored), USER restores the configured internal time.
    if (JSM_RTC *r = find_rtc()) {
        u8 b[20]; int n = 0;
        b[n++] = 1;                  // version
        b[n++] = (u8)r->source;
        b[n++] = r->has_date ? 1 : 0;
        b[n++] = r->present  ? 1 : 0;
        auto put = [&](const rtc_datetime &d) {
            b[n++] = d.year & 0xff; b[n++] = (d.year >> 8) & 0xff;
            b[n++] = d.month; b[n++] = d.day;
            b[n++] = d.hour; b[n++] = d.minute; b[n++] = d.second; b[n++] = d.weekday;
        };
        put(r->user); put(r->game);
        mz_zip_writer_add_mem(&zip, "rtc.bin", b, n, MZ_BEST_COMPRESSION);
    }
    if (io.nvram.vec) {
        persistent_store &sps = io.nvram.get().nvram.store;
        if (sps.data && sps.actual_size > 0)
            mz_zip_writer_add_mem(&zip, "nvram.bin",
                                  sps.data, sps.actual_size,
                                  MZ_BEST_COMPRESSION);
    }
    ...
    // When SRAM travels with save states, also flush it to its own file so the on-disk save
    // matches the state just written.
    if (sram_in_state) sync_persistent_storage(true);
...
void full_system::load_state_from_slot(int slot)
{
    ...
    if (sram_in_state) {
        ...
        persistent_store *pps = primary_store();
        if (sram_data && sram_sz > 0 && pps && pps->data && sram_sz == pps->actual_size) {
            memcpy(pps->data, sram_data, sram_sz);
            pps->dirty = true;
        }
        ...
        if (io.nvram.vec) {
            persistent_store &sps = io.nvram.get().nvram.store;
            size_t sys_sz = 0;
            void *sys_data = mz_zip_reader_extract_file_to_heap(&zip, "nvram.bin", &sys_sz, 0);
            if (sys_data && sys_sz > 0 && sps.data && sys_sz == sps.actual_size) {
                memcpy(sps.data, sys_data, sys_sz);
                sps.dirty = true;
            }
            if (sys_data) mz_free(sys_data);
        }
    }

    // Restore the RTC PIO config the host saved. The source mode (a sticky user choice) governs:
    //   WALLCLOCK -> reseed from the *current* wall clock (real time is respected even now), so a
    //                state saved long ago doesn't pin the clock to its old time.
    //   USER      -> reseed from the restored user/internal time the player configured.
    if (JSM_RTC *r = find_rtc()) {
        size_t sz = 0;
        void *d = mz_zip_reader_extract_file_to_heap(&zip, "rtc.bin", &sz, 0);
        if (d && sz >= 20 && ((u8 *)d)[0] == 1) {
            u8 *b = (u8 *)d; int n = 1;
            r->source   = (rtc_time_source)b[n++];
            r->has_date = b[n++] != 0;
            r->present  = b[n++] != 0;
            auto get = [&](rtc_datetime &dt) {
                dt.year = (u16)(b[n] | (b[n + 1] << 8)); n += 2;
                dt.month = b[n++]; dt.day = b[n++];
                dt.hour = b[n++]; dt.minute = b[n++]; dt.second = b[n++]; dt.weekday = b[n++];
            };
            get(r->user); get(r->game);
            r->reseed = true; // core adopts (source==USER ? user : current wall) next frame
        }
        if (d) mz_free(d);
    }
    ...
    if (f) {
        ...
        if (wrote) {
            ...
            // Persist the restored SRAM so the on-disk save matches the loaded state.
            if (sram_in_state) sync_persistent_storage(true);
