...
public:
    ...
    std::string get_bios_dir(const char* slug) const;
    void set_bios_dir(const char* slug, const char* dir);
    ...
    // Persistent-store (SRAM/NVRAM) disk-flush pacing. Interval is the minimum gap between
    // periodic flushes while running; auto-sync-on-pause flushes on the play->pause transition.
    int  get_sram_sync_interval_ms() const;     // default 10000 (10s)
    void set_sram_sync_interval_ms(int ms);
    bool get_sram_autosync_on_pause() const;    // default true
    void set_sram_autosync_on_pause(bool v);

    // RTC: default clock source for systems with an RTC (false = wall-clock, true = user time).
    bool get_rtc_default_user() const;          // default false (wall-clock)
    void set_rtc_default_user(bool v);
