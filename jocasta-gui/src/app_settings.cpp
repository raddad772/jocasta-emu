...
const char* AppSettings::sys_to_cli(jsm::systems sys)
{
    switch (sys) {
        ...
        case jsm::systems::TURBOGRAFX16_CD: return "tg16cd";
        case jsm::systems::TURBOGRAFX16_ARCADE_CD: return "tg16acd";
...
const char* AppSettings::sys_to_label(jsm::systems sys)
{
    switch (sys) {
        ...
        case jsm::systems::TURBOGRAFX16_CD: return "TurboGrafx-16 CD";
        case jsm::systems::TURBOGRAFX16_ARCADE_CD: return "TurboGrafx-16 Arcade CD";
...
void AppSettings::migrate_bios_dirs()
{
    static const struct { const char* old_sec; const char* slug; } MAP[] = {
        { "system.gb", "gameboy" },
        { "system.gbc", "gameboy" },
        { "system.gba", "gba" },
        { "system.ps1", "ps1" },
        { "system.dreamcast", "dreamcast" },
        { "system.nds", "nds" },
        { "system.mac128k", "mac" },
        { "system.mac512k", "mac" },
        { "system.macplus", "mac" },
        { "system.apple2e", "apple2" },
        { "system.zx48", "zx_spectrum" },
        { "system.zx128", "zx_spectrum" },
        { "system.galaksija", "galaksija" },
        { "system.c64", "commodore64" },
        { "system.neogeo-aes", "neogeo" },
        { "system.neogeo-mvs", "neogeo" },
        { "system.ngp", "ngp" },
        { "system.ngpc", "ngp" },
        { "system.tg16cd", "tg16" },
        { "system.tg16acd", "tg16" },
    ...
    bool any = false;
    for (auto& m : MAP) {
        std::string new_sec = std::string("bios.") + m.slug;
        if (m_ini.has(new_sec) && m_ini[new_sec].has("dir")) {
            std::string cur = m_ini[new_sec].get("dir");
            if (cur != bios_default_dir(m.slug)) continue;
        }
        if (!m_ini.has(m.old_sec)) continue;
        auto& old = m_ini[m.old_sec];
        if (!old.has("bios_dir")) continue;
        std::string val = old.get("bios_dir");
        if (val.empty()) continue;
        m_ini[new_sec]["dir"] = val;
        m_ini[m.old_sec].remove("bios_dir");
        any = true;
    }
    if (any) save();
}
...
void AppSettings::ensure_defaults()
{
    ...
    for (int i = 0; i < SYS_DB_COUNT; i++) {
        const char* slug = SYS_DB[i].dir_slug;
        std::string sec = std::string("bios.") + slug;
        if (!m_ini.has(sec) || !m_ini[sec].has("dir")) {
            m_ini[sec]["dir"] = bios_default_dir(slug);
            wrote_defaults = true;
        }
    }
...
std::string AppSettings::get_bios_dir(const char* slug) const
{
    std::string sec = std::string("bios.") + slug;
...
void AppSettings::set_bios_dir(const char* slug, const char* dir)
{
    std::string sec = std::string("bios.") + slug;
    m_ini[sec]["dir"] = dir ? dir : "";
    save();
}
...
int AppSettings::get_sram_sync_interval_ms() const
{
    if (!m_ini.has("settings")) return 10000;
    auto sec = m_ini.get("settings");
    if (!sec.has("sram_sync_interval_ms")) return 10000;
    try { int v = std::stoi(sec.get("sram_sync_interval_ms")); return v < 0 ? 0 : v; }
    catch (...) { return 10000; }
}

void AppSettings::set_sram_sync_interval_ms(int ms)
{
    if (ms < 0) ms = 0;
    m_ini["settings"]["sram_sync_interval_ms"] = std::to_string(ms);
    save();
}

bool AppSettings::get_sram_autosync_on_pause() const
{
    if (!m_ini.has("settings")) return true;
    auto sec = m_ini.get("settings");
    if (!sec.has("sram_autosync_on_pause")) return true;
    try { return std::stoi(sec.get("sram_autosync_on_pause")) != 0; } catch (...) { return true; }
}

void AppSettings::set_sram_autosync_on_pause(bool v)
{
    m_ini["settings"]["sram_autosync_on_pause"] = v ? "1" : "0";
    save();
}

bool AppSettings::get_rtc_default_user() const
{
    if (!m_ini.has("settings")) return false;
    auto sec = m_ini.get("settings");
    if (!sec.has("rtc_default_user")) return false;
    try { return std::stoi(sec.get("rtc_default_user")) != 0; } catch (...) { return false; }
}

void AppSettings::set_rtc_default_user(bool v)
{
    m_ini["settings"]["rtc_default_user"] = v ? "1" : "0";
    save();
}
...
Binding AppSettings::get_default_kbd(const char* core_key, int dbcid)
{
    ...
    if (ck == "gameboy" || ck == "nes") {
    ...
    } else if (ck == "sms" || ck == "sg1000" || ck == "pv1000") {
        switch (dbcid) {
            case DBCID_co_fire1: return kb(ImGuiKey_Z);
            case DBCID_co_fire2: return kb(ImGuiKey_X);
            default: break;
        }
    } else if (ck == "tg16" || ck == "tg16cd" || ck == "tg16acd") {
        switch (dbcid) {
            case DBCID_co_fire1: return kb(ImGuiKey_X);
            case DBCID_co_fire2: return kb(ImGuiKey_Z);
            default: break;
        }
        ...
        // NeoGeo Pocket: A=fire1(Z), B=fire2(X). OPTION=start uses the universal Enter binding.
        switch (dbcid) {
            case DBCID_co_fire1: return kb(ImGuiKey_Z); // A
            case DBCID_co_fire2: return kb(ImGuiKey_X); // B
            default: break;
