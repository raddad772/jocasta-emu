...
static const char* const FILT_PS1[] = {"*.bin","*.cue","*.iso","*.chd","*.ccd",nullptr};
...
static const char* const FILT_PCECD[] = {"*.cue","*.bin","*.chd","*.ccd",nullptr};
...
static constexpr load_system_entry LOAD_SYSTEMS[] = {
    ...
    { "TurboGrafx-16 CD", "tg16cd", jsm::systems::TURBOGRAFX16_CD, false, FILT_PCECD, FILT_PCECD, nullptr },
    { "TurboGrafx-16 Arcade CD","tg16acd", jsm::systems::TURBOGRAFX16_ARCADE_CD,false, FILT_PCECD, FILT_PCECD, nullptr },
...
static void update_input(full_system* fsys, AppSettings& settings, const char* core_key, u32 *hotkeys, ImGuiIO& io, bool fullscreen = false) {
    ...
    if (fsys && !fullscreen) fsys->render_rtc_window();
...
void imgui_jocasta_app::render_settings_page_general()
{
...

    {
        ...
        // Disk-flush pacing: how often SRAM/NVRAM is written out while running. A frequently
        // dirtied store (e.g. NGP work RAM) would otherwise hit the disk every frame.
        int interval_s = settings.get_sram_sync_interval_ms() / 1000;
        if (ImGui::SliderInt("SRAM flush interval (sec)##g", &interval_s, 1, 60)) {
            if (interval_s < 1) interval_s = 1;
            settings.set_sram_sync_interval_ms(interval_s * 1000);
            if (fsys) fsys->sram_sync_interval_ms = interval_s * 1000;
        }
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("Minimum seconds between periodic SRAM/NVRAM disk writes while running.\n"
                              "Always flushed on pause (if enabled below), save-state, and exit.");

        bool g_pausesync = settings.get_sram_autosync_on_pause();
        if (ImGui::Checkbox("Flush SRAM when paused##g", &g_pausesync)) {
            settings.set_sram_autosync_on_pause(g_pausesync);
            if (fsys) fsys->sram_autosync_on_pause = g_pausesync;
        }
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("Write SRAM/NVRAM to disk on the play->pause transition.");

        bool g_rtc_user = settings.get_rtc_default_user();
        if (ImGui::Checkbox("RTC starts in user-set time (else wall clock)##g", &g_rtc_user))
            settings.set_rtc_default_user(g_rtc_user);
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("For systems with an RTC: default to a user-provided time instead of\n"
                              "the host wall clock. Change live in the RTC window.");
    }
...
void imgui_jocasta_app::render_settings_page_bios()
{
    struct SlugCache {
        std::string slug;
        std::vector<std::pair<const BIOSEntry*, std::vector<BIOSCheckResult>>> entries;
    };
    static std::vector<SlugCache> cache;
    static bool cache_dirty = true;

    auto invalidate = [&]() { cache_dirty = true; };

    if (ImGui::SmallButton("Re-check all")) invalidate();
    ImGui::SameLine();
    ...
    if (cache_dirty) {
        ...
        for (int i = 0; i < SYS_DB_COUNT; i++) {
            const BIOSEntry* e = &SYS_DB[i];
            std::string slug(e->dir_slug);
            std::string dir = bios_get_dir(e->sys, settings);
            auto results = bios_check(e->sys, dir);

            SlugCache* sc = nullptr;
            for (auto& s : cache) { if (s.slug == slug) { sc = &s; break; } }
            if (!sc) { cache.push_back({ slug, {} }); sc = &cache.back(); }
            sc->entries.push_back({ e, std::move(results) });
        }
        cache_dirty = false;
    }
    ...
    for (auto& sc : cache) {
        const BIOSEntry* first = sc.entries[0].first;
        std::string configured = settings.get_bios_dir(sc.slug.c_str());
        std::string effective  = configured.empty()
            ? bios_default_dir(sc.slug.c_str())
            : configured;

        char buf[512];
        snprintf(buf, sizeof(buf), "%s", effective.c_str());
        ImGui::PushID(sc.slug.c_str());

        {
            ...
            float label_w = ImGui::CalcTextSize(first->menu_label).x;
            float avail    = ImGui::GetContentRegionAvail().x;
            float reserved = label_w + spacing + browse_w + spacing + open_w + spacing;
            if (!configured.empty()) reserved += reset_w + spacing;
            float input_w = std::max(avail - reserved, 80.0f);

            ImGui::Text("%s", first->menu_label);
            ImGui::SameLine();
            ImGui::SetNextItemWidth(input_w);
            if (configured.empty())
                ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetStyleColorVec4(ImGuiCol_TextDisabled));
            bool edited = ImGui::InputText("##bios", buf, sizeof(buf),
                                           ImGuiInputTextFlags_EnterReturnsTrue);
            if (configured.empty()) ImGui::PopStyleColor();
            if (edited) { settings.set_bios_dir(sc.slug.c_str(), buf); invalidate(); }
            ImGui::SameLine();
            if (ImGui::Button("Browse...")) {
                std::string start = existing_dialog_dir(effective, false);
                char title[128];
                snprintf(title, sizeof(title), "BIOS Directory - %s", first->menu_label);
                const char* picked = tinyfd_selectFolderDialog(title, start.c_str());
                if (picked) { settings.set_bios_dir(sc.slug.c_str(), picked); invalidate(); }
            }
            ImGui::SameLine();
            ...
            if (!configured.empty()) {
                ImGui::SameLine();
                if (ImGui::Button("Reset")) { settings.set_bios_dir(sc.slug.c_str(), ""); invalidate(); }
            }
        }

        ImGui::Indent(16.f);
        for (auto& [entry, results] : sc.entries) {
            if (sc.entries.size() > 1)
                ImGui::TextDisabled("%s:", entry->menu_label);
            for (auto& r : results) {
                bios_status st = r.status;
                ImVec4 col = (r.file->optional && st == bios_status::missing)
                    ? ImVec4(0.5f, 0.5f, 0.5f, 1.f)
                    : status_color(st);
                ImGui::TextColored(col, "[%s]", status_label(st));
                ImGui::SameLine();
                ImGui::TextDisabled("%s", r.file->filename);
                ImGui::SameLine();
                ImGui::TextDisabled("- %s%s%s",
                    r.file->label,
                    r.file->optional ? " (optional)" : "",
                    r.file->writable ? " [firmware]" : "");
            }
        }
        ImGui::Unindent(16.f);
    ...
    ImGui::BeginChild("##opt_core_content", ImVec2(0, 0), false);
    {
        ...
        // Boot and core-specific options
        {
            ...
            static const jsm::systems all_sys[] = {
                ...
                jsm::systems::TURBOGRAFX16_CD, jsm::systems::TURBOGRAFX16_ARCADE_CD,
            ...
            // Fast boot
            // NOTE: GBA is excluded -- its skip_BIOS() HLE leaves the ARM32 core in a
            // state that breaks some games (e.g. Doom), so GBA always boots the real BIOS.
...
{
    if (!sysdb_find_bios(system))
        return true;

    std::string dir = bios_get_dir(system, settings);
    if (bios_ok_to_launch(system, dir))
        return true;

    auto results = bios_check(system, dir);
    ...
    for (auto& r : results)
        if (!r.file->optional && r.status == bios_status::missing)
            msg += "  " + std::string(r.file->filename) + "\n";
...
void imgui_jocasta_app::render_main_menu_bar()
{
    ...
    if (ImGui::BeginMenu("File")) {
        ...
        auto render_groups = [&](auto action) {
            ...
            if (ImGui::BeginMenu("RCA")) {
                for (auto s : { jsm::systems::COSMAC_VIP_2k, jsm::systems::COSMAC_VIP_4k })
                    if (auto* e = find_sys(s)) action(*e);
                ImGui::EndMenu();
            }
            if (ImGui::BeginMenu("NEC")) {
                for (auto s : { jsm::systems::TURBOGRAFX16,
                                jsm::systems::TURBOGRAFX16_CD,
                                jsm::systems::TURBOGRAFX16_ARCADE_CD })
                    if (auto* e = find_sys(s)) action(*e);
                ImGui::EndMenu();
            }
            ImGui::Separator();
...
void imgui_jocasta_app::load_path_for_system(jsm::systems system, const char *path, bool is_folder)
{
    ...
    std::string bios_dir = bios_get_dir(system, settings);
...
int imgui_jocasta_app::do_setup_before_mainloop()
{
    ...
    if (startup_has_load) {
        ...
        if (!check_required_bios_or_warn(which))
            return 0;
        fsys = create_fsys();
        std::string bios_dir = bios_get_dir(which, settings);
        snprintf(fsys->bios_override_dir, sizeof(fsys->bios_override_dir), "%s", bios_dir.c_str());
...
{
    if (!fsys) return;
    bool was_playing = (fsys->run_state == FSS_play);
    fsys->run_state = FSS_pause;
    audio_needs_prime = false;
    // Fade out over ~5 callback periods (~83 ms at 60 fps) to avoid clicks.
    fsys->audio.begin_fadeout(5);
    // Optional: flush SRAM/NVRAM to disk on the play->pause transition (settings-gated).
    if (was_playing && fsys->sram_autosync_on_pause)
        fsys->sync_persistent_storage(true);
}
...
void imgui_jocasta_app::mainloop(ImGuiIO& io) {
    if (fsys) {
        ...
        // Feed the RTC PIO this frame's wall-clock and apply boot/fast-forward (re)seeding before
        // the core runs (the GUI renders + edits the PIO, then finish_frame lets the core consume).
        fsys->update_rtc(ff_held);
