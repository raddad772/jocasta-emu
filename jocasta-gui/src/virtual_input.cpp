...
// NeoGeo Pocket / Pocket Color controller.
// Buttons: up/dn/lt/rt + A(fire1) B(fire2) + OPTION(start). No select.
//   row 0:  gap ^
//   row 1:  < gap >   gap   [A][B]
//   row 2:  gap v
//   row 3:  [ OPTION ]
static void render_ngp_controller(JSM_CONTROLLER &ctr, int port)
{
    vi_first_pos(port);
    if (!ImGui::Begin("NeoGeo Pocket Controller", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        ImGui::End(); return;
    }
    ...
    const float gap = ImGui::GetStyle().ItemSpacing.x * 3;
    ImVec2 bsz{bp, bp};
    ImVec2 opt{60.0f * sc, 20.0f * sc};

    // NGP face: A on the left, B on the right.
    render_dpad_row(ctr, port, dp, 0);
    render_dpad_row(ctr, port, dp, 1, true);
    vgap(gap); ImGui::SameLine();
    vbtn("A", bsz, find_btn(ctr, DBCID_co_fire1), port); ImGui::SameLine();
    ...
    // OPTION sits below the d-pad on the real unit.
    vbtn("OPTION", opt, find_btn(ctr, DBCID_co_start), port);
                           ...
                           bool show_controller, bool show_keyboard)
{
    ...
    if (kind == jsm::systems::NEOGEO_POCKET || kind == jsm::systems::NEOGEO_POCKET_COLOR) {
        if (!fsys->io.controller1.vec) return;
        physical_io_device &pio = fsys->io.controller1.get();
        if (!pio.connected || !pio.enabled) return;
        render_ngp_controller(pio.controller, 1);
        return;
    }
    ...
    if (kind == jsm::systems::TURBOGRAFX16 || kind == jsm::systems::TURBOGRAFX16_CD
                                           || kind == jsm::systems::TURBOGRAFX16_ARCADE_CD) {
        if (!fsys->io.controller1.vec) return;
        physical_io_device &pio = fsys->io.controller1.get();
        if (!pio.connected || !pio.enabled) return;
