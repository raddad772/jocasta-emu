...
#include "component/cpu/arm32/disassemblers/disassemblers.h"
...
struct arm7_test {
    char name[100];
    arm7_test_state initial;
    arm7_test_state final;
    arm7_test_transactions transactions;
    u32 num_cycles;
    u32 opcode;
    u32 base_addr;
...
struct arm7_test_struct {
    explicit arm7_test_struct(u64 *waitstates) : cpu(nullptr, &cycles_executing, waitstates) {

    }
    ARM32::core<ARM32::AT_ARM7TDMI, scheduler_t> cpu;
    arm7_test test;
    arm7_test_transactions my_transactions;
    u64 cycles_executing;
    u64 trace_cycles;
};

static u8* load_state(arm7_test_state *state, u8 *ptr, u32 initial)
{
    u8 *start = ptr;
    u32 state_sz = R32;
    u32 kind = R32;
...
static u8* load_transactions(arm7_test_transactions *trs, u8 *ptr)
{
    u8 *start = ptr;
    u32 full_sz = R32;
    u32 mn = R32;
    ...
    for(u32 i = 0; i < num_transactions; i++) {
        transaction *t = &trs->items[trs->num];
        trs->num++;
        u32 a = R32;
        assert(a < 4);
        t->kind = static_cast<transaction_kind>(a);
        t->size = R32;
        t->addr = R32;
        t->data = R32;
        t->cycle = R32;
...
static u32 fetchins_test_cpu(void *ptr, u32 addr, u8 access)
{
    addr &= maskalign[sz];
    auto *ts = static_cast<arm7_test_struct *>(ptr);
    u32 v = 0;
    u32 mask = 0;
    switch(sz) {
    ...
    transaction *myt = &ts->my_transactions.items[ts->my_transactions.num++];
    myt->kind = static_cast<transaction_kind>(0);
    ...
    transaction *theirt = nullptr;
    for (u32 i = 0; i < ts->test.transactions.num; i++) {
        transaction *t = &ts->test.transactions.items[i];
        if ((t->addr == addr) && (t->kind == 0) && (t->visited == 0) && (t->size == sz)) {
            theirt = t;
        }
    }
    if (theirt) theirt->visited = 1;
    else {
        printf("\nCOULDNT FIND TRANSACTION!");
    }
    ts->cycles_executing++;
    return (u32)v & mask;
}

template<u8 sz, bool do_debug, bool is_peek>
static u32 read_test_cpu(void *ptr, u32 addr, u8 access)
{
    //addr &= maskalign[sz];
    transaction *theirt = nullptr;
    ...
    if constexpr (!is_peek) {
        auto *ts = static_cast<arm7_test_struct *>(ptr);
        transaction *myt = &ts->my_transactions.items[ts->my_transactions.num++];
        myt->kind = static_cast<transaction_kind>(1);
        myt->cycle = ts->trace_cycles;
        myt->addr = addr;
        myt->size = sz;
        myt->access = access;
...
template<u8 sz, bool do_debug>
static void write_test_cpu(void *ptr, u32 addr, u8 access, u32 val)
{
    //printf("\nWRITE %08x", addr);
    auto *ts = static_cast<arm7_test_struct *>(ptr);
    transaction *myt = &ts->my_transactions.items[ts->my_transactions.num++];
    //printf("\nWRITE ADDR:%08x VAL:%08x", addr, val);
    myt->kind = static_cast<transaction_kind>(2);
    myt->cycle = ts->trace_cycles;
    myt->addr = addr;
    myt->data = val;
    myt->size = sz;
...
static u32 do_test_read_trace(void *ptr, u32 addr, u8 sz) {
    switch (sz) {
        case 1: return read_test_cpu<1, false, true>(ptr, addr, 0);
        case 2: return read_test_cpu<2, false, true>(ptr, addr, 0);
        case 4: return read_test_cpu<4, false, true>(ptr, addr, 0);
    }
    NOGOHERE;
}
...
static void copy_state_to_cpu(ARM32::core<ARM32::AT_ARM7TDMI, scheduler_t>* cpu, arm7_test_state *ts)
{
    for (u32 i = 0; i < 16; i++) {
        cpu->regs.R[i] = ts->R[i];
...
static u32 compare_transactions(arm7_test_struct *ts)
{
    ...
    for (u32 i = 0; i < ts->my_transactions.num; i++) {
        transaction *my = &ts->my_transactions.items[i];
        transaction *their = &ts->test.transactions.items[i];
...
static u32 compare_state_to_cpu(arm7_test_struct *ts, arm7_test_state *final, arm7_test_state *initial, u32 c_skip)
{
    u32 all_passed = 1;
...
static u32 dopppt(char *ptr, size_t sz, transaction *t)
{
    char *m = ptr;

    m += snprintf(ptr, sz, "(%02d) %c.%c %06x:%08x/%d  ", t->cycle, kkttr[t->kind], t->size == 1 ? '1' : t->size == 2 ? '2' : '4', t->addr, t->data, t->access);
    return m - ptr;
}

static void pprint_transactions(arm7_test_transactions *ts, arm7_test_transactions *mts, arm7_test_struct *test)
{
    char buf[500];
    printf("\n---Transactions");
    printf("\n    Theirs (%02d)                             Mine (%02d)", ts->num, mts->num);
    ...
    while(1) {
        ...
        if ((t1 != nullptr) && (t2 != nullptr)) {
            u32 c = t2->data;
            if (t1->data != t2->data) ptr += snprintf(ptr, sizeof(buf) - (ptr - buf),"!");
            else ptr += snprintf(ptr,  sizeof(buf) - (ptr - buf), " ");
            if (t1->addr != t2->addr) ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "-");
            else ptr += snprintf(ptr,  sizeof(buf) - (ptr - buf), " ");
        }
        else ptr += snprintf(ptr,  sizeof(buf) - (ptr - buf), "  ");
        if (t1 == nullptr) {
            ...
            ptr += dopppt(ptr,  sizeof(buf) - (ptr - buf), t1);
        }
        if (t2 != nullptr) {
            ptr += dopppt(ptr,  sizeof(buf) - (ptr - buf), t2);
        }
        printf("\n%s", buf);
...
static u32 do_test(arm7_test_struct *ts, const char*file, const char *fname, u32 c_skip) {
    ...
    for (u32 i = 0; i < numtests; i++) {
        ...
        u32 topcode = ts->test.opcode;
        if (ts->cpu.regs.CPSR.T)
            ts->cpu.run_instruction<false>();
        else {
            //u32 opcode = ts->cpu.pipeline.opcode[0];
            u32 opcode_addr = ts->cpu.pipeline.addr[0];
            ts->cpu.pipeline.opcode[0] = ts->cpu.pipeline.opcode[1];
            ts->cpu.pipeline.addr[0] = ts->cpu.pipeline.addr[1];
            ts->cpu.regs.R[15] &= 0xFFFFFFFE;

            ts->cpu.pipeline.opcode[1] = ts->cpu.fetch_ins<4, true>();
            ts->cpu.pipeline.addr[1] = ts->cpu.regs.R[15];

            ARM32::arm32_cached_ins<ARM32::AT_ARM7TDMI, scheduler_t> ci;
            ts->cpu.cached_decode_arm32<false>(topcode, ci);
            if (ts->cpu.condition_passes(ci.cc))
                (ts->cpu.*ci.exec_debug)(ci);
            else {
                ts->cpu.regs.R[15] += 4;
                ts->cpu.pipeline.access = ARM32P_code | ARM32P_sequential;
            }
        }
        ...
        if ((!compare_state_to_cpu(ts, &ts->test.final, &ts->test.initial, c_skip)) || !compare_transactions(ts) || ts->test.failed) {
            printf("\n\nTest %d failed! %s opcode:%08x", i, fname, ts->test.opcode);
            jsm_string out{100};
            if (ts->test.initial.CPSR & 0x20) {
                ARM32::THUMBv4_disassemble(ts->test.opcode, &out, -1, nullptr);
            }
            else {
                ARM32::ARMv4_disassemble(ts->test.opcode, &out, -1, nullptr);
            }
...
void test_arm7tdmi_cached()
{
    ...
    arm7_test_struct ts(&blah);
    rt.ptr = &ts;

    ts.cpu.setup_tracing(&rt, &ts.trace_cycles, 0);
    ts.cpu.read_func8 = &read_test_cpu<1, false, false>;
    ts.cpu.read_func16 = &read_test_cpu<2, false, false>;
    ts.cpu.read_func32 = &read_test_cpu<4, false, false>;
    ts.cpu.write_func8 = &write_test_cpu<1, false>;
    ts.cpu.write_func16 = &write_test_cpu<2, false>;
    ts.cpu.write_func32 = &write_test_cpu<4, false>;
    ts.cpu.fetch_ins_func16 = &fetchins_test_cpu<2, false>;
    ts.cpu.fetch_ins_func32 = &fetchins_test_cpu<4, false>;

    ts.cpu.read_func8_debug = &read_test_cpu<1, true, false>;
    ts.cpu.read_func16_debug = &read_test_cpu<2, true, false>;
    ts.cpu.read_func32_debug = &read_test_cpu<4, true, false>;
    ts.cpu.write_func8_debug = &write_test_cpu<1, true>;
    ts.cpu.write_func16_debug = &write_test_cpu<2, true>;
    ts.cpu.write_func32_debug = &write_test_cpu<4, true>;
    ts.cpu.fetch_ins_func16_debug = &fetchins_test_cpu<2, true>;
    ts.cpu.fetch_ins_func32_debug = &fetchins_test_cpu<4, true>;
    ...
    char PATH[500];
    construct_cpu_test_path(PATH, "ARM7TDMI", "", sizeof(PATH));
    ...
    u32 nn = 45;
    u32 test_start = 0;
    filebuf = static_cast<char *>(malloc(FILE_BUF_SIZE));
