...
void core::trace_format(u32 opcode)
{
    ...
    //u32 do_tracething = (dbg.tvptr && dbg.do_debug && dbg.traces.huc6280.instruction);
...
u8 core::internal_read(u32 addr, bool has_effect) const
{
    ...
    switch(k) {
        ...
        case 0x1000: // IO
            ...
            val |= 0b10110000; // TurboGrafx-16, no CD-ROM add-on
            break;
        case 0x1400: // IRQ
            if (pins.BM) return 0;
            switch(addr & 3) {
            ...
            break;
        case 0x1800:
            val = read_func(read_ptr, addr, val, true);
            break;
