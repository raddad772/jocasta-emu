...
void core::multiply_matrix_by_vector_MVMVA(CMD *config, MATRIX mat, u8 vei, control_vector crv)
{
    ...
    for (u32 r = 0; r < 3; r++) {
        // When using far-color as the translation vector, the FC value must NOT
        // participate in the 44-bit MAC overflow check (A flags). Only the product
        // M[r][0]*V[0] is checked at c=0; FC is added after, without a re-check.
        i64 res = far_color ? 0 : static_cast<i64>(control_vectors[crv][r]) << 12;
        for (u32 c = 0; c < 3; c++) {
            const i32 v = V[vector_index][c];
            const i32 m = matrices[mat][r][c];
            ...
            res = i64_to_i44(static_cast<u8>(r), res + static_cast<i64>(product));
            if (far_color && c == 0) {
                res += static_cast<i64>(control_vectors[crv][r]) << 12;
                i32_to_i16_saturate(config, r, static_cast<i32>(res >> config->shift));
                res = 0;
            }
        }
