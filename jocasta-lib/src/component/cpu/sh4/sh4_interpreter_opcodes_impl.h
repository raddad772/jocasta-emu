...
#define READ8(addr) mem_read<1, do_debug>(addr)
#define READ16(addr) mem_read<2, do_debug>(addr)
#define READ32(addr) mem_read<4, do_debug>(addr)
#define READ64(addr) mem_read<8, do_debug>(addr)
#define WRITE8(addr, val) mem_write<1, do_debug>(addr, (val) & 0xFF)
#define WRITE16(addr, val) mem_write<2, do_debug>(addr, (val) & 0xFFFF)
#define WRITE32(addr, val) mem_write<4, do_debug>(addr, (val) & 0xFFFFFFFF)
#define WRITE64(addr, val) mem_write<8, do_debug>(addr, val)


#define FP64SAVE(addr, from) u64 v = (from); WRITE64(addr, (v << 32) | (v >> 32))
#define FP64LOAD(addr, to) u64 v = READ64(addr); (to) = (v << 32) | (v >> 32)

#define SH4ins(x) template<bool do_debug> void core::ins_##x(ins_t *ins)

#define DELAY_SLOT { PCinc; fetch_and_exec<do_debug>(true); }
#define fQ regs.SR.Q
#define fM regs.SR.M
#define fT regs.SR.T
...
SH4ins(CMPIM) { // If R0 = (sign extension)imm: 1 -> T, Else: 0 -> T
    regs.SR.T = R(0) == sign_extend<8>(IMM);
    PCinc;
}
...
SH4ins(BF) { // If T = 0: disp*2 + PC + 4 -> PC, Else: nop
    ...
    /*i64 val = rPC + 2;
    val += ((2 * (i32)sign_extend<8>(DISP)) + 2) * (i32)(regs.SR.T ^ 1);
    rPC = (u32)val;*/

    i32 disp = static_cast<i32>(sign_extend<8>(DISP));
...
SH4ins(BFS) { // If T = 0: disp*2 + PC + 4 -> PC, Else: nop, (Delayed branch)
    ...
    /*i64 val = (i64)rPC + 4;
    val += (2 * (i32)sign_extend<8>(DISP)) * (i32)(regs.SR.T ^ 1);
    DELAY_SLOT;
...
SH4ins(BT) { // If T = 1: disp*2 + rPC + 4 -> rPC, Else: nop
    /*i64 val = (i64)rPC + 2;
    val += ((2 * (i32)sign_extend<8>(DISP)) + 2) * (i32)regs.SR.T;
    rPC = (u32)val;*/
    i32 disp = static_cast<i32>(sign_extend<8>(DISP));
...
SH4ins(BTS) { // If T = 1: disp*2 + PC + 4 -> PC, Else: nop, (Delayed branch)
    /*i64 val = (i64)rPC + 4;
    val += (2 * (i32)sign_extend<8>(DISP)) * (i32)regs.SR.T;*/
    i32 disp = static_cast<i32>(sign_extend<8>(DISP));
    u32 temp = rPC;
...
SH4ins(BRA) { // disp*2 + PC + 4 -> PC, (Delayed branch)
    /*i64 val = (i64)rPC + 4;
    val += (2 * (i32)sign_extend<12>(DISP));
    DELAY_SLOT;
    rPC = (u32)val;*/
    i32 disp = static_cast<i32>(sign_extend<12>(DISP));
    u32 temp = rPC;
    temp += 4 + (disp << 1);
    DELAY_SLOT;
    rPC = temp;
...
SH4ins(BSR) { // PC + 4 -> PR, disp*2 + PC + 4 -> PC, (Delayed branch)
    /*i64 val = (i32)sign_extend<12>(DISP) * 2;
    regs.PR = rPC + 4;
    val = rPC + 4 + val;
    DELAY_SLOT;
    rPC = (u32)val;*/
...
SH4ins(FTRC_single) { // (long)FRm -> FPUL
    ...
    {
        i32 bits = std::bit_cast<i32>(fpFRm);
        if (bits > 0) // Using integer math to avoid issues with Inf and NaN
            regs.FPUL.u--;
    }
