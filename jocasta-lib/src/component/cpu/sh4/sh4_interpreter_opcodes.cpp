...
#define READ8(addr) mem_read(addr, 1)
#define READ16(addr) mem_read(addr, 2)
#define READ32(addr) mem_read(addr, 4)
#define READ64(addr) mem_read(addr, 8)
#define WRITE8(addr, val) mem_write(addr, 1, (val) & 0xFF)
#define WRITE16(addr, val) mem_write(addr, 2, (val) & 0xFFFF)
#define WRITE32(addr, val) mem_write(addr, 4, (val) & 0xFFFFFFFF)
#define WRITE64(addr, val) mem_write(addr, 8, val)
...
#define DELAY_SLOT { PCinc; fetch_and_exec<true>(true); }
#define fQ regs.SR.Q
#define fM regs.SR.M
#define fT regs.SR.T
...
static void cpSH4(u32 dest, u32 src) {
    memcpy(&decoded[0][dest], &decoded[0][src], 65536*sizeof(ins_t));
    memcpy(&decoded[1][dest], &decoded[1][src], 65536*sizeof(ins_t));
    memcpy(&disassembled[dest][0], &disassembled[src][0], 65536*30);
    memcpy(&mnemonic[dest][0], &mnemonic[src][0], 65535*30);
}

static void iterate_opcodes(sh4_str_ret* r, ins_func ins0, ins_func ins1, const char* mnem, u32 override, u32 szpr)
{
        ...
        for (u32 m = 0; m < m_max; m++) {
            for (u32 d = 0; d < d_max; d++) {
                for (u32 i = 0; i < i_max; i++) {
                    ...
                    ins_t *is = &decoded[0][szpr][opcode];
                    ...
                    if (szpr > 0) {
                        // copy to szpr=3
                        memcpy(&decoded[0][3][opcode], &decoded[0][szpr][opcode], sizeof(ins_t));
                        memcpy(&decoded[1][3][opcode], &decoded[1][szpr][opcode], sizeof(ins_t));
...
static void decode_and_iterate_opcodes(const char* inpt, ins_func ins0, ins_func ins1, const char *mnem, u32 override, u32 szpr)
{
    sh4_str_ret a;
    process_instruct(&a, inpt);
    iterate_opcodes(&a, ins0, ins1, mnem, override, szpr);
}


#define OE(opcstr, func, mn) decode_and_iterate_opcodes(opcstr, func<false>, func<true>, mn, 0, 0)
#define OEo(opcstr, func, mn, sz, pr) decode_and_iterate_opcodes(opcstr, func<false>, func<true>, mn, 1, ((sz<<1) | pr))
...
void core::do_sh4_decode() {
    ...
    for (u32 szpr = 0; szpr < 4; szpr++) {
        for (u32 i = 0; i < 65536; i++) {
            if (!decoded[0][szpr][i].decoded) {
                decoded[0][szpr][i].exec = &core::ins_EMPTY<false>;
                decoded[1][szpr][i].exec = &core::ins_EMPTY<true>;
                snprintf(disassembled[szpr][i], 30, "UNKNOWN OPCODE %04x", i);
                snprintf(mnemonic[szpr][i], 30, "UNKNOWN OPCODE %04x", i);
                unencoded++;
                decoded[0][szpr][i].decoded = decoded[1][szpr][i].decoded = true;
            }
        }
    }
}
