...
u32 TMU_old::read_TCNT(u32 ch, bool is_regread) const {
    return REGTCNT;
}

i64 TMU_old::read_TCNT64(u32 ch) const {
    return static_cast<i64>(base64[ch]) - static_cast<i64>((TCUSE >> shift[ch]) & mask64[ch]);
}


void TMU_old::sched_chan_tick(u32 ch)
{
    ...
    if (mask[ch]) {
        i64 tcode = *master_cycles + cycles;
        cpu->scheduler->add_or_run_abs(tcode, ch, this, &scheduled_tmu_callback, nullptr);
        printf("\nSCHEDULE CHAN %d TICK FOR %lld", ch, *master_cycles);
    }
    else {
        printf("\nINSTANT TIMER TICK RUN? %d", ch);
        cpu->scheduler->add_or_run_abs(-1, ch, this, &scheduled_tmu_callback, nullptr);
    }
}


void TMU_old::write_TCNT(u32 ch, u32 data)
{
    printf("\nT%d write CNT: %04x cyc:%lld", ch, data, *master_cycles);
...
void TMU_old::turn_onoff(u32 ch, u32 on)
{
    u32 mTCNT = read_TCNT(ch, false);
    printf("\nTURN CH%d TO %d cyc:%llu", ch, on, *master_cycles);
...
void TMU_old::write_TSTR(u32 val)
{
    TSTR = val;
    for (u32 i = 0; i < 3; i++) {
...
void TMU_old::update_counts(u32 ch)
{
...
void TMU_old::write_TCR(u32 ch, u32 val)
{
    TCR[ch] = val & 0xFFFF;
    update_counts(ch);
}

void TMU_old::write(u32 addr, u8 sz, u64 val, bool* success)
{
...
u64 TMU_old::read(u32 addr, u8 sz, bool* success)
{
    switch(addr | 0xF0000000) {
        case 0xFFD80004: // TSTR
            return TSTR;
...
void scheduled_tmu_callback(void *ptr, u64 key, u64 sch_cycle, u32 jitter)
{
    TMU_old *th = static_cast<TMU_old *>(ptr);
    u32 ch = key;
    printf("\nSCHEDULED CALLBACK! %d %08x cyc:%llu", ch, th->mask[ch], *th->master_cycles);
    if (th->mask[ch]) {
        u32 tcnt = th->read_TCNT(ch, false);
