...
void core::interrupt() {
    //sh4dbgloglog(trace.exception_id, DBGLS_TRACE, "IRQ PRIO:%d IMASK:%d", interrupt_highest_priority, regs.SR.IMASK);
    ...
    assert(src);
    //printf("\nIRQ! SRC %d", src->source);
    regs.INTEVT = src->intevt;
    if (src->source != IRQ_irl) src->raised = 0; // IRL is de-asserted externally
    IRQ_set_highest_priority();
    if (dbg.dvptr && dbg.dvptr->ids_enabled[trace.irq_id]) {
        sh4dbgloglog(trace.irq_id, DBGLS_TRACE, "IRQ %s @ PC %08x", IRQ_NAMES[src->source], regs.PC);
    }
    exec_interrupt();
}
    ...
    static void bubble_sort(void *ptrs[], u8 sz, int (*compare_func)(const void *a, const void *b))
{
    ...
    do {
        ...
        const u32 limit = static_cast<u32>(sz) - 1;
        for (u32 i = 0; i < limit; i++) {
            if (compare_func(ptrs[i], ptrs[i + 1]) > 0) {
                ptr_tmp = ptrs[i];
                ptrs[i] = ptrs[i + 1];
                ptrs[i + 1] = ptr_tmp;
                swapped = true;
