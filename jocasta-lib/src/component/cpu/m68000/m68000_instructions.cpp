...
INS(BCC)
        ...
        STEP(1)
            if (!state.instruction.result) {
                ...
                u32 cnt = sign_extend<8>(ins->ea[1].reg);
                if (cnt == 0) cnt = sign_extend<16>(regs.IRC);
                regs.PC -= 2;
                //regs.IPC -= 2;
                regs.IPC += 2;
                regs.PC += cnt;
...
INS(BRA)
        STEP0
            ...
            start_wait(2, S_exec);
        STEP(1)
            u32 cnt = sign_extend<8>(ins->ea[0].reg);
            if (cnt == 0) cnt = sign_extend<16>(regs.IRC);
            regs.PC -= 2;
            regs.IPC -= 2;
            regs.PC += cnt;
...
INS(BSR)
        ...
        STEP(2)
            i32 offset;
            if (ins->ea[0].reg != 0) {
                offset = sign_extend<8>(ins->ea[0].reg);
                //offset = sign_extend<8>(offset);
            } else {
                offset = sign_extend<16>(regs.IRC);
            }
...
INS(MOVEA)
        ...
        STEP(1)
            ...
            if (ins->sz == 2) v = sign_extend<16>(v);
            if (ins->sz == 1) v = sign_extend<8>(v);
...
INS(MOVEQ)
        STEP0
            regs.D[ins->ea[1].reg] = sign_extend<8>(ins->ea[0].reg);
            regs.SR.C = regs.SR.V = 0;
...
INS(TST)
        ...
        STEP(1)
            ...
            regs.SR.Z = v == 0;
            if (ins->sz == 1) v = sign_extend<8>(v);
            else if (ins->sz == 2) v = sign_extend<16>(v);
