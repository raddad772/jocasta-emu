...
struct ins_t;
typedef void (*iack_handler_f)(void*);
struct iack_handler {
    iack_handler_f handler[2];
    void *ptr;
};
...
private:
    ...
    void decode();
    void trace_format();
    template<bool do_debug> u32 process_interrupts();
    ...
    void swap_ASP();
    void bus_cycle_iaq();
    template<bool do_debug> void exc_interrupt();
    void exc_group0();
    void exc_group12();
    ...
    u32 read_ea_addr(u32 opnum, u32 sz, bool hold, u32 prefetch);
...
#undef INS
    ...
    void register_iack_handler(void *ptr, iack_handler_f debug_off, iack_handler_f debug_on);
    ...
    DBG_START
        DBG_EVENT_VIEW_START
            IRQ
        DBG_EVENT_VIEW_END
        DBG_LOG_VIEW_SIMPLE u32 irq_id{};
    DBG_END

};

#ifndef M68K_NOIMPL
#include "m68000_exec.h"
#endif
