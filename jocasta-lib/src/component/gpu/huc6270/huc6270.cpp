...
void chip::latch_bg_regs()
{
    latch.bg.x_tiles = io.bg.x_tiles;
    latch.bg.y_tiles = io.bg.y_tiles;
    latch.bg.x_tiles_mask = latch.bg.x_tiles - 1;
    latch.bg.y_tiles_mask = latch.bg.y_tiles - 1;

    bg.x_tiles = latch.bg.x_tiles;
    bg.y_tiles = latch.bg.y_tiles;
    bg.x_tiles_mask = latch.bg.x_tiles_mask;
    bg.y_tiles_mask = latch.bg.y_tiles_mask;
    regs.BAT_size = bg.x_tiles * bg.y_tiles;

}

void chip::latch_h_regs()
{
    latch.HSW = io.HSW;
    latch.HDS = io.HDS;
    latch.HDW = io.HDW;
...
u16 chip::read_VRAM(u32 addr)
{
    if (addr < 0x8000) {
        regs.vram_open_bus = VRAM[addr];
    }
    return regs.vram_open_bus;
}
...
{
    latch.BYR = io.BYR;
    regs.yscroll = latch.BYR.u;
    regs.next_yscroll = regs.yscroll;
    regs.yscroll_pending = 0;
}
...
void chip::eval_sprites() {
    ...
    for (u32 sn = 0; sn < 64; sn++) {
        ...
        u32 cgx = (spr[3] >> 8) & 1;
        i32 width = sprite_widths_px[cgx];
...
void chip::setup_new_line() {
    ...
    if (timing.v.state == S_display || ((timing.v.state == S_wait_for_display) && (timing.v.counter == 1))) {
        ...
        latch.BXR = io.BXR;
        if (regs.yscroll_pending) {
            latch.BYR = io.BYR;
            regs.next_yscroll = latch.BYR.u;
            regs.yscroll_pending = 0;
        }
        ...
        regs.first_render = 1;
        regs.draw_clock = 0;
        bg.x_tile = (latch.BXR.u >> 3) & bg.x_tiles_mask;
...
void chip::new_h_state(states st)
{
    ...
    switch(st) {
        case S_sync_window:
            latch_h_regs();
            timing.h.counter = (latch.HSW+1) << 3;
        ...
        case S_wait_for_display:
            timing.h.counter = (latch.HDS+1) << 3;
        ...
        case S_display:
            ...
            hblank(false);
            regs.x_counter = 0;
            timing.h.counter = (latch.HDW+1) << 3;
            break;
        case S_wait_for_sync_window:
            setup_new_line();
            hblank(true);
            timing.h.counter = (latch.HDE+1) << 3;
...
void chip::new_v_state(states st)
{
    ...
    switch(st) {
        case S_sync_window:
            vblank(false);
            latch_v_regs();
            timing.v.counter = latch.VSW + 1;
            break;
        case S_wait_for_display:
            vblank(false);
            setup_new_frame();
            timing.v.counter = latch.VDS + 2;
            break;
        case S_display:
            ...
            timing.v.counter = latch.VDW.u + 2; // It will get decremented by setup_new_line()
...
void chip::cycle()
{
    ...
    if ((timing.v.state == S_display) && (timing.h.state == S_display)) {
...
#endif
            u32 scroll_discard = latch.BXR.u & 7;
            if (scroll_discard) {
        ...
        if (pixel_shifter.num > 0) {
            ...
            if (!latch.bg_on) bg_color = 0;
            if (!latch.sprites_on) sp_color = 0;
    ...
    if ((timing.h.state == S_display) && (regs.x_counter == ((static_cast<i32>(latch.HDW) - 1) * 8 + 2)))
...
void chip::reset()
{
    io.bg.x_tiles = 32;
    io.bg.y_tiles = 32;
    io.bg.x_tiles_mask = 31;
    io.bg.y_tiles_mask = 31;
    io.HSW = 2;
    io.HDS = 2;
    io.HDW = 31;
    io.HDE = 4;
...
void chip::update_RCR(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    if (signal && (signal != th->io.STATUS.RR)) {
...
void chip::vblank(bool val)
{
    ...
    if (val) {
        ...
        if (regs.vram_satb_pending || io.DCR.DSR)
            vram_satb();
    }
    else {
        update_irqs();
...
void chip::write_lsb(u8 val)
{
    ...
    switch(io.ADDR) {
        ...
        case 0x09: {
            ...
            io.bg.x_tiles_mask = io.bg.x_tiles - 1;
            io.bg.y_tiles_mask = io.bg.y_tiles - 1;
            return; }
        case 0x0A:
            io.HSW = val & 0x1F;
            return;
