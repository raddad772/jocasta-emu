...
void chip::reset()
{
    regs.next_clock_div = regs.clock_div;
...
void chip::schedule_scanline(void *ptr, u64 key, u64 cclock, u32 jitter)
{
    ...
    if (th->regs.clock_div != th->regs.next_clock_div) {
        th->regs.clock_div = th->regs.next_clock_div;
        th->vdc0->regs.divisor = th->regs.clock_div;
    }
...
void chip::write(u32 maddr, u8 val)
{
    ...
    switch(addr) {
        case 0: //CR
            io.DCC = val;
            regs.next_clock_div = 4 - (val & 3);
            if ((val & 3) == 3) {
                printf("\nWARN ILLEGAL DIVISOR");
                regs.next_clock_div = 2;
            }
...
void chip::pixel_clock(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    for (u32 i = 0; i < th->regs.clock_div; i++) {
