...
#define S(x) Sadd(state, &(x), sizeof(x))
    S(bus.DMA_OUT); S(bus.SC); S(bus.D); S(bus.IRQ); S(bus.EF1);
    S(x); S(y); S(master_frame);
    u8 da = display_area ? 1 : 0;
    S(da);
    S(io.enable); S(io.latch.enable);
#undef S
}

void core::deserialize(serialized_state &state) {
#define L(x) Sload(state, &(x), sizeof(x))
    L(bus.DMA_OUT); L(bus.SC); L(bus.D); L(bus.IRQ); L(bus.EF1);
    L(x); L(y); L(master_frame);
    u8 da;
    L(da);
    display_area = da != 0;
    L(io.enable); L(io.latch.enable);
#undef L
}
