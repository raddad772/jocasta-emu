...
// From Nuked-OPN2: extra increment steps for high rates (rate >= 48), indexed by [rate&3][timer_low]
...
// From Nuked-OPN2: LFO PM shift tables indexed by [pms][lfo_l]
...
void do_math()
{
    ...
    for (u32 mn = 0; mn < 0x200; mn++) {
        ...
        double n = static_cast<double>((i << 1) | 1);
...
u16 LFO::fm(const u8 lfo_counter, const u8 pms, const u16 f_num)
{
    ...
    u32 fnum   = f_num;
    u32 fnum_h = fnum >> 4;   // computed before the <<1, matching Nuked
    fnum <<= 1;

    // Triangular lfo_l: maps lfo_counter[5:2] into 0..7 triangle
...
void OPERATOR::run_env()
{
    ...
    if (!ch->ym2612->eg_global.tick) return;
    ...
    if (envelope.state == EP_decay && (envelope.attenuation & ~0xF) == sustain_level) {
    ...
    // Compute increment using Nuked's exact timer-based mechanism
    u8 eg_inc = 0;
    if (rate > 0) {
        const u8 sl = ch->ym2612->eg_global.shift_lock;
        const u8 tl = ch->ym2612->eg_global.timer_low;
        if (rate < 48) {
...
void core::cycle()
{
    // Advance global EG timer -- matches Nuked's quotient/shift_lock mechanism.
    // Ticks once every 3 calls; shift_lock encodes the lowest set bit of the timer
    // so each operator can determine its update rate exactly as the hardware does.
    eg_global.quotient = (eg_global.quotient + 1) % 3;
    eg_global.tick = (eg_global.quotient == 0);
    if (eg_global.tick) {
        eg_global.timer_low = eg_global.timer & 3;         // latch before increment
        eg_global.timer = (eg_global.timer + 1) & 0xFFF;
        if (eg_global.timer == 0) {
