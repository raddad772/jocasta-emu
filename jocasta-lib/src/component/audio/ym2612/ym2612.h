...
struct OPERATOR {
    ...
    struct YM2612_ENV {
        ...
        u32 prev_sustain_level{}; // delayed copy, matches hardware pipeline
...
public:
    ...
    struct {
        u16 timer{};      // 12-bit cascading counter
        u8  timer_low{};  // bottom 2 bits, latched before each tick
        u8  shift_lock{}; // 1 + position of lowest set bit of timer (0 if timer==0)
        u8  quotient{};   // cycles 0->1->2->0; tick fires when it wraps to 0
        bool tick{};      // true for exactly one cycle() call per three
    } eg_global{};
