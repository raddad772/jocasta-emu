        ...
        switch (reg) {
            ...
            case 1: case 3: case 5: case 7: {
                ...
                if (!(selected_drive->phases & (1u << p))) {
                    ...
                    int curr_qt = static_cast<int>(selected_drive->quarter_track);
                    int delta = static_cast<int>(p) - (curr_qt & 3); // raw, may be +/-3
                    if (delta > 2) delta -= 4;             // choose shorter path
                    if (delta < -2) delta += 4;             // delta now in {-2,-1,0,+1,+2}

                    int max_qt = selected_drive->disc ? static_cast<int>(selected_drive->disc->disc.tracks.size() * 2 - 2) : 68; // 35-track image ... QT 0..68
                    int new_qt = curr_qt + delta;
                    if (new_qt < 0)       new_qt = 0;
                    if (new_qt > max_qt)  new_qt = max_qt;

                    if (new_qt != curr_qt) {
                        selected_drive->quarter_track = static_cast<u32>(new_qt);
                        u32 new_track = static_cast<u32>(new_qt) >> 1;
