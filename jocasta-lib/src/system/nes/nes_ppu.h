...
#ifndef JOCASTA_EMUS_NES_PPU_H
#define JOCASTA_EMUS_NES_PPU_H
...
private:
    ...
    void new_frame();
    void set_scanline(i32 to);
    void new_scanline();

public:
    explicit NES_PPU(NES::core *nes);
    void reset();
    ...
    u32 prev_rendering_enabled = 1; // 1-cycle lag behind new_rendering_enabled (gives 2-cycle total delay from write to effect)
    u32 new_rendering_enabled = 1;

    // $2006 second-write V-update delay (hardware applies V 3 PPU cycles after the write)
    i32 v_update_delay = 0;   // counts down from 3 to 0; 0 = no pending update
    u32 v_pending = 0;        // the T value captured at write time
