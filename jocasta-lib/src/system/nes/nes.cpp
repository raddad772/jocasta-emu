...
}  // namespace NES  -- the global factory below must NOT be inside it (called by new_system)
...
static void NESIO_load_cart(JSM, multi_file_set &mfs, physical_io_device &pio) {
    auto *nes = dynamic_cast<core *>(jsm);
...
static void setup_crt(JSM_DISPLAY *d)
{
    ...
    // NTSC crystal 21.477272 MHz, 341 px x 262 lines x 4 = 357368 ... ~60.0988 Hz
...
void core::setup_audio(std::vector<physical_io_device> &inIOs)
{
    ...
    chan->sample_rate = static_cast<u32>(static_cast<double>(APU_CYCLES_PER_FRAME) * (21477272.0 / (341.0 * 262.0 * 4.0)));
