...
void CLOCK::serialize(serialized_state &state) const {
...
void CLOCK::deserialize(serialized_state &state) {
...
void core::serialize(serialized_state &state) const {
...
static void serialize_apu(core &nes, serialized_state &state) {
...
void PPU::serialize(serialized_state &state) {
    state.new_section("PPU", NESSS_ppu, 1);
#define S(x) Sadd(state, &x, sizeof(x))

    // render_cycle...//visible, postrender, prerender
    u32 v = 0;
    if (render_cycle == &PPU::scanline_visible) v = 1;
    if (render_cycle == &PPU::scanline_postrender) v = 2;
...
void serialize_cart(core &nes, serialized_state &state) {
...
void deserialize_apu(core &nes, serialized_state &state) {
...
void PPU::deserialize(serialized_state &state) {
#define L(x) Sload(state, &x, sizeof(x))
    u32 v = 0;
...
void deserialize_cart(core &nes, serialized_state &state) {
