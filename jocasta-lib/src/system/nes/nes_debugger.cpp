...
static int print_dasm_addr(void *nesptr, u32 addr, char *out, size_t out_sz) {
    NES::core* th = static_cast<NES::core *>(nesptr);
...
static void create_and_bind_registers(NES::core &th, disassembly_view &dv)
{
    u32 tkindex = 0;
...
static void setup_disassembly_view(NES::core& th, debugger_interface *dbgr)
{
...
{
    NES::core* th = static_cast<NES::core *>(ptr);
    ...
    for (u32 tilemap_num = 0; tilemap_num < 2; tilemap_num++) {
        ...
        for (u32 tiles_num = 0; tiles_num < 256; tiles_num++) {
            ...
            for (u32 inner_y = 0; inner_y < 8; inner_y++) {
                u32 bp0 = th->PPU_read_noeffect(tile_addr);
                u32 bp1 = th->PPU_read_noeffect(tile_addr+8);
...
static void render_image_view_nametables(debugger_interface *dbgr, debugger_view *dview, void *ptr, u32 out_width)
{
    ...
    for (u32 i = 0; i < 240; i++) {
        NES::core::NESDBGDATA::DBGNESROW &row = th->dbg_data.rows[i];
        ...
        for (u32 nt_y = 0; nt_y < 2; nt_y++) {
            ...
            for (u32 get_tile_y = 0; get_tile_y < 30; get_tile_y++) {
                for (u32 inner_tile_y = 0; inner_tile_y < 8; inner_tile_y++) {
                    ...
                    NES::core::NESDBGDATA::DBGNESROW &row = th->dbg_data.rows[snum == -1 ? 0 : snum];
                    ...
                    for (u32 get_tile_x = 0; get_tile_x < 32; get_tile_x++) {
                        ...
                        u32 tilenum = th->PPU_read_noeffect(addr);
                        u32 tile_addr = 0x1000 * bg_pattern_table;
                        u32 rownum = inner_tile_y;
                        u32 taddr = tile_addr + (tilenum << 4) + rownum;
                        u32 p0 = th->PPU_read_noeffect(taddr);
                        u32 p1 = th->PPU_read_noeffect(taddr + 8);
...
static void setup_image_view_nametables(NES::core& th, debugger_interface *dbgr)
{
...
static void setup_image_view_tilemap(NES::core& th, debugger_interface *dbgr)
{
    th.dbg.image_views.nametables = dbgr->make_view(dview_image);
    debugger_view &dview = th.dbg.image_views.nametables.get();
    image_view &iv = dview.image;
...
static void setup_waveforms(NES::core& th, debugger_interface *dbgr)
{
...
static void setup_events_view(NES::core& th, debugger_interface *dbgr)
{
    // Setup events view
...
void NES::core::setup_debugger_interface(debugger_interface &intf)
{
    dbg.interface = &intf;
    auto* dbgr = &intf;
