...
void PPU::update_nmi() const
{
    nes->cpu.notify_NMI(status.nmi_out && io.nmi_enable);
}

void PPU::scanline_prerender() {
    // 261
    if (line_cycle == 1) {
        io.sprite0_hit = 0;
    ...
    if (rendering_enabled) {
        // Drive coarse-X increments, fine-Y increment, and horizontal T...V copy (cycle 257)
        cycle_scanline_addr();
        // Fetch background tiles for the first scanline
        perform_bg_fetches();
        // Reload vertical scroll components from T (cycles 280-304, prerender only)
...
void PPU::write_cgram(u32 addr, u32 val)
{
    nes->mapper->a12_watch(addr | 0x3F00);
    if ((addr & 0x13) == 0x10) addr &= 0xEF;
    CGRAM[addr & 0x1F] = val & 0x3F;
}

u32 PPU::read_cgram(u32 addr) const {
    if ((addr & 0x13) == 0x10) addr &= 0xEF;
...
void PPU::mem_write(u32 addr, u32 val)
{
    if ((addr & 0x3FFF) < 0x3F00) nes->PPU_write(addr, val);
    else write_cgram(addr & 0x1F, val);
}

void PPU::write_regs(u32 addr, u32 val)
{
    switch ((addr & 7) | 0x2000) {
        ...
        case 0x2006: // PPUADDR
            ...
            if (io.w == 0) {
                ...
                // Hardware delays the T...V copy by 3 PPU cycles (matches Mesen2).
                // The pending value is committed by the countdown in cycle().
                v_pending = io.t;
                v_update_delay = 3;
                io.w = 0;
            }
...
u32 PPU::read_regs(u32 addr, u32 val, u32 has_effect)
{
    u32 output = val;
    switch (addr) {
        case 0x2002:
        ...
        case 0x2007:
            ...
            if ((io.v & 0x3FFF) >= 0x3F00) {
                output = read_cgram(io.v & 0x1F);
                // Fill VRAM latch with the nametable byte mirrored under the palette
                latch.VRAM_read = nes->PPU_read_effect(io.v & 0x2FFF);
            }
            else {
                output = latch.VRAM_read;
                latch.VRAM_read = nes->PPU_read_effect(io.v & 0x3FFF);
            }
            io.v = (io.v + io.vram_increment) & 0x7FFF;
            //nes->a12_watch(nes, io.v & 0x3FFF);
            break;
        default:
...
u32 PPU::fetch_chr_line(u32 table, u32 tile, u32 line, u32 has_effect) {
    u32 r = (0x1000 * table) + (tile * 16) + line;
    last_sprite_addr = r + 8;
    u32 lo = nes->PPU_read_effect(r);
    u32 hi = nes->PPU_read_effect(r + 8);
    u32 output = 0;
    for (u32 i = 0; i < 8; i++) {
        output <<= 2;
        output |= (lo & 1) | ((hi & 1) << 1);
...
u32 PPU::fetch_chr_line_low(u32 addr) const {
    u32 low = nes->PPU_read_effect(addr);
    u32 output = 0;
    for (u32 i = 0; i < 8; i++) {
        output <<= 2;
        output |= (low & 1);
...
u32 PPU::fetch_chr_line_high(u32 addr, u32 o) const {
    u32 high = nes->PPU_read_effect(addr + 8);
    u32 output = 0;
    for (u32 i = 0; i < 8; i++) {
        output <<= 2;
        output |= ((high & 1) << 1);
    ...
    if ((line_cycle >= 257) && (line_cycle <= 320)) { // Sprite tile fetches
        ...
        switch (sub_cycle) {
            ...
            case 6: // 263
                break;
            case 7:
                nes->mapper->a12_watch(last_sprite_addr);
...
void PPU::perform_bg_fetches() { // Only called from prerender and visible scanlines
    ...
    if (((line_cycle > 0) && (line_cycle <= 256)) || (line_cycle > 320)) {
        ...
        switch (line_cycle & 7) {
            case 1: // nametable, tile #
                bg_fetches[0] = nes->PPU_read_effect(0x2000 | (io.v & 0xFFF));
            ...
            case 3: {
                ...
                bg_fetches[1] = (nes->PPU_read_effect(attrib_addr) >> shift) & 3;
                break; }
            case 5: // low buffer
...
void PPU::scanline_visible()
{
    i32 sx = line_cycle-1;
    i32 sy = nes->clock.ppu_y;
    i32 bo = (sy * 256) + sx;
    if (!rendering_enabled) {
        if (line_cycle > 0 && line_cycle <= 256) {
    ...
    if (io.bg_enable) {
        ...
        if (io.bg_hide_left_8 && sx < 8) bg_has_pixel = false;
    }
    ...
    for (i32 m = 7; m >= 0; m--) {
        if ((sprite_x_counters[m] >= -8) && (sprite_x_counters[m] <= -1) && line_cycle < 256) {
            ...
            if (io.sprite_hide_left_8 && sx < 8) my_color = 0;
            if (my_color != 0) {
...
void PPU::new_frame() {
    ...
    assert(reinterpret_cast<u64>(cur_output) > 1);
}

void PPU::set_scanline(i32 to) {
    if (to == 0) {
        render_cycle = &PPU::scanline_visible;
    }
    else if (to == nes->clock.timing.post_render_ppu_idle) {
        render_cycle = &PPU::scanline_postrender;
    }
    else if (to == nes->clock.timing.ppu_pre_render) {
        render_cycle = &PPU::scanline_prerender;
    }
}
...
void PPU::new_scanline() {
    ...
    if ((nes->clock.ppu_y >= 0) && (nes->clock.ppu_y < 240)) {
        core::NESDBGDATA::DBGNESROW &rw = nes->dbg_data.rows[nes->clock.ppu_y];
...
u32 PPU::cycle(u32 howmany) {
        for (u32 i = 0; i < howmany; i++) {
            // $2006 V-update delay: hardware applies the new V address 3 PPU cycles
            // after the second $2006 write (matching Mesen2's _updateVramAddrDelay=3).
            if (v_update_delay > 0) {
                v_update_delay--;
                if (v_update_delay == 0) {
                    io.v = v_pending;
            ...
            // Two-cycle rendering-enable pipeline (matches Mesen2 _prevRenderingEnabled).
            // Write $2001 at cycle N ... new value visible to render functions at cycle N+2.
            rendering_enabled = prev_rendering_enabled;
            prev_rendering_enabled = new_rendering_enabled;

            line_cycle++;
            display->scan_x++;
            nes->clock.ppu_frame_cycle++;
            // NTSC odd-frame skip: the pre-render scanline is 340 dots (not 341)
            // on odd frames when rendering is enabled.  Dot 340 is skipped entirely.
            bool odd_frame_skip = nes->clock.frame_odd &&
                                  (nes->clock.ppu_y == nes->clock.timing.ppu_pre_render) &&
                                  rendering_enabled &&
                                  (line_cycle == 340);
            if (line_cycle == 341 || odd_frame_skip) new_scanline();
