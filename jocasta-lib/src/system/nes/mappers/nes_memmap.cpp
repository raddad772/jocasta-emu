//
// Created by . on 10/3/24.
//

#include <cassert>
#include <cstdlib>

#include "helpers/debugger/debugger.h"

#include "helpers/simplebuf.h"
#include "nes_memmap.h"

namespace NES {

void memmap_map(memmap *mmap, u32 shift, u32 range_start, u32 range_end, simplebuf8* buf, u32 offset, u32 is_readonly, debugger_interface *iface, u32 bus_num, persistent_store *SRAM)
{
    //printf("\nMemmap! Start:%04x End:%04x shift:%d", range_start, range_end, shift);
    u32 range_size = 1 << shift;
    u32 is_SRAM = 0;
    if (buf && SRAM) {
        is_SRAM = buf->ptr == SRAM->data;
    }
    for (u32 addr = range_start; addr < range_end; addr += range_size) {
        memmap *m = mmap + (addr >> shift);
        if (iface && ((m->offset != offset) || (m->buf != buf))) {
            // Mark as dirty!
            debugger_interface_dirty_mem(iface, bus_num, range_start, range_end);
        }
        //printf("\n#%d set to%04x", addr >> shift, addr);
        m->offset = offset;
        m->buf = buf;
        m->empty = buf == nullptr;
        m->read_only = is_readonly;
        m->addr = addr;
        m->is_SRAM = is_SRAM;
        m->SRAM = SRAM;
        m->mask = range_size - 1;

        if (buf && buf->sz > 0)
            offset = (offset + range_size) % buf->sz;
    }
}

void memmap_init_empty(memmap *map, u32 addr_start, u32 addr_end, u32 shift)
{
    for (u32 i = addr_start; i < addr_end; i += (1 << shift)) {
        memmap *m = &map[i >> shift];
        m->empty = 1;
        m->addr = i;
        m->offset = 0;
        m->read_only = 1;
        m->is_SRAM = 0;
        m->buf = nullptr;
    }
}

u32 memmap::read(u32 read_addr, u32 old_val)
{
    if (empty) {
        printf("\nREAD EMPTY ADDR %04x", read_addr);
        return old_val;
    }
    assert(buf);
    assert(buf->ptr);
    //if (do_print) printf("\nADDR:%04x  mask:%04x   offset:%x  sz:%lld", addr, mask, offset, buf->sz);
    return buf->ptr[((read_addr & mask) + offset) % buf->sz];
}

void memmap::write(u32 write_addr, u32 val)
{
    if (read_only || empty) return;
    assert(buf);
    assert(buf->ptr);
    buf->ptr[((write_addr & mask) + offset) % buf->sz] = val;
    if (is_SRAM && SRAM) SRAM->dirty = true;
}

} // namespace NES
