...
{
    memmap_map(PPU_map[sprites_status], 10, range_start, range_end, buf, bank << 10, is_readonly, nullptr, 1, bus->SRAM);
}

void MMC5::map_CHR2K(u32 sprites_status, u32 range_start, u32 range_end, simplebuf8 *buf, u32 bank, u32 is_readonly)
{
    memmap_map(PPU_map[sprites_status], 10, range_start, range_end, buf, bank << 11, is_readonly, nullptr, 1, bus->SRAM);
}

void MMC5::map_CHR4K(u32 sprites_status, u32 range_start, u32 range_end, simplebuf8 *buf, u32 bank, u32 is_readonly)
{
    memmap_map(PPU_map[sprites_status], 10, range_start, range_end, buf, bank << 12, is_readonly, nullptr, 1, bus->SRAM);
}

void MMC5::map_CHR8K(u32 sprites_status, u32 range_start, u32 range_end, simplebuf8 *buf, u32 bank, u32 is_readonly)
{
    memmap_map(PPU_map[sprites_status], 10, range_start, range_end, buf, bank << 13, is_readonly, nullptr, 1, bus->SRAM);
}
...
void MMC5::remap_CHR()
{
    switch(io.CHR_page_size) {
        ...
        case 1: // 4KB
            ...
            map_CHR4K(1, 0x1000, 0x1FFF, &bus->CHR_ROM, io.CHR_bank[7],  READONLY);
            break;
        case 2: // 2KB
            ...
            map_CHR2K(1, 0x1000, 0x17FF, &bus->CHR_ROM, io.CHR_bank[10], READONLY);
...
#define S(x) Sadd(state, &(x), sizeof(x))
    S(io.ppu.sprites_8x16);
    S(io.ppu.substitutions_enabled);
    S(io.PRG_bank_mode);
    S(io.CHR_page_size);
    S(io.PRG_RAM_protect[0]);
    S(io.PRG_RAM_protect[1]);
    S(io.PRG_RAM_protect_mux);
    S(io.exram_mode);
    S(io.upper_CHR_bits);
    S(io.CHR_bank);
    S(io.PRG_bank);
...
#define L(x) Sload(state, &(x), sizeof(x))
    L(io.ppu.sprites_8x16);
    L(io.ppu.substitutions_enabled);
    L(io.PRG_bank_mode);
    L(io.CHR_page_size);
    L(io.PRG_RAM_protect[0]);
    L(io.PRG_RAM_protect[1]);
    L(io.PRG_RAM_protect_mux);
    L(io.exram_mode);
    L(io.upper_CHR_bits);
    L(io.CHR_bank);
    L(io.PRG_bank);
    ...
    L(irq.scanline);
    L(irq.pending);
    Sload(state, exram.ptr, exram.sz);
#undef L
    remap(false);
}
...
void MMC5::a12_watch(u32 addr) {
    ...
    if (irq.reads == 2) {
        ...
        if (irq.in_frame == 0) {
            ...
            irq.scanline++;
            //printf("\nNEW SCANLINE! PPUY: %d MYY:%d", bus->clock.ppu_y, irq.scanline); //bus->ppu.line_cycle)
...
MMC5::MMC5(core *bus) : MAPPER(bus)
{
    this->overrides_PPU = true;
    exram.allocate(1024);
    memmap_init_empty(PPU_map[0], 0x0000, 0x3FFF, 10);
    memmap_init_empty(PPU_map[1], 0x0000, 0x3FFF, 10);
}
