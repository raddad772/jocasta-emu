...
void SXROM::writecart(u32 addr, u32 val, u32 &do_write)
{
    ...
    if (val & 0x80) {
        ...
        last_cycle_write = bus->clock.cpu_master_clock;
        addr = 0x8000;
    } else {
        if (bus->clock.cpu_master_clock == (last_cycle_write + bus->clock.timing.cpu_divisor)) {
...
void SXROM::setcart(CART &cart)
{
    bus->ppu_mirror_mode = cart.header.mirroring;
}


SXROM::SXROM(core *bus) : MAPPER(bus)
{
