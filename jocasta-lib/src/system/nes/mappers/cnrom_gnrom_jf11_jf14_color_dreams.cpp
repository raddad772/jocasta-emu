...
#define THISM GNROM *th = static_cast<GNROM *>(bus->mapper_ptr)
...
{
    printf("\nGNROM/JF11/JF14/Color Dreams Resetting, so remapping  ..");
    bus->PPU_mirror_set();
    io.CHR_bank_num = 0;
    io.PRG_bank_num = 0;
    remap();
...
void GNROM_JF11_JF14_color_dreams::setcart(CART &cart)
{
    bus->ppu_mirror_mode = cart.header.mirroring ^ 1;
}

GNROM_JF11_JF14_color_dreams::GNROM_JF11_JF14_color_dreams(core *bus, mappers in_kind) : MAPPER(bus), kind(in_kind)
{
