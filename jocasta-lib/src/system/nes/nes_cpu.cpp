...
void controllerport::latch(u32 what) const {
    if (!device) return;
    switch(kind) {
        case NES_JOYPAD:
...
r2A03::r2A03(core* nes) : cpu(M6502::nesdecoded_opcodes), nes(nes)
{
    tracing = 0;
...
void r2A03::run_cycle() {
    if (io.dma.running) {
        ...
        io.dma.step = 0;
        nes->ppu.write_regs(0x2004, nes->CPU_read(io.dma.addr, 0, 1));
        io.dma.bytes_left--;
    ...
    cpu.cycle();
    if (cpu.pins.RW) {
        nes->CPU_write(cpu.pins.Addr, cpu.pins.D);
    }
    else {
        cpu.pins.D = open_bus = nes->CPU_read(cpu.pins.Addr, open_bus, 1);
    }
}
