...
TG16::core::core() {
    ...
    scheduler.run.func[0] = &block_step;
    scheduler.run.func[1] = &block_step;
...
void TG16::core::sample_audio(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    if (th->audio.output_ring) {
        th->audio.output_ring->push(u16_to_float2(th->cpu.psg.out.l),
                                    u16_to_float2(th->cpu.psg.out.r));
    }
}
...
void TG16::core::setup_crt(JSM_DISPLAY &d)
{
    ...
    // NTSC crystal 21.477272 MHz, 1364 master cycles/line × 262 lines ... ≈60.0988 Hz
    ...
    if (::dbg.do_debug && dbg.waveforms_psg.main.vec != nullptr) {
        ...
        if (wf->samples_requested > 0) {
            audio.master_cycles_per_max_sample =
                static_cast<double>(vce.regs.cycles_per_frame) / static_cast<double>(wf->samples_requested);
        ...
        if (chan_samples > 0) {
            audio.master_cycles_per_min_sample =
                static_cast<double>(vce.regs.cycles_per_frame) / static_cast<double>(chan_samples);
