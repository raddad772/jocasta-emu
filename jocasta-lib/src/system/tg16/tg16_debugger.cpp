...
#define YN(x) (x) ? "yes" : "no"
    ...
    tbp("RCR:%02x/%d(%d)\n", th->vdc0.io.RCR.u, th->vdc0.io.RCR.u, th->vdc0.io.RCR.u-64);

    if (th->is_cd) {
        auto &cd = th->cdrom;
        tbp("\nCD-ROM\n~~~~~~~\n");
        u8 act = cd.irqc.active_irqs, en = cd.irqc.enabled_irqs;
        tbp("IRQ active=%02X enabled=%02X  line=%c\n", act, en,
            (act & en) ? 'Y' : 'N');
        tbp("  DataIn:%c/%c  StatMsg:%c/%c  SubCode:%c/%c  End:%c/%c  Half:%c/%c   (active/enabled)\n",
            act&0x40?'Y':'N', en&0x40?'Y':'N',
            act&0x20?'Y':'N', en&0x20?'Y':'N',
            act&0x10?'Y':'N', en&0x10?'Y':'N',
            act&0x08?'Y':'N', en&0x08?'Y':'N',
            act&0x04?'Y':'N', en&0x04?'Y':'N');
        tbp("SCSI phase=%d  BSY=%c REQ=%c MSG=%c CD=%c IO=%c\n",
            static_cast<int>(cd.scsi.phase),
            cd.scsi.BSY?'Y':'N', cd.scsi.REQ?'Y':'N', cd.scsi.MSG?'Y':'N',
            cd.scsi.CD?'Y':'N', cd.scsi.IO?'Y':'N');
        tbp("ADPCM playing=%c half=%c end=%c length=%05X\n",
            cd.adpcm.playing?'Y':'N', cd.adpcm.half_reached?'Y':'N',
            cd.adpcm.end_reached?'Y':'N', cd.adpcm.length);
    }
}
...
static void setup_waveforms_cd(TG16::core *th, debugger_interface *dbgr)
{
    if (!th->is_cd) return;

    th->dbg.waveforms_cd.view = dbgr->make_view(dview_waveforms);
    auto *dview = &th->dbg.waveforms_cd.view.get();
    auto *wv = &dview->waveform;
    snprintf(wv->name, sizeof(wv->name), "CD Audio");

    wv->waveforms.reserve(4);

    debug_waveform *dw = &wv->waveforms.emplace_back();
    th->dbg.waveforms_cd.main.make(wv->waveforms, wv->waveforms.size() - 1);
    snprintf(dw->name, sizeof(dw->name), "CD-DA");
    dw->kind = dwk_main;
    dw->samples_requested = 400;
    dw->is_unsigned = false;

    dw = &wv->waveforms.emplace_back();
    th->dbg.waveforms_cd.chan[0].make(wv->waveforms, wv->waveforms.size() - 1);
    snprintf(dw->name, sizeof(dw->name), "ADPCM");
    dw->kind = dwk_channel;
    dw->samples_requested = 200;
    dw->is_unsigned = false;
}
