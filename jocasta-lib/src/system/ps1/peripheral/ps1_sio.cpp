...
{
    //printf("\nTHING:%d  TRACES:%d", ::dbg.traces.ps1.sio0.rw, ::dbg.do_debug);
...
void SIO0::write(void *ptr, u32 addr, u8 sz, u32 val)
{
    auto *th = static_cast<SIO0 *>(ptr);
...
#define R_SIO_BAUD 0x1F80104E
    ...
    //val &= masksz[sz];
    switch(addr) {
        case R_SIO_CTRL: th->write_ctrl(sz, val); return;
        case R_SIO_MODE: th->write_mode(sz, val); return;
        case R_SIO_STAT: th->write_stat(sz, val); return;
        case R_TX_DATA: th->write_tx_data(sz, val); return;
        case R_SIO_MISC: th->io.misc = val; return;
        case R_SIO_BAUD:
            th->io.baud = val & 0xFFFF;
...
u32 SIO0::read(void *ptr, u32 addr, u8 sz)
{
    auto *th = static_cast<SIO0 *>(ptr);
    //printf("\nRD SIO0 ADDR:%04x SZ:%d", addr, sz);
    switch(addr) {
        case R_SIO_CTRL:
            return th->read_ctrl(sz) & masksz[sz];
        case R_SIO_MODE:
            return th->read_mode(sz) & masksz[sz];
        case R_SIO_STAT:
            return th->read_stat(sz) & masksz[sz];
        case R_RX_DATA:
            return th->read_rx_data(sz) & masksz[sz];
        case R_SIO_MISC:
            return th->io.misc;
        case R_SIO_BAUD:
            return th->io.baud;
    }
    printf("\nUnhandled SIO0 read from %08x (%d)", addr, sz);
    return 0;
}
