...
u32 core::mainbus_read(u32 addr, u8 sz)
{
    ...
    auto &t = mem.pages[do_debug][sz >> 1][pg];
...
void core::mainbus_write(u32 addr, u8 sz, u32 val)
{
    clock.waitstates += DEFAULT_WAITSTATES;
    addr &= 0x1FFF'FFFF;
    if (mem.cache_isolated) {
        cpu.icache_isolated_write(addr);
        return;
    }
    auto &t = mem.pages[do_debug][sz >> 1][getbits<21, 28>(addr)];
    t.write(t.ptr, addr, val);
}

template u32 core::mainbus_read<false>(u32, u8);
template u32 core::mainbus_read<true>(u32, u8);
template void core::mainbus_write<false>(u32, u8, u32);
template void core::mainbus_write<true>(u32, u8, u32);
