...
static void render_image_view_spuinfo(debugger_interface *dbgr, debugger_view *dview, void *ptr, u32 out_width) {
    ...
    tb->clear();
    static constexpr char EPNAMES[5][50] = {"Off    ", "Attack ", "Decay  ", "Sustain", "Release"};
    for (u32 i = 0; i < 24; i++) {
...
static void render_image_view_sysinfo(debugger_interface *dbgr, debugger_view *dview, void *ptr, u32 out_width) {
    ...
    tb->sprintf("\nDICR: %08x", th->dma.read(0x1F8010F4, 4));
...
static void draw_box(u32 *outbuf, i32 left, i32 top, i32 right, i32 bottom, i32 out_width, i32 out_height, u32 color) {
    i32 pleft = left < 0 ? 0 : left;
    i32 pright = right >= out_width-2 ? out_width-2 : right;
    if (pright > 1023) pright = 1023;
    i32 ptop = top < 0 ? 0 : top;
    i32 pbottom = bottom >= out_height-2 ? out_height-2 : bottom;

    u32 *l_top = outbuf + (ptop * out_width);
    u32 *l_bot = outbuf + (pbottom * out_width);
    for (i32 x = pleft; x <= pright; x++) {
        l_top[x] = color;
        l_bot[x] = color;
    }
    for (i32 y = ptop; y <= pbottom; y++) {
        l_top[pleft] = color;
        l_top[pright] = color;
        l_top += out_width;
    }
...
static void render_image_view_vram(debugger_interface *dbgr, debugger_view *dview, void *ptr, u32 out_width) {
    ...
    u32 *img32 = outbuf;
    i32 mouse_x = iv->mouse_x - 2;
    i32 mouse_y = iv->mouse_y + 12;
    ...
    debugger_widget_textbox *tb = &dview->options[0].textbox;
    tb->clear();
    tb->sprintf("COORD %d,%d", mouse_x, mouse_y);
    if ((mouse_x >= 0) && (mouse_x <= 1023) && (mouse_y >= 0) && (mouse_y <= 511)) {
        u32 vram_sample = cR16(th->gpu.VRAM, (mouse_y * 2048) + mouse_x);
        // Draw mouse-dot
        u32 *outptr = img32 + (mouse_y * out_width) + mouse_x;
        *outptr ^= 0xFFFFFF;

        // Draw text for CMD
        u32 dbgaddr = (mouse_y * 1024) + iv->mouse_x;
        u32 cmd = th->gpu.dbg.buf.cmd[dbgaddr];
        u32 first_color = th->gpu.dbg.buf.first_color[dbgaddr];
        tb->sprintf("  SMP:%04x  CMD %02x  2SCREEN:%06x  ", vram_sample, cmd, ps1_to_screen(vram_sample) & 0xFFFFFF);
        u32 top3 = cmd >> 5;
        switch (top3) {
            case 0b001: {// poly
                u32 num_verts = (cmd >> 3) & 1 ? 4 : 3;
                tb->sprintf("polygon(%d):", num_verts);
                bool goraud = (cmd >> 4) & 1;
                ...
                tb->sprintf(" first_color:%08x", first_color);
                break; }
            case 0b010: // line
                tb->sprintf("line");
                break;
            case 0b011: {// rectangle
                ...
                u32 ss = (cmd >> 3) & 3;
                tb->sprintf(" %s %s %s %s",
                    ss == 0 ? "variable" : (ss == 1 ? "1x1     " : (ss == 2 ? "8x8     " : "16x16   ")),
                    textured ? "textured" : "solid   ",
                    semit ? "transp." : "opaque ",
                    raw ? "raw     " : "modulate"
                );
                if ((!raw) || (!textured)) tb->sprintf(" first_color:%08x", first_color);
                break;}
            case 0b100: // VRAM-to-VRAM
                tb->sprintf("VRAM-to-VRAM");
                break;
            case 0b110: // VRAM-to-CPU
                tb->sprintf("VRAM-to-CPU");
                break;
            case 0b101: // CPU-to-VRAM
                tb->sprintf("CPU-to-VRAM");
                break;
            default:
                switch (cmd) {
                    case 2: // QuickRect
                        tb->sprintf("quickrect");
                        break;
                default:
                        tb->sprintf("unknown");
                        break;
        ...
        // If there is CLUT or texpage info...
        u32 texel = th->gpu.dbg.buf.texel[dbgaddr];
        if (texel & 0x80000000) { // We had a texel here...
            u32 xy_depth = th->gpu.dbg.buf.xy_depth[dbgaddr];
            u8 depth = xy_depth >> 19;
            u16 tex_y = (xy_depth >> 10) & 0x1FF;
            u16 tex_x = (xy_depth >> 0) & 0x3FF;
            u32 cdata = th->gpu.dbg.buf.clut[dbgaddr];
            u16 texpage = cdata >> 16;
            u16 clut = cdata & 0xFFFF;
            u16 uv = th->gpu.dbg.buf.uv[dbgaddr];
            u16 tex_u = uv & 0xFF;
            u16 tex_v = uv >> 8;
            tb->sprintf("\nTexel:%04x  depth:%d  texpage:%04x  clut:%04x", texel & 0xFFFF, depth, texpage, clut);
            tb->sprintf("\n U:%d V:%d   vram_x:%d  vram_y:%d", tex_u, tex_v, tex_x, tex_y);
            // If it's a color lookup...
            if (depth < 15) {
                // Shade CLUT
                u32 clutlen = depth == 8 ? 256 : 16;
                u32 clut_x = (clut & 0x3F) << 4;
                u32 clut_y = (clut >> 6) & 0x1FF;
                u32 clut_entry = texel & 0xFF;
                if (depth == 4) clut_entry &= 0x0F;
                u32 clut_sx = (clut_x + clut_entry) & 1023;
                u32 addr = (clut_y * 2048) + (clut_entry * 2);
                addr += (clut_sx * 2);
                u16 clutsample = cR16(th->gpu.VRAM, addr);

                tb->sprintf("\n CLUT@%d,%d(%d long, x=%d)  smp:%04x  smp_from_render:%04x", clut_x,clut_y, clutlen, clut_sx, clutsample, th->gpu.dbg.buf.clut_sample[dbgaddr]);
                // Draw chosen CLUT=shaded
                draw_box(outbuf, clut_x-1, clut_y-1, clut_x+clutlen+1, clut_y+1, out_width, 511, 0xFF0000FF);
                // Draw chosen CLUT sample boxed
                draw_box(outbuf, clut_sx-1, clut_y-1, clut_sx+1, clut_y+1, out_width, 511, 0xFF00FFFF);
            }
            else {
                tb->sprintf("\n No CLUT, 15-bit!");
            }
            // Box around texpage
            i32 page_x = (texpage & 0x0F) << 6; // * 64
            i32 page_y = ((texpage >> 4) & 1) << 8;
            u32 COLOR = 0xFFFFFFFF;
            draw_box(outbuf, page_x-1, page_y-1, page_x+256, page_y+256, out_width, 512, COLOR);
            // Draw box around chosen texel
            draw_box(outbuf, tex_x-1, tex_y-1, tex_x+1, tex_y+1, out_width, 512, COLOR);
            // Draw box around chosen CLUT value
        }
        else {
            tb->sprintf("\nNo texel\n\n\n");
        }
...
static void setup_dbglog(debugger_interface *dbgr, core *th)
{
    ...
    general.add_node(dv, "Current Issue", "Issue", PS1D_GENERAL_ISSUE, dma_c);
    ...
    dma_c = 0xFF2080;
    dbglog_category_node &mdec = root.add_node(dv, "MDEC", nullptr, 0, dma_c);
    mdec.children.reserve(10);
    mdec.add_node(dv, "MDEC Block Start", "MDEC B Start", PS1D_MDEC_BLOCK_START, dma_c);
    mdec.add_node(dv, "MDEC Block Finish", "MDEC B Finish", PS1D_MDEC_BLOCK_FINISH, dma_c);
}
...
static void setup_image_view_vram(core* th, debugger_interface *dbgr) {
    ...
    debugger_widgets_add_textbox(dview->options, "blah!", 0);
    auto *w = &dview->options.back();
    w->post = true;
}
...
static void readcpumembus(void *ptr, u32 addr, void *dest)
{
    ...
    u32 offset = 0;
    for (u32 i = 0; i < 4; i++) {
        u32 a = th->mainbus_read((addr + offset) & 0x1FFFFFFF, 4);
        cW32(dest, offset, a);
        offset += 4;
    }
}
...
static void readvram(void *ptr, u32 addr, void *dest)
{
    ...
    u32 offset = 0;
    for (u32 i = 0; i < 4; i++) {
        u32 a = cR32(th->gpu.VRAM, (addr + offset) & 0xFFFFF);
        cW32(dest, offset, a);
        offset += 4;
    }
}

static void readcdrom(void *ptr, u32 addr, void *dest)
{
    // Read 16 bytes from addr into dest
    u8 *out = static_cast<u8 *>(dest);
    auto *th = static_cast<core *>(ptr);
    u32 offset = 0;
    for (u32 i = 0; i < 4; i++) {
        u32 a = cR32(th->cdrom.data.data.ptr, (addr + offset) % th->cdrom.data.data.sz);
        cW32(dest, offset, a);
        offset += 4;
    }
}
...
static void setup_memory_view(core *th, debugger_interface *dbgr) {
    ...
    mv->modules.reserve(5);
    mv->add_module("CPU Bus", 0, 8, 0, 0x1EFFFFFF, th, &readcpumembus);
    mv->add_module("CDROM Image", 1, 8, 0, 0x2FFFFFFF, th, &readcdrom);
    mv->add_module("VRAM", 2, 5, 0, 0xFFFFF, th, &readvram);
