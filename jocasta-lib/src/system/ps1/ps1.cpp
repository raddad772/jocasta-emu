...
static constexpr double SPU_SAMPLE_RATE = 44100.0;
...
static void vblank(void *bound_ptr, u64 key, u64 current_clock, u32 jitter)
{
    ...
    if (false) {
        printf("\n7A520: ");
        for (u32 i = 0; i < 16; i++) {
            u32 addr = 0x7A520 + (i * 2);
            u32 data = cR16(th->gpu.VRAM, addr);
            printf("%04x ", data);
        }
    }
}
...
static void sample_audio_debug_med(void *ptr, u64 key, u64 clock, u32 jitter) {
    auto *th = static_cast<core *>(ptr);
    // Stop if all medium waveform buffers are full (check first one)
    auto *dw0 = th->dbg.waveforms2.cd.chan_cache[0];
    if (!dw0 || dw0->user.buf_pos >= (dw0->samples_requested << 1)) return; // buffer full — stop
...
void core::schedule_frame(u64 start_clock, u32 is_first)
{
    if (!is_first) {
        ...
        if (audio.master_cycles_per_audio_sample == 0)
            audio.master_cycles_per_audio_sample = static_cast<double>(clock.timing.cpu.hz) / SPU_SAMPLE_RATE;
        audio.cycles_per_frame = static_cast<double>(clock.timing.frame.cycles);
        audio.next_sample_cycle = static_cast<double>(start_clock) + audio.master_cycles_per_audio_sample;
    ...
    for (u32 line = 0; line < clock.timing.frame.lines; line++) {
        scheduler.only_add_abs_w_tag(cur_clock, 0, this, &hblank, nullptr, 1);
    ...
    scheduler.only_add_abs_w_tag(start_clock+clock.timing.frame.cycles, 0, this, &do_next_scheduled_frame, nullptr, 2);
}

void core::set_audio_ring(audio_output_ring *ring)
{
    audio.output_ring = ring;
    // PS1 CPU clock: 33,868,800 Hz NTSC. SPU output: 44,100 Hz.
    // One audio sample every 768 CPU cycles (44100 * 768 ≈ 33,868,800).
    audio.cycles_per_frame = static_cast<double>(clock.timing.frame.cycles);
    audio.master_cycles_per_audio_sample = static_cast<double>(clock.timing.cpu.hz) / SPU_SAMPLE_RATE;
}
...
void core::present_screen() {
    ...
    u32 hwidth = gpu.display_area.width > 640 ? 640 : gpu.display_area.width;
    u32 vheight = gpu.display_area.height > PS1_MAX_LINES ? PS1_MAX_LINES : gpu.display_area.height;

    gpu.display->cur_output_width = hwidth;
    gpu.display->cur_output_height = vheight;

    for (u32 sy = 0; sy < vheight; sy++) {
        // Draw line!
        // Pad 64 on left
        //u64 lineptr_start = lineptr - gpu.cur_output;
        lineptr = gpu.cur_output + (sy * 640);
    ...
    gpu.display->active_draw_buffer ^= 1;
    gpu.cur_output = static_cast<u32 *>(gpu.display->output[gpu.display->active_draw_buffer]);
}
...
u32 core::finish_frame()
{
    ...
    if (::dbg.do_debug && dbg.waveforms2.main_cache != nullptr) {
        const u64 now = clock.master_cycle_count;

        // nosolo = true when no cd/reverb channel is in solo ... all channels play
        bool any_solo = false;
        if (dbg.waveforms2.cd.chan_cache[0] && dbg.waveforms2.cd.chan_cache[0]->ch_output_solo) any_solo = true;
        for (u32 i = 0; i < 4; i++) {
            if (dbg.waveforms2.reverb.chan_cache[i] && dbg.waveforms2.reverb.chan_cache[i]->ch_output_solo) any_solo = true;
        }
        audio.nosolo = !any_solo;

        // Reset all waveform buffers for this frame
        dbg.waveforms2.main_cache->setup(audio.cycles_per_frame);
        for (u32 i = 0; i < 24; i++) {
            if (dbg.waveforms2.channels.chan_cache[i])
                dbg.waveforms2.channels.chan_cache[i]->setup(audio.cycles_per_frame);
        }
        if (dbg.waveforms2.cd.chan_cache[0])
            dbg.waveforms2.cd.chan_cache[0]->setup(audio.cycles_per_frame);
        for (u32 i = 0; i < 4; i++) {
            if (dbg.waveforms2.reverb.chan_cache[i])
                dbg.waveforms2.reverb.chan_cache[i]->setup(audio.cycles_per_frame);
        }

        // Arm main (stereo) waveform sampler
        if (dbg.waveforms2.main_cache->samples_requested > 0) {
            audio.master_cycles_per_max_sample = audio.cycles_per_frame / static_cast<double>(dbg.waveforms2.main_cache->samples_requested);
        ...
        // Arm medium (cd + reverb) stereo waveform sampler
        auto *cd0 = dbg.waveforms2.cd.chan_cache[0];
        if (cd0 && cd0->samples_requested > 0) {
            audio.master_cycles_per_med_sample = audio.cycles_per_frame / static_cast<double>(cd0->samples_requested);
            audio.next_sample_cycle_med = static_cast<double>(now) + audio.master_cycles_per_med_sample;
            scheduler.only_add_abs(static_cast<i64>(audio.next_sample_cycle_med), 0, this, &sample_audio_debug_med, nullptr);
        }

        // Arm per-voice (min) waveform sampler
        auto *ch0 = dbg.waveforms2.channels.chan_cache[0];
        if (ch0 && ch0->samples_requested > 0) {
            audio.master_cycles_per_min_sample = audio.cycles_per_frame / static_cast<double>(ch0->samples_requested);
...
void core::reset()
{
    ...
    //printf("\nPS1 reset!");
    memset(mem.MRAM, 0, sizeof(mem.MRAM));
...
void core::sample_audio()
{
    ...
    if (audio.output_ring) {
        audio.output_ring->push(i16_to_float(spu.sample_l), i16_to_float(spu.sample_r));
    }
...
static void setup_crt(JSM_DISPLAY *d)
{
    ...
    d->pixelometry.rows.visible = PS1_MAX_LINES;
    d->pixelometry.rows.max_visible = PS1_MAX_LINES;;
...
void core::describe_io()
{
    ...
    d->display.allocate_output(0, 640*PS1_MAX_LINES*4); // Each frame we must blit to this
    d->display.allocate_output(1, 640*PS1_MAX_LINES*4);
    d->display.output_debug_metadata[0] = nullptr;
    d->display.output_debug_metadata[1] = nullptr;
    setup_crt(&d->display);
    d->display.fps = clock.timing.fps;  // 59.826 NTSC, 49.761 PAL
