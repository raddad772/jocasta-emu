...
// TODO
// 1. seek and read can conflict scheduling
// 2. better CD loading/parsing
// 3.
#include <cstring>
...
u32 core::mainbus_read(u32 addr, u8 sz) {
    ...
    switch (addr) {
        ...
        case 0x1F801801: return read_01(sz);
        case 0x1F801802: return read_02(sz);
        case 0x1F801803: return read_03(sz);
        default: break;
    }
    NOGOHERE;
}
...
void core::cmd_getlocl() {
    result(3, {
        seek.getlocl_header[0],
        seek.getlocl_header[1],
        seek.getlocl_header[2],
        seek.getlocl_header[3],
        seek.getlocl_header[4],
        seek.getlocl_header[5],
        seek.getlocl_header[6],
        seek.getlocl_header[7],
    });
    finish_CMD(false, 0);
}
...
void core::latch_seek() {
    head.LBA = seek.target_LBA;
    u8 mm, ss, ff;
    CD::lba_to_msf(seek.target_LBA, mm, ss, ff);
    //printf("\n(LATCH SEEK) %02d:%02d:%02d", mm, ss, ff);
}
...
void core::cmd_setloc() {
    ...
    stat_irq();
    seek.target_LBA = CD::msf_to_lba(mm, ss, ff);
...
void core::cmd_play() {
    ...
    if (track != 0) {
        if (track >= data.num_tracks + 1) {
            ...
            track = data.get_track_from_LBA(head.LBA) + 1;
        }
        // Seek to track
        if (track > 0) {
            head.LBA = data.tracks[track-1].idx1_lba;
        }
    }
    else if (seek.needs_seek) {
        latch_seek();
    }
    seek.needs_seek = false;
    head.sample_index = 0;
...
void core::do_cmd_read(u64 clock) {
    if (read.still_sched && !seek.needs_seek) {
        printf("\nWARN ReadN/S ABORT for already going!");
        //dbg_break("HEYARG", 0);
        finish_CMD(true, 3);
        return;
    }
...
void core::cmd_getlocp() {
    // Within track
    // Mine GetLocP t02 i01 m00 s00 f00 m27 s46 f65
    // Ares GetLocP t02 i01 m00 s00 f00 m27 s46 f65
    // subtract 215 LBA...
    u32 global_mm, global_ss, global_ff;
    u32 track_num, idx, track_mm, track_ss, track_ff;

    //printf("\nGetLocP result: %02d %02d %02d %02d", track_num+1, idx, seek.last_read.track.amm, seek.last_read.track.ass);
    data.global_CD_time(head.LBA, global_mm, global_ss, global_ff);
    data.track_CD_time(head.LBA, track_num, idx, track_mm, track_ss, track_ff);;
    result(3, {
        encode_bcd(track_num+1), // track #
        encode_bcd(idx), // index
        encode_bcd(track_mm), // track amm/ass/asect
        encode_bcd(track_ss),
        encode_bcd(track_ff),
        encode_bcd(global_mm), // global amm/ass/asect
        encode_bcd(global_ss),
        encode_bcd(global_ff),
    });
    finish_CMD(false, 0);
}
...
void core::read_sector() {
    ...
    u8 *ptr = data.ptr_to_data_abs(head.LBA);
    ...
    getlocl_header();
    if (mode == 2) {
        if (io.MODE.xa_filter && ((subheader & 0x44) == 0x44)) {
    ...
    u8 mm, ss, ff;
    CD::lba_to_msf(head.LBA, mm, ss, ff);
    dbgloglog_bus(PS1D_CDROM_SECTOR_READS, DBGLS_INFO, "(READ) Read sector %02d:%02d:%02d  raw_sector:%d  LBA:%d  mode:%d  form:%d  subheader:%02x", mm, ss, ff, io.MODE.sector_size, head.LBA, mode, form, subheader);
...
void core::getlocl_header() {
    u8 *p = data.ptr_to_data_abs(head.LBA);
    memcpy(seek.getlocl_header, p + 12, 8);
}
...
void core::next_sector() {
    head.LBA++;
    if (head.LBA >= 333000) head.LBA = 150;
}
...
void core::generate_report() {
    if (!io.MODE.report) return;
    u32 global_mm, global_ss, global_ff;
    u32 track_num, idx, track_mm, track_ss, track_ff;
    ...
    if ((global_ff == 0) || (global_ff == 0x20) || (global_ff == 0x40) || (global_ff == 0x60)) {
        //printf("\nRESULT GLOBAL stat:%02x t:%d idx:%d %02d:%02d:%02d", io.stat.u, track_num, idx, global_mm, global_ss, global_ff);
        result(1, {
            io.stat.u,
            encode_bcd(track_num),
            encode_bcd(idx),
            encode_bcd(global_mm),
            encode_bcd(global_ss),
            encode_bcd(global_ff),
            0,
            0
    ...
    else if ((track_ff == 0x10) || (track_ff == 0x30) || (track_ff == 0x50) || (track_ff == 0x70)) {
        //printf("\nRESULT TRACK stat:%02x t:%d idx:%d %02d:%02x:%02d", io.stat.u, track_num, idx, track_mm, encode_bcd(track_ss) | 0x80, track_ff);
        result(1, {
            io.stat.u,
            encode_bcd(track_num),
            encode_bcd(idx),
            encode_bcd(global_mm),
            static_cast<u8>(encode_bcd(global_ss) | 0x80),
            encode_bcd(global_ff),
            0,
            0
        });
...
void core::get_CD_audio_cdda(i16 &left, i16 &right) {
    ...
    auto *smp = reinterpret_cast<i16 *>(data.ptr_to_data_abs(head.LBA));
    smp += head.sample_index * 2;
    head.sample_index++;
    if (smp == nullptr) printf("\nCRAP!");
...
void core::cmd_gettn() {
    result(3, {io.stat.u, encode_bcd(1), encode_bcd(static_cast<u8>(data.num_tracks))} );
...
void core::cmd_gettd() {
    ...
    if (track_num == 0) {
        ...
        calc = data.tracks[track_num-1].idx1_lba;
    }
    ...
    //printf("\n(CDROM) CMD GetTD %d %02d:%02d", track_num, mm, ss);
    finish_CMD(false, 0);
}
...
void core::cmd_set_session_finish(u64 clock) {
    ...
    if (disk.num_sessions == 1){
        ...
        printf("\nWARN SETSESSION ON MULTISESSION DISC!");
    }
}
...
i64 core::seek_cycles() {
    ...
    if (head.LBA == seek.target_LBA) {
        ///printf("\nQUICK SEEK!");
        //return 5000;
    }
    i64 r = (ONEFRAME*20)+5000;
    // ONEFRAME*20
...
void core::dbg_irq(u32 num) {
    ...
    *ptr = 0;
    ...
    for (u32 i = 0; i < io.response.len; i++) {
        ...
        ptr += snprintf(ptr, sizeof(iostr) - (ptr - iostr), " %02x", io.response.entries[p]);
    }
