...
static constexpr u64 MDEC_TICKS_PER_BLOCK = 448;
static constexpr u64 MDEC_DECODE_TICKS = MDEC_TICKS_PER_BLOCK * 6;

static void mdec_copy_out(void *ptr, u64 /*key*/, u64 /*clock*/, u32 /*jitter*/) {
    static_cast<MDEC *>(ptr)->copy_out();
}

bool MDEC::decode_macroblock() {
    if (io.stat.output_depth <= 1) {
        if (!decode_block(BLOCK.y0, BLOCK.luma)) return false;
        convert_y(output, BLOCK.y0);
    }
    else {
        if (!decode_block(BLOCK.cr, BLOCK.chroma)) return false;
        if (!decode_block(BLOCK.cb, BLOCK.chroma)) return false;
        if (!decode_block(BLOCK.y0, BLOCK.luma)) return false;
        if (!decode_block(BLOCK.y1, BLOCK.luma)) return false;
        if (!decode_block(BLOCK.y2, BLOCK.luma)) return false;
        if (!decode_block(BLOCK.y3, BLOCK.luma)) return false;
...
void MDEC::do_decode() {
    if (fifo_in.len == 0 || !decode_macroblock()) {
        io.mode = MM_Idle;
        return;
    }
    decode_sched_id = bus->scheduler.only_add_abs(
        bus->scheduler.current_time() + MDEC_DECODE_TICKS, 0, this, &mdec_copy_out, &decode_sched);
}
    ...
    switch (io.mode) {
        case MM_Idle:
            ...
            case 1:
                    ...
                    dbgloglog_bus(PS1D_MDEC_BLOCK_START, DBGLS_TRACE, "MDEC block start words: %d", io.num_remaining_param_words);
        ...
        case MM_DecodeMacroblock:
            ...
            if (io.num_remaining_param_words == 0) {
                dbgloglog_busn(PS1D_MDEC_BLOCK_FINISH, DBGLS_TRACE, "MDEC block finished");
                io.mode = MM_Decoding;
                do_decode();
            }
            break;
...
void MDEC::write_ctrl(u32 val) {
    ...
    if (val & 0x80000000) { // reset
        if (decode_sched) bus->scheduler.delete_if_exist(decode_sched_id);
        decode_sched = 0;
        io.mode = MM_Idle;
        io.offset = 0;
        num_param_words = 0;
