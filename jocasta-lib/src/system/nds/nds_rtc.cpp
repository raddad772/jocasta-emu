...
void core::RTC_set_irq(u32 which)
{
    ...
    if ((!(oldstat & 0x30) && (io.rtc.irq_flag & 0x30))) {
        if ((io.sio.rcnt & 0xC100) == 0x8100) {
            update_IF7<do_debug>(IRQ_SERIAL);
        }
    }
}
...
void core::RTC_reset()
{
    ...
    io.rtc.divider = 0;
    if (::dbg.do_debug) scheduler.add_or_run_abs<true>(32768, 0, this, &RTC_tick<false>, &RTC_tick<true>, nullptr);
    else scheduler.add_or_run_abs<false>(32768, 0, this, &RTC_tick<false>, &RTC_tick<true>, nullptr);
}
...
void core::RTC_byte_in()
{
    ...
    RTC_cmd_write<do_debug>(val);
}

template void core::write_RTC<true>(u8 sz, u32 val);
template void core::write_RTC<false>(u8 sz, u32 val);
template<bool do_debug>
...
void core::RTC_tick(void *ptr, u64 key, u64 clock, u32 jitter) // Called on scanline start
{
    ...
    bus->io.rtc.sch_id = bus->scheduler.add_or_run_abs<do_debug>(tstamp, 0, bus, &RTC_tick<false>, &RTC_tick<true>, nullptr);
    bus->io.rtc.divider++;
    if ((bus->io.rtc.divider & 0x7FFF) == 0) {
        ...
        if (bus->io.rtc.date_time[6] >= 0x60) { // 60 seconds...
            ...
            bus->RTC_process_irqs<do_debug>(0);
        }
    }
    ...
    bus->RTC_process_irqs<do_debug>(1);
}
}