...
struct timer7_t {
    ...
    template<bool do_debug> static void overflow_callback(void *ptr, u64 timer_num, u64 current_clock, u32 jitter);
    template<bool do_debug>void cascade_step(u64 current_time);
    template<bool do_debug>void overflow(u64 current_time);
    u32 read();
    template<bool do_debug> void write_cnt(u32 val);
