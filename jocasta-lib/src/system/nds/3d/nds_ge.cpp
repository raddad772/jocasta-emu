...
void FIFO::update_len(bool from_dma, bool did_add)
{
    ...
    if (ge->GXSTAT.cmd_fifo_irq_mode) {
        ...
        if ((old_bit != new_bit) && new_bit)
            ge->bus->update_IF9<do_debug>(IRQ_GXFIFO);
    }

    if (!did_add && !from_dma && (len < 128))
        ge->bus->trigger_dma9_if<do_debug>(DMA_GE_FIFO);
}
...
static u32 determine_winding_order(POLY *poly, BUFFERS *b)
{
    ...
    for (i32 i = 1; i < poly->vertex_list.len; i++) {
        n_comp = n_comp->next;
...
void GE::handle_cmd()
{
    ...
    fifo.update_len<do_debug>(false, false);
    assert(fifo.cmd_scheduled == 0);
    printfifo("\nSCHEDULING GE CMD %02x FOR %d CYCLES!", cmd, num_cycles);
    GXSTAT.test_busy = tbl_test_busy[cmd];
    scheduler->only_add_abs(bus->clock.current7() + num_cycles, cmd, this, &do_cmd<false>, &do_cmd<true>, &fifo.cmd_scheduled);
}
...
void GE::fifo_write_direct(u32 val, bool from_dma, bool from_addr)
{
    ...
    fifo.update_len<do_debug>(from_dma, true);
    if (!from_addr && !GXSTAT.ge_busy) handle_cmd<do_debug>();
}
...
void GE::fifo_write_from_addr(CMDS cmd, u32 val)
{
    ...
    if (fifo.waiting_for_cmd) fifo_write_direct<do_debug>(cmd, false, true);
    fifo_write_direct<do_debug>(val, false, true);
    if (!GXSTAT.ge_busy) handle_cmd<do_debug>();
}

template void GE::write<true>(u32 addr, u8 sz, u32 val);
template void GE::write<false>(u32 addr, u8 sz, u32 val);
template<bool do_debug>
void GE::write(u32 addr, u8 sz, u32 val)
{
    if ((addr >= R9_GXFIFO) && (addr < (R9_GXFIFO + 0x40))) {
        //printf("\nAddr %08x val %08x", addr, val);
        fifo_write_direct<do_debug>(val, 0, 0);
        return;
    }
    switch(addr) {
        ...
        case R9_GXSTAT:
            ...
            if (check_irq())
                bus->update_IF9<do_debug>(IRQ_GXFIFO);
            return;


#define gcmd(label) case R9_G_CMD_##label: fifo_write_from_addr<do_debug>(CMD_##label, val); return
        gcmd(MTX_MODE);
        gcmd(MTX_PUSH);
        gcmd(MTX_POP);
        gcmd(MTX_STORE);
...
void GE::vblank_up()
{
    ...
    if (io.swap_buffers) {
        scheduler->only_add_abs(bus->clock.current7() + 392, 0, this, &do_swap_buffers<false>, &do_swap_buffers<true>, nullptr);
    }
    else {
        //re->render_frame();
    }
