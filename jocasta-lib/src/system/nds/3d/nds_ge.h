...
struct RE {
    ...
    NDS::core *bus;
    template<bool do_debug> void render_frame();
    template<bool do_debug> void render_to(BUFFERS *b, LINEBUFFER *lb, POLY_LIST *r);
    void copy_and_sort_list(BUFFERS *b, POLY_LIST *r);
    GE *ge{};
    void reset();
    template<bool do_debug> void clear_line(LINEBUFFER *l);
    template<bool do_debug> void render_line(BUFFERS *b, i32 line_num, LINEBUFFER *line, POLY_LIST *plist);
...
struct FIFO {
    ...
    template<bool do_debug> void update_len(bool from_dma, bool did_add);
};
...
struct GE {
    ...
    template<bool do_debug> void handle_cmd();
    template<bool do_debug> void fifo_write_direct(u32 val, bool from_dma, bool from_addr);
    template<bool do_debug> void fifo_write_from_addr(CMDS cmd, u32 val);
    template<bool do_debug> void write(u32 addr, u8 sz, u32 val);
