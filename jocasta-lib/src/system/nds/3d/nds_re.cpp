...
void find_closest_points_marked(BUFFERS *b, POLY *p, i32 comp_y, EDGE *edges)
{
    ...
    for (i32 i = 0; i < p->vertex_list.len; i++) {
        for (i32 j = 0; j < 2; j++) {
...
static void final_tex_coord(TEX_SAMPLER *ts, i32 *s, i32 *t)
{
    ...
    {
        if (ms < 0) ms = 0;
        else if (ms >= static_cast<i32>(ts->s_size)) ms = ts->s_mask;
    }
    ...
    {
        if (mt < 0) mt = 0;
        else if (mt >= static_cast<i32>(ts->t_size)) mt = ts->t_mask;
    }
...
void RE::render_to(BUFFERS *b, LINEBUFFER *lb, POLY_LIST *r) {
    ...
    for (u32 i = 0; i < 192; i++) {
        render_line<do_debug>(b, i, &lb[i], r);
    }
}
template void RE::render_frame<true>();
template void RE::render_frame<false>();

template<bool do_debug>
void RE::render_frame() {
    render_to<do_debug>(&ge->buffers[ge->ge_has_buffer ^ 1], out.linebuffer, &render_list);
    if constexpr(do_debug) {
        if (ge->debug_cam.enabled) {
            render_to<do_debug>(&ge->debug_cam.buffers[ge->ge_has_buffer ^ 1], out.debug_linebuffer, &debug_render_list);
        }
    }
}
