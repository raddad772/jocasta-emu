...
static void draw_line(u32 *outbuf, i32 out_width, i32 out_height, i32 x0, i32 y0, i32 x1, i32 y1)
{
...
static void render_image_view_re_wireframe(debugger_interface *dbgr, debugger_view *dview, void *ptr, u32 out_width) {
    ...
    for (u32 i = 0; i < b->polygon_index; i++) {
        ...
        for (i32 vn = 1; vn < p->vertex_list.len+1; vn++) {
            v0 = v1;
            v1 = v1->next;
...
static void setup_dbglog(debugger_interface *dbgr, core *th)
{
    ...
    th->arm7.dbg.dvptr = &dv;
    th->arm7.dbg.dv_id = NDS_CAT_ARM7_INSTRUCTION;
