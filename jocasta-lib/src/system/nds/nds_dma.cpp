...
static void dma7_irq(void *ptr, u64 key, u64 cur_time, u32 jitter)
{
    ...
    th->update_IF7<do_debug>(IRQ_DMA0 + key);
}
    ...
    while((ch.io.enable) && (ch.op.started)) {
        if (ch.op.sz == 2) {
            ...
            if (ch.op.src_addr >= 0x02000000) {
                value = mainbus_read7<2, do_debug, false>(this, ch.op.src_addr, ch.op.src_access);
            ...
            mainbus_write7<2, do_debug>(this, ch.op.dest_addr, ch.op.dest_access, value);
            num_transfer++;
        }
        else {
            if (ch.op.src_addr >= 0x02000000)
                ch.io.open_bus = mainbus_read7<4, do_debug, false>(this, ch.op.src_addr, ch.op.src_access);
            else
                waitstates.current_transaction++;
            mainbus_write7<4, do_debug>(this, ch.op.dest_addr, ch.op.dest_access, ch.io.open_bus);
            num_transfer++;
        }

        ch.op.src_access = ARM32P_sequential | ARM32P_dma;
        ch.op.dest_access = ARM32P_sequential | ARM32P_dma;
        ...
        if (ch.op.word_count == 0) {
            ...
            ch.op.first_run = 0;
            if (ch.io.irq_on_end) {
                scheduler.add_or_run_abs<do_debug>(clock.current7() + num_transfer, ch.num, this, &dma7_irq<false>, &dma7_irq<true>, nullptr);
                //dma7_irq(ch.num, 0, 0);
            }
...
template void core::dma7_start<true>(DMA_ch &ch, u32 i);
template void core::dma7_start<false>(DMA_ch &ch, u32 i);
template <bool do_debug>
void core::dma7_start(DMA_ch &ch, u32 i)
{
    if (ch.active) return;
    if constexpr(do_debug) dbgloglog(NDS_CAT_DMA_START, DBGLS_INFO, "DMA7 %d start", i);
    ch.op.started = 1;
    ...
    ch.op.dest_access = ARM32P_nonsequential | ARM32P_dma;
    ch.op.src_access = ARM32P_nonsequential | ARM32P_dma;
    dma7_go_ch<do_debug>(ch);
}
...
{
    auto *th = static_cast<core *>(ptr);
    th->update_IF9<do_debug>(IRQ_DMA0 + key);
}
...
void core::dma9_go_ch(DMA_ch &ch) {
    ...
    while ((ch.io.enable) && (ch.op.started)) {
        if (ch.op.sz == 2) {
            u16 value;
            if (ch.op.src_addr >= 0x02000000) {
                value = mainbus_read9<2, do_debug, false>(this, ch.op.src_addr, ch.op.src_access);
                ch.io.open_bus = (value << 16) | value;
            } else {
                if (ch.op.dest_addr & 2) {
                    value = ch.io.open_bus >> 16;
            ...
            mainbus_write9<2, do_debug>(this, ch.op.dest_addr, ch.op.dest_access, value);
            num_transfer++;
        }
        else {
            if (ch.op.src_addr >= 0x02000000)
                ch.io.open_bus = mainbus_read9<4, do_debug, false>(this, ch.op.src_addr, ch.op.src_access);
            else {
                waitstates.current_transaction++;
            }
            mainbus_write9<4, do_debug>(this, ch.op.dest_addr, ch.op.dest_access, ch.io.open_bus);
            num_transfer++;
        }
        ...
        if (ch.op.word_count == 0) {
            ...
            ch.op.first_run = 0;
            if (ch.io.irq_on_end) {
                scheduler.add_or_run_abs<do_debug>(clock.current9() + num_transfer, ch.num, this, &dma9_irq<false>, &dma9_irq<true>, nullptr);
                //dma9_irq(ch.num, 0, 0);
            }
...
template void core::dma9_start<true>(DMA_ch &ch, u32 i);
template void core::dma9_start<false>(DMA_ch &ch, u32 i);
template <bool do_debug>
void core::dma9_start(DMA_ch &ch, u32 i)
{
    if (ch.active) return;
    if constexpr(do_debug) dbgloglog(NDS_CAT_DMA_START, DBGLS_INFO, "DMA9 %d start", i);
...
// #############################
template void core::check_dma7_at_vblank<true>();
template void core::check_dma7_at_vblank<false>();
...
void core::check_dma7_at_vblank()
{
    ...
    for (u32 i = 0; i < 4; i++) {
        ...
        if ((ch.io.enable) && (!ch.op.started) && (ch.io.start_timing == DMA_VBLANK)) {
            dma7_start<do_debug>(ch, i);
        }
    }
}

template void core::check_dma9_at_vblank<true>();
template void core::check_dma9_at_vblank<false>();
template <bool do_debug>
void core::check_dma9_at_vblank()
{
    ...
    for (u32 i = 0; i < 4; i++) {
        ...
        if ((ch.io.enable) && (!ch.op.started) && (ch.io.start_timing == 1)) {
            dma9_start<do_debug>(ch, i);
        }
    }
}

template void core::check_dma9_at_hblank<true>();
template void core::check_dma9_at_hblank<false>();
...
template void core::trigger_dma7_if<true>(u32 start_timing);
template void core::trigger_dma7_if<false>(u32 start_timing);
...
template void core::trigger_dma9_if<true>(u32 start_timing);
template void core::trigger_dma9_if<false>(u32 start_timing);
template <bool do_debug>
