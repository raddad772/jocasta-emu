...
struct ENG2D {
    ...
    core *ppu;
    template<u8 sz> [[nodiscard]] u64 read_vram_bg(u32 addr) const {
        ...
        addr &= 0x3FFF;
        MT_READ1248(0x3FFF, addr, ptr);
        NOGOHERE;
    }

    template<u8 sz> [[nodiscard]] u32 read_vram_obj(u32 addr) const {
        ...
        MT_READ124(0x3FFF, addr, ptr);
    }

    template<u8 sz> [[nodiscard]] u32 read_pram_bg_ext(u32 slotnum, u32 addr) const {
        ...
        addr &= 0x1FFF;
        MT_READ124(0x1FFF, addr, memp.bg_extended_palette[slotnum & 3]);
    }

    template<u8 sz> [[nodiscard]] u32 read_pram_bg(u32 addr) const { MT_READ124(0x1FF, addr, mem.bg_palette); }
    ...
    [[nodiscard]] u32 read_ext_palette_obj(u32 palette, u32 index) const {
        {
            if (memp.obj_extended_palette) {
                ...
                addr &= 0x1FFF;
                u32 v = reinterpret_cast<u16 *>(memp.obj_extended_palette)[addr>> 1];
                return v;
            }
            return 0;
        }
    }
    [[nodiscard]] u32 read_pram_obj2(u32 palette, u32 index) {
        if (io.obj.extended_palettes)
            return read_ext_palette_obj(palette, index) & 0x7FFF;
        const u32 addr = ((palette << 5) | (index << 1)) & 0x1FF;
        return reinterpret_cast<u16 *>(mem.obj_palette)[addr >> 1] & 0x7FFF;
    }
    template<u8 sz> [[nodiscard]] u32 read_pram_obj(u32 addr) { MT_READ124(0x1FF, addr, mem.obj_palette); }
    template<u8 sz> [[nodiscard]] u32 read_oam(u32 addr) { MT_READ124(0x3FF, addr, mem.oam); }
    void draw_obj_line();
    ...
    template<bool do_debug> void draw_line0();
    template<bool do_debug> void draw_line1();
    template<bool do_debug> void draw_line2();
    template<bool do_debug> void draw_line3();
    template<bool do_debug> void draw_line5();
...
struct core {
    ...
    template<bool do_debug> void run_dispcap();
    template<bool do_debug> void draw_line(u32 eng_num);
    ...
    u32 read9_io(u32 addr, u8 sz, u8 access);
    u32 read9_io8(u32 addr, u8 sz, u8 access);

    [[nodiscard]] u32 read7_io(u32 addr, u8 sz, u8 access) const;
    [[nodiscard]] u32 read7_io8(u32 addr, u8 sz, u8 access) const;

    template<bool do_debug> static void vblank(void *ptr, u64 key, u64 clock, u32 jitter);
    template<bool do_debug> static void hblank(void *ptr, u64 key, u64 clock, u32 jitter);

    u32 *cur_output_top{};
    u32 *cur_output_bot{};
    cvec_ptr<physical_io_device> display_top_ptr{};
    cvec_ptr<physical_io_device> display_bot_ptr{};
    JSM_DISPLAY *display_top{};
    JSM_DISPLAY *display_bot{};
