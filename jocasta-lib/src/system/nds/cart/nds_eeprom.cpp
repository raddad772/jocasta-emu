...
void ridge::eeprom_write(u32 val)
{
    ...
    bool can_write = backup.status.write_enable;
    ...
    if (can_write) {
        ...
        backup.wrote_since_select = true;
    }
...
void ridge::eeprom_handle_spi_cmd(u32 val, u32 is_cmd)
{
    switch(backup.cmd) {
        ...
        case 5: // READ STATUS
            ...
            val = backup.status.u;
            if (backup.detect.sz <= 512) val |= 0xF0;
            backup.data_out.b8[0] = val;
            backup.data_out.b8[1] = val;
            backup.data_out.b8[2] = val;
            backup.data_out.b8[3] = val;
            break;
        case 1: // Write status
            if (is_cmd) return;
            if ((backup.data_in_pos == 0) && (backup.status.write_enable)) {
                flprintf("\nWR STATUS:%02x", val);
                backup.status.u = (backup.status.u & 3) | (val & 0x0C);
                backup.wrote_since_select = true;
            }
            else {
                printf("\nWRITE STATUS EXTRA!?");
