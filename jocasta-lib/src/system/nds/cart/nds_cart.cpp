...
void ridge::reset()
{
    ...
    backup.cmd_addr = 0;
    backup.data_out.b32 = 0xFFFFFFFF;
    backup.status.u = 0;
    backup.chipsel = 0;
    backup.wrote_since_select = 0;
    io.spi.busy_until = 0;
}
...
u32 ridge::read_spicnt() const {
    ...
    else v |= (bus->clock.current7() < io.spi.busy_until) << 7;
    return v;
}
...
void ridge::raise_transfer_irq() {
    bus->update_IFs_card<do_debug>(IRQ_CART_DATA_READY);
    printf("\nRAISE TRANSFER IRQ...");
}
...
u32 ridge::read_spi(u32 bnum) const {
    if (!io.nds_slot_enable || !io.spi.slot_mode) return 0;
    if (bus->clock.current7() < io.spi.busy_until) return 0;
    return backup.data_out.b8[bnum];
}

template u32 ridge::read_rom<true>(u32 addr, u8 sz);
template u32 ridge::read_rom<false>(u32 addr, u8 sz);
template<bool do_debug>
u32 ridge::read_rom(u32 addr, u8 sz)
{
    ...
    if (cmd.pos_out == cmd.sz_out) {
        ...
        bus->scheduler.add_or_run_abs<do_debug>(rom_busy_until, ANB_after_read, this, &ridge::check_transfer<false>, &ridge::check_transfer<true>, &sch_sch);
    }
...
void ridge::after_read()
{
    ...
    if (bus->io.rights.nds_slot_is7) bus->trigger_dma7_if<do_debug>(2);
    else bus->trigger_dma9_if<do_debug>(DMA_DS_CART_SLOT);
}
...
void ridge::check_transfer(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    switch(key) {
        case ANB_none:
            return;
        case ANB_handle_cmd:
            th->handle_cmd<do_debug>();
            return;
        case ANB_after_read:
            th->after_read<do_debug>();
            return;
    }
}
...
{
    if (!io.nds_slot_enable || !io.spi.slot_mode) return;
    if (bus->clock.current7() < io.spi.busy_until) return;
    io.spi.busy_until = bus->clock.current7() + (8 * (io.spi.divider ? io.spi.divider : 8));
...
template void ridge::write_romctrl<true>(u32 val);
template void ridge::write_romctrl<false>(u32 val);
template<bool do_debug>
void ridge::write_romctrl(u32 val)
{
    ...
    if ((val & 0x80000000) && !io.romctrl.busy) {
        ...
        if (sch_sch) bus->scheduler.delete_if_exist(sch_id);
        bus->scheduler.add_or_run_abs<do_debug>(rom_busy_until, ANB_handle_cmd, this, &ridge::check_transfer<false>, &ridge::check_transfer<true>, &sch_sch);
    }
}
