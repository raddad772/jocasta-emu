...
struct ridge {
    ...
    template<bool do_debug> [[nodiscard]] u32 read_rom(u32 addr, u8 sz);
    [[nodiscard]] u32 get_block_size() const;
    void spi_write_spicnt(u32 val, u32 bnum);
    template<bool do_debug> void after_read();
    template<bool do_debug> static void check_transfer(void *ptr, u64 key, u64 clock, u32 jitter);
    template<bool do_debug> void raise_transfer_irq();
    void reset();
    void direct_boot();
    template<bool do_debug> void set_block_start_status(u32 val, u32 transfer_ready_irq);
    template<bool do_debug> void handle_cmd();
    void inc_addr();
    void spi_transaction(u32 val);
    template<bool do_debug> void write_romctrl(u32 val);
