...
void timer7_t::cascade_step(u64 current_time)
{
    ...
    if (val_at_stop == 0) {
        overflow<do_debug>(current_time);
    }
}
template<bool do_debug>
...
void timer7_t::overflow(u64 current_time) {
    ...
    if (!cascade)
        sch_id = bus->scheduler.add_or_run_abs<do_debug>(overflow_at, num, bus, &timer7_t::overflow_callback<false>, &timer7_t::overflow_callback<true>, &sch_scheduled_still);

    if (irq_on_overflow) {
        //printf("\nIRQ!");
        bus->update_IF7<do_debug>(IRQ_TIMER0 + num);
    }
    ...
    if (num < 3) {
        ...
        if (tp1.cascade) {
            tp1.cascade_step<do_debug>(current_time);
        }
    }
}
...
void timer9_t::overflow(u64 current_time)
{
    ...
    overflow_at = enable_at + reload_ticks;
    if (!cascade)
        sch_id = bus->scheduler.add_or_run_abs<do_debug>(overflow_at, num, bus, &timer9_t::overflow_callback<false>, &timer9_t::overflow_callback<true>, &sch_scheduled_still);

    if (irq_on_overflow) {
        bus->update_IF9<do_debug>(IRQ_TIMER0 + num);
    }
...
void timer7_t::overflow_callback(void *ptr, u64 timer_num, u64 current_clock, u32 jitter)
{
    ...
    th->timer7[timer_num].overflow<do_debug>(th->clock.current7());
}
...
{
    auto *th = static_cast<core *>(ptr);
    th->timer9[timer_num].overflow<do_debug>(th->clock.current9());
}

template void timer7_t::write_cnt<true>(u32 val);
template void timer7_t::write_cnt<false>(u32 val);
template void timer9_t::write_cnt<true>(u32 val);
template void timer9_t::write_cnt<false>(u32 val);
template<bool do_debug>
