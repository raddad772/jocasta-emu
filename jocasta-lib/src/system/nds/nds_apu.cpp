...
void MCH::update_len()
{
    switch(io.format) {
        case FMT_pcm8:
            io.sample_len = 4 * (io.loop_start_pos + io.len);
            break;
        case FMT_pcm16:
            io.sample_len = 2 * (io.loop_start_pos + io.len);
            break;
        case FMT_ima_adpcm:
            // End = (PNT + LEN) words from source, minus 1 header word, in nibbles
            io.sample_len = 8 * (io.loop_start_pos + io.len - 1);
            break;
        case FMT_psg:
            break;
    }
...
void MCH::run_pcm8()
{
    sample = static_cast<i16>(static_cast<i8>(NDS::core::mainbus_read7<1, false, true>(apu->bus, io.source_addr + status.pos, 0)) << 8);
    status.pos++;
}

void MCH::run_pcm16()
{
    sample = static_cast<i16>(NDS::core::mainbus_read7<2, false, true>(apu->bus, io.source_addr + (status.pos << 1), 0));
    status.pos++;
}
...
{
    if ((status.pos & 7) == 0) {
        adpcm.data = NDS::core::mainbus_read7<4, false, true>(apu->bus, io.source_addr + ((status.pos >> 3) << 2) + 4, 0);
    }
    // Save ADPCM state at the loop point (before decoding the loop-point nibble),
    // so that on loop we restore to the correct context for re-decoding from here.
    if (status.pos == status.real_loop_start_pos) {
        adpcm.loop_sample = static_cast<i16>(adpcm.sample);
...
void MCH::run_psg()
{
    // HIGH for first (wave_duty+1) of 8 clocks; pos must wrap at 8
    sample = static_cast<i16>((status.pos <= io.wave_duty) ? 0x7FFF : -0x7FFF);
    status.pos = (status.pos + 1) & 7;
}
...
void MCH::probe_trigger(u32 old_status)
{
    ...
    if (io.format == FMT_ima_adpcm) {
        u32 adpcm_header = NDS::core::mainbus_read7<4, false, true>(apu->bus, io.source_addr, 0);
...
void core::master_sample_callback(void *ptr, u64 nothing, u64 cur_clock, u32 jitter)
{
    ...
    if (th->output_ring) {
        // Direct-push path: no intermediate buffer, no fixed frame size.
        // push() silently drops when full (fast-forward protection built-in).
        float l = ((static_cast<float>(left)  + 512.0f) / 511.5f) - 1.0f;
        float r = ((static_cast<float>(right) + 512.0f) / 511.5f) - 1.0f;
        th->output_ring->push(l, r);
    } else {
        // Legacy path: accumulate into apu.buffer for sample_audio() to drain.
