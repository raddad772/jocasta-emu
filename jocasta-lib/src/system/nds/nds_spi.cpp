...
void core::SPI_irq(void *ptr, u64 num_cycles, u64 clock, u32 jitter)
{
    ...
    if (th->SPI.cnt.irq_enable)
        th->update_IF7<do_debug>(IRQ_SPI);
}
...
void core::SPI_transaction(u32 val)
{
    ...
    // Schedule IRQ
    SPI.irq_id = scheduler.add_or_run_abs<do_debug>(SPI.busy_until, 0, this, &SPI_irq<false>, &SPI_irq<true>, nullptr);
}
...
template void core::SPI_write<true>(u8 sz, u32 val);
template void core::SPI_write<false>(u8 sz, u32 val);
template<bool do_debug>
void core::SPI_write(u8 sz, u32 val)
{
    SPI_transaction<do_debug>(val & 0xFF);
}
}