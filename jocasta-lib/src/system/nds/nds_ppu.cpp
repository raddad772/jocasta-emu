...
void ENG2D::draw_obj_on_line(u32 oam_offset, u32 prio_to_draw)
{
    ...
    attr[2] = read_oam<2>(oam_offset+4);
    u32 priority = (attr[2] >> 10) & 3;
    if (priority != prio_to_draw) return;

    attr[0] = read_oam<2>(oam_offset);
    ...
    attr[1] = read_oam<2>(oam_offset+2);
    i32 x = attr[1] & 0x1FF;
    u32 sz = (attr[1] >> 14) & 3;
    if (x & 0x100) x -= 512;
    ...
    if (affine) {
        ...
        pa = read_oam<2>(pgroup + 6);
        pb = read_oam<2>(pgroup + 14);
        pc = read_oam<2>(pgroup + 22);
        pd = read_oam<2>(pgroup + 30);
        pa = sign_extend<16>(pa);
        pb = sign_extend<16>(pb);
        pc = sign_extend<16>(pc);
        pd = sign_extend<16>(pd);
    }
    else {
        pa = 1 << 8;
        pb = 0;
    ...
    for (i32 lx = -half_width; lx <= half_width; lx++) {
        PX mypx = {.priority=priority, .has=1, .sp_mosaic=m_mosaic, .dbg_mode=mode, .dbg_bpp = dbpp};
        ...
        if (mode == 3) { // bitmap
            ...
            c = read_pram_obj<2>(((palette << 4) | c) << 1);
...
void ENG2D::draw_bg_line_normal(u32 bgnum)
{
    ...
    do {
        do {
            encoder = read_vram_bg<2>(base + grid_x++ * 2);
            if (encoder != last_encoder) {
                ...
                if (bg.bpp8) { // 8bpp
                    u64 data = read_vram_bg<8>(tile_base + (number << 6 | _tile_y << 3));
                    for (u32 ix = 0; ix < 8; ix++) {
                        ...
                        if (index == 0) {
                            ...
                            if (io.bg.extended_palettes)
                                c = read_pram_bg_ext<2>(expal_slot, palette << 9 | index << 1);
                            else
                                c = read_pram_bg<2>(index << 1);
                            tile[mx].has = 1;
                    ...
                    u32 data = read_vram_bg<4>(tile_base + (number << 5 | _tile_y << 2));
                    for (u32 ix = 0; ix < 8; ix++) {
                        u32 mx = ix ^ xxor;
                        u32 index = data & 15;
                        data >>= 4;
                        if (index == 0) {
                            ...
                            tile[mx].has = 1;
                            tile[mx].priority = bg.priority;
                            u32 c = read_pram_bg<2>(palette << 5 | index << 1);
                            tile[mx].color = c15to18(c);
                        }
                    }
                }
...
void ENG2D::affine_normal(MBG &bg, i32 x, i32 y, PX &out, void *xtra) const
{
    ...
    u32 tile_number = read_vram_bg<1>(xd.map_base + (y >> 3) * xd.block_width + (x >> 3));

    u32 addr = (xd.tile_base + tile_number * 64);
    u32 c = read_vram_bg<1>(addr + ((y & 7) << 3) + (x & 7));
...
void ENG2D::affine_rotodc(MBG &bg, i32 x, i32 y, PX &out, void *xtra) const
{
    ...
    u32 color = read_vram_bg<2>(bg.screen_base_block + (y * xd.width + x) * 2);
...
void ENG2D::affine_rotobpp8(MBG &bg, i32 x, i32 y, PX &out, void *xtra) const
{
    ...
    const u32 color = read_vram_bg<1>(bg.screen_base_block+ y * xd.width + x);
    if (color) {
        out.has = 1;
        out.color = c15to18(read_pram_bg<2>(color << 1));
        out.priority = bg.priority;
    } else {
        out.has = 0;
    }
...
{
    auto &xd = *static_cast<affine_normal_xtradata *>(xtra);
    const u32 encoder = read_vram_bg<2>(xd.map_base + ((y >> 3) * xd.block_width + (x >> 3)) * 2);
    ...
    const u8 index = read_vram_bg<1>(addr + (tile_y << 3) + tile_x);
    if (index == 0) {
        out.has = 0;
    }
    else {
        ...
        if (io.bg.extended_palettes)
            out.color = c15to18(read_pram_bg_ext<2>(bg.num, index << 1));
        else
            out.color = c15to18(read_pram_bg<2>(index << 1));
        out.has = 1;
        out.priority = bg.priority;
    }
}
...
void ENG2D::calculate_windows()
{
    ...
    //if constexpr(do_debug) {
        /*NDS_PPU_window *wo = &eng.window[WINOUTSIDE];
...
void ENG2D::output_pixel(u32 x, bool obj_enable, bool bg_enables[4]) {
    ...
    PX *sp_px = &obj.line[x];
    PX empty_px;
    empty_px.color=c15to18(read_pram_bg<2>(0));
    empty_px.priority=4;
    ...
    if (target_a_layer == 1 && BG[0].do3d && ppu->bus->re.io.DISP3DCNT.alpha_blending && blend.targets_b[target_b_layer]) {
        u32 a3d = BG[0].alpha_3d[x];
        if (a3d < 31) {
            u32 eva = a3d + 1;
            output_color = nds_alpha_3d(output_color, blend_b, eva, 32 - eva);
        }
...
void core::run_dispcap()
{
    ...
    if constexpr(do_debug) memcpy(bus->dbg_info.eng[0].line[bus->clock.ppu.y].dispcap_px, src, x_size * 2);
}
...
void core::draw_line(u32 eng_num) {
    ...
    DBG_line *l;
    if constexpr(do_debug) {
        l = &bus->dbg_info.eng[eng_num].line[bus->clock.ppu.y];
    ...
    switch(eng.io.bg_mode) {
        case 0:
            eng.draw_line0<do_debug>();
            break;
        case 1:
            eng.draw_line1<do_debug>();
            break;
        case 2:
            eng.draw_line2<do_debug>();
            break;
        case 3:
            eng.draw_line3<do_debug>();
            break;
        case 5:
            eng.draw_line5<do_debug>();
            break;
        default: {
            static u32 warned = 0;
    ...
    u32 *screen = (eng_num ^ io.display_swap ^ 1) ? cur_output_bot : cur_output_top;
    u32 *line_output = screen + (bus->clock.ppu.y * OUT_WIDTH);
...
template void core::hblank<true>(void *ptr, u64 key, u64 clock, u32 jitter);
template void core::hblank<false>(void *ptr, u64 key, u64 clock, u32 jitter);
template void core::vblank<true>(void *ptr, u64 key, u64 clock, u32 jitter);
template void core::vblank<false>(void *ptr, u64 key, u64 clock, u32 jitter);
...
void core::hblank(void *ptr, u64 key, u64 clock, u32 jitter) // Called at hblank time
{
    ...
    if (key == 0) { // end hblank...new line!
        ...
        if (bus->clock.ppu.y >= bus->clock.timing.scanline.number) {
            ppu.new_frame<do_debug>();
        }
        ...
        if ((ppu.io.vcount_at7 == bus->clock.ppu.y) && ppu.io.vcount_irq_enable7) bus->update_IF7<do_debug>(IRQ_VMATCH);
        if ((ppu.io.vcount_at9 == bus->clock.ppu.y) && ppu.io.vcount_irq_enable9) bus->update_IF9<do_debug>(IRQ_VMATCH);
    }
    else {
        // Start of hblank. Now draw line!
        if (bus->clock.ppu.y < 192) {
            ppu.draw_line<do_debug>(0);
            ppu.draw_line<do_debug>(1);
            if (ppu.doing_capture) ppu.run_dispcap<do_debug>();
        } else {
            ppu.calculate_windows_vflags(ppu.eng2d[0]);
            ppu.calculate_windows_vflags(ppu.eng2d[1]);
        }
        if (ppu.io.hblank_irq_enable7) bus->update_IF7<do_debug>(IRQ_HBLANK);
        if (ppu.io.hblank_irq_enable9) bus->update_IF9<do_debug>(IRQ_HBLANK);
        bus->check_dma9_at_hblank<do_debug>();
    }
}
...
void core::new_frame() {
    ...
    display_top->active_draw_buffer ^= 1;
    display_bot->active_draw_buffer ^= 1;
    cur_output_top = static_cast<u32 *>(display_top->output[display_top->active_draw_buffer]);
    cur_output_bot = static_cast<u32 *>(display_bot->output[display_bot->active_draw_buffer]);
    ...
    bus->trigger_dma9_if<do_debug>(DMA_START_OF_DISPLAY);
}
...
void core::vblank(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    if (key) { // line 192
        if (ppu.io.vblank_irq_enable7) bus->update_IF7<do_debug>(IRQ_VBLANK);
        if (ppu.io.vblank_irq_enable9) bus->update_IF9<do_debug>(IRQ_VBLANK);
        bus->check_dma7_at_vblank<do_debug>();
        bus->check_dma9_at_vblank<do_debug>();
        bus->ge.vblank_up();
...
u32 core::read9_io8(u32 addr, u8 sz, u8 access)
{
    u32 en = 0;
    if (addr >= 0x04001000) {
        addr -= 0x1000;
...
u32 core::read7_io8(u32 addr, u8 sz, u8 access) const
{
    u32 v;
    switch(addr) {
       case R_DISPSTAT+0: // DISPSTAT lo
...
u32 core::read7_io(u32 addr, u8 sz, u8 access) const
{
    u32 v = read7_io8(addr, sz, access);
    if (sz >= 2) v |= read7_io8(addr+1, sz, access) << 8;
    if (sz == 4) {
        v |= read7_io8(addr+2, sz, access) << 16;
        v |= read7_io8(addr+3, sz, access) << 24;
    }
    return v;
}

u32 core::read9_io(u32 addr, u8 sz, u8 access)
{
    u32 v = read9_io8(addr, sz, access);
    if (sz >= 2) v |= read9_io8(addr+1, sz, access) << 8;
    if (sz == 4) {
        v |= read9_io8(addr+2, sz, access) << 16;
        v |= read9_io8(addr+3, sz, access) << 24;
    }
    return v;
}
