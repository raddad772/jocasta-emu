...
#include "mbc3.h"
namespace GB::MAPPER {
#define SELF struct MBC3* self = (MBC3*)parent->ptr
...
void MBC3_new(base *parent, CLOCK *clock, core *bus)
{
    auto *self = static_cast<MBC3 *>(malloc(sizeof(MBC3)));
    ...
    parent->CPU_read = &MBC3_CPU_read;
    parent->CPU_write = &MBC3_CPU_write;
    parent->reset = &MBC3_reset;
    parent->set_cart = &MBC3_set_cart;
...
static void MBC3_remap(MBC3 *self)
{
...
void MBC3_CPU_write(base* parent, u32 addr, u32 val)
{
    ...
    if ((addr >= 0xA000) && (addr < 0xC000)) { // cart RAM
        if ((self->has_RAM) && (self->regs.RAM_bank < 4)) {
            (static_cast<u8 *>(self->cart->SRAM->data))[(addr & 0x1FFF) + self->RAM_bank_offset] = val;
