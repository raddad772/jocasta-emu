...
{
    u32 k = MIN(static_cast<u32>(header.ROM_size), static_cast<u32>(size));
    header.ROM_size = k;
    assert(ROM!=nullptr);
    for (u32 i = 0; i < k; i++) {
        ROM[i] = inp[i];
...
{
    mapper = MAPPER::new_mapper(&bus->clock, bus, header.mapper);
    if (bus->mapper != nullptr) {
        MAPPER::delete_mapper(bus->mapper);
    }
    bus->mapper = mapper;
