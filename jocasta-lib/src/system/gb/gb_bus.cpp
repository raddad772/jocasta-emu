...
void core::CPU_write(u32 addr, u32 val) {
    ...
    if ((addr >= 0x8000) && (addr < 0xA000)) { // VRAM
        ...
        generic_mapper.VRAM[(addr & 0x1FFF) + generic_mapper.VRAM_bank_offset] = static_cast<u8>(val);
        return;
    }

    if ((addr >= 0xC000) && (addr < 0xD000)) { // WRAM lo bank
        generic_mapper.WRAM[addr & 0xFFF] = static_cast<u8>(val);
        return;
    }
    ...
    if ((addr >= 0xFF80) && (addr < 0xFFFF)) { // HRAM always accessible
        generic_mapper.HRAM[addr - 0xFF80] = static_cast<u8>(val);
        return;
    }
