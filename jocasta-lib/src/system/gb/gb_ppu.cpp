...
static inline u32 GB_sp_tile_addr(u32 tn, u32 y, u32 big_sprites, u32 attr, u32 cgb) {
    ...
    if (big_sprites) {
        ...
        if (y_flip) y = 15 - y;
        if (y > 7) tn++;
        return (0x8000 | (tn << 4) | ((y & 7) << 1)) + hbits;
    }
    if (y_flip) y = 7 - y;
...
void core::write_OAM(u32 addr, u32 val) {
    if (addr < 0xFEA0) OAM[addr - 0xFE00] = static_cast<u8>(val);
}
...
void core::advance_frame(bool update_buffer) {
    ...
    if (update_buffer) {
        ...
        cur_output_debug_metadata = static_cast<u16 *>(display->output_debug_metadata[display->active_draw_buffer]);
    }
}
...
void core::cycle() {
    ...
    switch (clock->ppu_mode) {
        case modes::HBLANK:
        case modes::VBLANK: break;
        case modes::OAM_search: // OAM search 0-80
            // 80 dots long, 2 per entry, find up to 10 sprites 0...n on this line
            OAM_search();
            if (line_cycle == 79) set_mode(PPU::pixel_transfer);
            break;
        case modes::pixel_transfer: // Pixel transfer. Complicated.
            pixel_transfer();
            if (clock->lx > 167)
                set_mode(PPU::HBLANK);
            break;
