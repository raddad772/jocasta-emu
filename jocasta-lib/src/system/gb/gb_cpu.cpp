...
TIMER::TIMER(void (*raise_IRQ_func)(CPU *), CPU *cpu_in) :
    raise_IRQ_ptr(cpu_in),
    raise_IRQ(raise_IRQ_func)
    {
...
void CPU::ghdma_run()
{
    ...
    if (hdma.til_next_byte < 1) {
        ...
        // A 16-byte block has finished
        if ((hdma.dest_index & 0x0F) == 0) {
            if (--hdma.length == 0) { // Terminate GDMA
                hdma.enabled = false;
                hdma.waiting = false;
                hdma.completed = true;
                hdma.active = false;
...
u32 CPU::read_IO(u32 addr, u32 val)
{
    ...
    switch(addr) {
        ...
        case 0xFF55: // HDMA5
            if (!clock->cgb_enable) return 0xFF;
            return (hdma.enabled ? 0 : 0x80) | ((hdma.length - 1) & 0x7F);
...
void CPU::write_IO(u32 addr, u32 val)
{
    ...
    switch(addr) {
        ...
        case 0xFF54: // HDMA4 bits
            if (!clock->cgb_enable) return;
            hdma.dest = (hdma.dest & 0xFF00) | (val & 0xF0);
            return;
        case 0xFF55: // HDMA5 transfer stuff!
            if (!clock->cgb_enable) return;

            // Writing bit 7 clear while H-blank DMA is armed cancels it.
            // GDMA writes, and writes that start a new H-blank DMA, reload the
            // source/destination latch below instead.
            if (hdma.enabled && (hdma.mode == 1) && ((val & 0x80) == 0)) {
                hdma.length = (val & 0x7F) + 1;
                hdma.enabled = false;
                hdma.waiting = false;
                hdma.completed = true;
                hdma.active = false;
            ...
            hdma.enabled = true;
            hdma.completed = false;
            hdma.active = hdma.mode == 0;
            hdma.waiting = hdma.mode != 0;
            if (hdma.mode == 1) {
                hdma.notify_hblank = (!bus->ppu.enabled) || ((clock->ppu_mode == PPU::HBLANK) && (clock->ly < 144));
            }
            else {
                hdma.notify_hblank = false;
            }
