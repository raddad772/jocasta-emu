...
void core::serialize_sp_obj_pointer(PPU::SPRITE *fo, serialized_state &state)
{
    u32 v = 80;
    if (fo == nullptr)
        v = 100;
...
void core::deserialize_sp_obj_pointer(PPU::SPRITE **fo, serialized_state &state)
{
    u32 v;
...
void PPU::FIFO::serialize(GB::core *gb, serialized_state &state)
{
#define S(x) Sadd(state, &( x), sizeof( x))
    S(variant);
    S(compat_mode);
...
void PPU::FIFO::deserialize(GB::core *gb, serialized_state &state)
{
