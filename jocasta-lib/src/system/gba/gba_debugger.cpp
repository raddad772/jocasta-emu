...
static void render_image_view_sprites(debugger_interface *dbgr, debugger_view *dview, void *my_ptr, u32 out_width) {
    ...
    for (u32 i = 0; i < 128; i++) {
        u32 sn = 127 - i;
        u16 *ptr = (reinterpret_cast<u16 *>(th->ppu.OAM)) + (sn * 4);
        u32 x = ptr[1] & 0x1FF;
        x = sign_extend<9>(x);
        ...
        i32 y_max = (y_min + static_cast<i32>(vpixels)) & 255;
        if (y_max < y_min)
            y_min -= 256;
        ...
        i32 half_x = 0 - (static_cast<i32>(hpixels) >> 1);
        i32 half_y = 0 - (static_cast<i32>(vpixels) >> 1);

        // Setup for affine...
        if (affine) {
            u32 pgroup = (ptr[1] >> 9) & 31;
            u32 pbase = (pgroup * 0x20) >> 1;
            u16 *pbase_ptr = (reinterpret_cast<u16 *>(th->ppu.OAM)) + pbase;
            pa = sign_extend<16>(pbase_ptr[3]);
            pb = sign_extend<16>(pbase_ptr[7]);
            pc = sign_extend<16>(pbase_ptr[11]);
            pd = sign_extend<16>(pbase_ptr[15]);
            ...
            ty = half_y << 8;
            if (hflip) {
                tx = static_cast<i32>(hpixels<<7) - (1 << 8);
                pa = static_cast<i32>(0xFFFFFF00);
            }
            if (vflip) {
                ty = static_cast<i32>(vpixels<<7) - (1 << 8);
                pd = static_cast<i32>(0xFFFFFF00);
            }
        }
        i32 screen_x_right = screen_x + static_cast<i32>(hpixels);
        i32 screen_y_bottom = screen_y + static_cast<i32>(vpixels);
        ...
        for (i32 draw_y = screen_y; draw_y < screen_y_bottom; draw_y++) {
            ...
            for (i32 draw_x = screen_x; draw_x < screen_x_right; draw_x++) {
                ...
                i32 tex_x = (fx >> 8) + (static_cast<i32>(tex_x_pixels) >> 1);
                i32 tex_y = (fy >> 8) + (static_cast<i32>(tex_y_pixels) >> 1);
...
static void render_image_view_bgmap(debugger_interface *dbgr, debugger_view *dview, void *ptr, u32 out_width, int bg_num) {
    ...
    u32 affine_char_base = 0;
    u32 affine_screen_base = 0;
    u32 has_affine = 0;
    u32 affine_htiles = 0;
    ...
    if (th->ppu.io.bg_mode == 4) {
        ...
        if (has_affine) {
            for (u32 tilemap_y = 0; tilemap_y < max_vsize; tilemap_y++) {
                ...
                for (u32 tilemap_x = 0; tilemap_x < max_hsize; tilemap_x++) {
                    ...
                    u32 tile_num = (static_cast<u8 *>(th->ppu.VRAM))[screenblock_addr];
                    ...
                    u32 has = 0;
                    u8 *lsptr = (static_cast<u8 *>(th->ppu.VRAM)) + line_start_addr;
                    u8 data = lsptr[px & 7];
        ...
        } else if (th->ppu.io.bg_mode < 3) {
            // Non-affine BG (modes 0-2 where this BG is text/regular)
            screen_base_block = bg->screen_base_block;
            character_base_block = bg->character_base_block;
            u32 tile_bytes = bpp8 ? 64 : 32;
            u32 line_size  = bpp8 ? 8  : 4;
            for (u32 tilemap_y = 0; tilemap_y < max_vsize; tilemap_y++) {
                u32 *lineptr = outbuf + (tilemap_y * out_width);
                u8 *scrollptr = &th->dbg_info.bg_scrolls[bg_num].lines[128 * tilemap_y];
                for (u32 tilemap_x = 0; tilemap_x < max_hsize; tilemap_x++) {
                    u32 is_shaded = (scrollptr[tilemap_x >> 3] >> (tilemap_x & 7)) & 1;
                    u32 block_x = tilemap_x >> 3;
                    u32 block_y = tilemap_y >> 3;
                    u32 line_in_tile = tilemap_y & 7;
                    u32 col_in_tile  = tilemap_x & 7;

                    u32 screenblock_addr = screen_base_block + (se_index_fast(block_x, block_y, bg->screen_size) << 1);
                    u16 attr = *reinterpret_cast<u16 *>((static_cast<u8 *>(th->ppu.VRAM)) + screenblock_addr);
                    u32 tile_num = attr & 0x3FF;
                    u32 hflip    = (attr >> 10) & 1;
                    u32 vflip    = (attr >> 11) & 1;
                    u32 palbank  = bpp8 ? 0 : ((attr >> 12) & 15) << 4;

                    if (vflip) line_in_tile = 7 - line_in_tile;
                    if (hflip) col_in_tile  = 7 - col_in_tile;
                    u32 draw_col = col_in_tile;
                    if (!bpp8) draw_col >>= 1;

                    u32 tile_start_addr = character_base_block + (tile_num * tile_bytes);
                    u32 line_addr = tile_start_addr + (line_in_tile * line_size);

                    u32 color = 0xFF000000;
                    if (line_addr < 0x10000) {
                        u8 *tileptr = (static_cast<u8 *>(th->ppu.VRAM)) + line_addr;
                        u32 c = tileptr[draw_col];
                        if (!bpp8) {
                            if (tilemap_x & 1) c >>= 4;
                            c &= 15;
                        }
                        if (c != 0) {
                            color = gba_to_screen(th->ppu.palette_RAM[palbank + c]);
                        }
                    }
                    lineptr[tilemap_x] = shade_boundary_func(is_shaded * 3, color);
                }
...
static void render_image_view_bg(debugger_interface *dbgr, debugger_view *dview, void *ptr, u32 out_width, int bg_num)
{
    ...
    for (i32 screen_y = 0; screen_y < 160; screen_y++) {
        ...
        i32 hpixels_mask = static_cast<i32>(hpixels) - 1;
        i32 vpixels_mask = static_cast<i32>(vpixels) - 1;
        ...
        for (i32 screen_x = 0; screen_x < 240; screen_x++) {
            ...
            if (!no_px) {
                ...
                if (is_affine) {
                    ...
                    u32 line_start_addr = tile_start_addr + (line_in_tile * 8);
                    u8 *tileptr = (static_cast<u8 *>(th->ppu.VRAM)) + line_start_addr;
                    color = tileptr[px & 7];
                    if (color != 0) {
                        has = 1;
                        bpp8 = 1;
...
{
    auto* th = static_cast<core *>(gbaptr);
    printf("\nWARNING IMPL get_disassembly_ARM7TDMI");
...
static void setup_dbglog(debugger_interface *dbgr, core *th) {
    ...
    th->cpu.dbg.dvptr = dv;
    th->cpu.dbg.dv_id = CAT_ARM7_INSTRUCTION;
