...
MT_IRW32(GBA::core, mainbus_read_VRAM, mainbus_write_VRAM);
MT_IRW32(GBA::core, mainbus_read_palette, mainbus_write_palette);
MT_IRW32(GBA::core, mainbus_read_OAM, mainbus_write_OAM);
...
void core::draw_sprite_affine(const u16 *ptr, PPU::WINDOW *w, u32 num)
{
    ...
    const i32 pa = sign_extend<16>(pbase_ptr[3]);
    const i32 pb = sign_extend<16>(pbase_ptr[7]);
    const i32 pc = sign_extend<16>(pbase_ptr[11]);
    const i32 pd = sign_extend<16>(pbase_ptr[15]);
...
void core::draw_bg_line_affine(u32 bgnum)
{
    ...
    for (i64 screen_x = 0; screen_x < 240; screen_x++) {
        ...
        if constexpr (do_debug) gba->dbg_info.bg_scrolls[bgnum].lines[(py << 7) + (px >> 3)] |= 1 << (px & 7);
    }
    affine_line_end(bg);
}
...
#define scroll_line reinterpret_cast<u8 *>(&gba->dbg_info.bg_scrolls[bgnum].lines[((bg.vscroll + gba->clock.ppu.y) & bg.vpixels_mask) * 128])
    u32 startx = fine_x;
    for (u32 i = startx; i < 8; i++) {
        ...
        if constexpr (do_debug) scroll_line[hpos >> 3] |= 1 << (hpos & 7);
    }
...
{
    draw_obj_line();
    draw_bg_line_normal<do_debug>(0);
    draw_bg_line_normal<do_debug>(1);
    draw_bg_line_normal<do_debug>(2);
    draw_bg_line_normal<do_debug>(3);
    apply_mosaic();
    calculate_windows<do_debug>(false);
...
void core::draw_line1(u16 *line_output)
{
    ...
    draw_bg_line_normal<do_debug>(0);
    draw_bg_line_normal<do_debug>(1);
    draw_bg_line_affine<do_debug>(2);
    apply_mosaic();
...
void core::draw_line2(u16 *line_output)
{
    ...
    draw_bg_line_affine<do_debug>(2);
    draw_bg_line_affine<do_debug>(3);
    calculate_windows<do_debug>(false);
    apply_mosaic();
...
void core::do_hblank(bool in_hblank) {
    ...
    if (in_hblank) { // mid-scanline
        if  (gba->clock.ppu.y < 160) {
            ...
            switch (io.bg_mode) {
                case 0:
                    draw_line0<do_debug>(line_output);
                    break;
                case 1:
                    draw_line1<do_debug>(line_output);
                    break;
                case 2:
                    draw_line2<do_debug>(line_output);
                    break;
                case 3:
                    draw_line3<do_debug>(line_output);
                    break;
                case 4:
                    draw_line4<do_debug>(line_output);
                    break;
                case 5:
                    draw_line5<do_debug>(line_output);
                    break;
                default:
                    assert(1 == 2);
            }
        ...
        if  (gba->clock.ppu.y < 160) {
#define dbgl gba->dbg_info.line[gba->clock.ppu.y]
#define dbgbg dbgl.bg[i]
            for (u32 i = 2; i < 4; i++) {
                BG &bg = mbg[i];
                //auto &dbgbg = ;
                if ( (gba->clock.ppu.y == 1) || bg.x_written) {
                    bg.x_lerp = bg.x;
                    if constexpr (do_debug) dbgbg.reset_x = 1;
                }
                if constexpr (do_debug) dbgbg.x_lerp = bg.x_lerp;

                if ((gba->clock.ppu.y == 1) || (bg.y_written)) {
                    bg.y_lerp = bg.y;
                    if constexpr (do_debug) dbgbg.reset_y = 1;
                }
                if constexpr (do_debug) dbgbg.y_lerp = bg.y_lerp;
...
void core::schedule_frame(void *ptr, u64 key, u64 clock, u32 jitter) {
    ...
    if (::dbg.do_debug) th->do_schedule_frame<true>(cur_clock);
    else th->do_schedule_frame<false>(cur_clock);
}
...
void core::do_schedule_frame(i64 cur_clock)
{
    ...
    for (u32 line = 0; line < 228; line++) {
        ...
        gba->scheduler.only_add_abs(cur_clock+MASTER_CYCLES_BEFORE_HBLANK, 1, this, &hblank<false>, &hblank<true>, nullptr);
        gba->scheduler.only_add_abs_w_tag(cur_clock+MASTER_CYCLES_PER_SCANLINE, 0, this, &hblank<false>, &hblank<true>, nullptr, 1);
    ...
    cur_output = static_cast<u16 *>(display->output[display->active_draw_buffer]);
...
{
    const i64 now = static_cast<i64>(gba->clock.master_cycle_count);
    i64 frame_start = static_cast<i64>(gba->clock.ppu.scanline_start) -
                      (static_cast<i64>(gba->clock.ppu.y) * MASTER_CYCLES_PER_SCANLINE);

    if (frame_start > now) frame_start = now;

    for (u32 line = gba->clock.ppu.y; line < 228; line++) {
        const i64 line_start = frame_start + static_cast<i64>(line) * MASTER_CYCLES_PER_SCANLINE;
        const i64 hblank_at = line_start + MASTER_CYCLES_BEFORE_HBLANK;
        const i64 line_end = line_start + MASTER_CYCLES_PER_SCANLINE;

        if (line == 160) {
            if ((line > gba->clock.ppu.y) || !gba->clock.ppu.vblank_active) {
                gba->scheduler.only_add_abs(line_start, 1, gba, &vblank, nullptr);
            }
        }
        if (line == 227) {
            if ((line > gba->clock.ppu.y) || gba->clock.ppu.vblank_active) {
                gba->scheduler.only_add_abs(line_start, 0, gba, &vblank, nullptr);
            }
        }

        if ((line > gba->clock.ppu.y) || (!gba->clock.ppu.hblank_active && (hblank_at >= now))) {
            gba->scheduler.only_add_abs(hblank_at, 1, this, &hblank<false>, &hblank<true>, nullptr);
        }
        if (line_end >= now) {
            gba->scheduler.only_add_abs_w_tag(line_end, 0, this, &hblank<false>, &hblank<true>, nullptr, 1);
        }
    }

    gba->scheduler.only_add_abs_w_tag(frame_start + MASTER_CYCLES_PER_FRAME, 0, this, &schedule_frame, nullptr, 2);
}

template<u8 sz, bool do_debug>
u32 core::read_invalid(u32 addr, u8 access)
{
    printf("\nREAD UNKNOWN PPU ADDR:%08x", addr);
    return gba->open_bus<sz>(addr);;
}

template<u8 sz, bool do_debug>
void core::write_invalid(u32 addr, u8 access, u32 val)
{
    printf("\nWRITE UNKNOWN PPU ADDR:%08x sz:%d DATA:%08x", addr, sz, val);
}

template <u8 sz, bool do_debug, bool peek>
u32 core::mainbus_read_palette(GBA::core *gba, u32 addr, u8 access)
{
    auto *th = &gba->ppu;
    addr &= 0x3FF;

    if constexpr (sz == 4) {
        if constexpr (!peek) gba->waitstates.current_transaction += 2;
        return reinterpret_cast<u32 *>(th->palette_RAM)[addr >> 2];
    }

    if constexpr (!peek) gba->waitstates.current_transaction++;
    if constexpr (sz == 1) return reinterpret_cast<u8 *>(th->palette_RAM)[addr];
    if constexpr (sz == 2) return th->palette_RAM[addr >> 1];
    NOGOHERE;
}

template<u8 sz, bool do_debug, bool peek>
u32 core::mainbus_read_VRAM(GBA::core *gba, u32 addr, u8 access)
{
    auto *th = &gba->ppu;
    addr &= 0x1'FFFF;
    ...
    if constexpr (sz == 4) {
        if constexpr (!peek) gba->waitstates.current_transaction += 2;
        return reinterpret_cast<u32 *>(th->VRAM)[addr >> 2];
    }
    if constexpr(!peek) gba->waitstates.current_transaction++;
    if constexpr (sz == 1) return th->VRAM[addr];
    if constexpr (sz == 2) return reinterpret_cast<u16 *>(th->VRAM)[addr >> 1];
    NOGOHERE;
}

template<u8 sz, bool do_debug, bool peek>
u32 core::mainbus_read_OAM(GBA::core *gba, u32 addr, u8 access) {
    auto *th = &gba->ppu;
    addr &= 0x3FF;
    if constexpr(!peek) gba->waitstates.current_transaction++;
    if constexpr(sz == 1) return th->OAM[addr];
    if constexpr(sz == 2) return reinterpret_cast<u16 *>(th->OAM)[addr >> 1];
    if constexpr(sz == 4) return reinterpret_cast<u32 *>(th->OAM)[addr >> 2];
    NOGOHERE;
}

template<u8 sz, bool do_debug>
void core::mainbus_write_palette(GBA::core *gba, u32 addr, u8 access, u32 val) {
    auto *th = &gba->ppu;
    addr &= 0x3FF;
    if constexpr (sz == 4) {
        gba->waitstates.current_transaction += 2;
        reinterpret_cast<u32 *>(th->palette_RAM)[addr >> 2] = val;
        return;
    }
    gba->waitstates.current_transaction++;
    if constexpr(sz == 1) {
        val = (val & 0xFF) | ((val << 8) & 0xFF00);
    }
    reinterpret_cast<u16 *>(th->palette_RAM)[addr >> 1] = val;
}

template<u8 sz, bool do_debug>
void core::mainbus_write_VRAM(GBA::core *gba, u32 addr, u8 access, u32 val)
{
    auto *th = &gba->ppu;
    if constexpr(do_debug) {
    ...
    if (addr >= vram_boundary) {
        if constexpr (sz != 1) {
            ...
            if constexpr(sz == 2) reinterpret_cast<u16 *>(th->VRAM)[addr >> 1] = val;
            if constexpr(sz == 4) reinterpret_cast<u32 *>(th->VRAM)[addr >> 2] = val;
        }
        return;
    }
    if constexpr(sz == 4) {
        reinterpret_cast<u32 *>(th->VRAM)[addr >> 2] = val;
        return;
    }
    if constexpr (sz == 1) {
        addr &= 0x1FFFE;
    ...
    reinterpret_cast<u16 *>(th->VRAM)[addr >> 1] = val;
}

template<u8 sz, bool do_debug>
void core::mainbus_write_OAM(GBA::core *gba, u32 addr, u8 access, u32 val)
{
    gba->waitstates.current_transaction++;
    if constexpr (sz == 1) return;
    auto *th = &gba->ppu;
    addr &= 0x3FF;
    if constexpr(sz == 2) reinterpret_cast<u16 *>(th->OAM)[addr >> 1] = val;
    if constexpr(sz == 4) reinterpret_cast<u32 *>(th->OAM)[addr >> 2] = val;
}
...
template u32 core::mainbus_read_IO<true>(GBA::core *gba, u32 addr, u8 access);
template u32 core::mainbus_read_IO<false>(GBA::core *gba, u32 addr, u8 access);
template<bool do_debug>
u32 core::mainbus_read_IO(GBA::core *gba, u32 addr, u8 access)
{
    auto *th = &gba->ppu;
    u32 v = 0;
    switch(addr) {
        ...
        case MOSAIC:
        case MOSAIC+1:
            return gba->open_bus<1>(addr);
        default: break;
    }
    return th->read_invalid<1, do_debug>(addr, access);
}
...
template void core::mainbus_write_IO<false>(GBA::core *gba, u32 addr, u8 access, u32 val);
template void core::mainbus_write_IO<true>(GBA::core *gba, u32 addr, u8 access, u32 val);

template<bool do_debug>
void core::mainbus_write_IO(GBA::core *gba, u32 addr, u8 access, u32 val) {
    auto *th = &gba->ppu;
    if constexpr(do_debug) {
...
#undef BG3
    ...
    th->write_invalid<1, do_debug>(addr, access, val);
}

}

#undef INSTANTIATE_READ_FUNC
