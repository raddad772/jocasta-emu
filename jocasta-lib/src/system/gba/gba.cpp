...
u32 core::finish_frame()
{
    ...
    double master_div = static_cast<double>(MASTER_CYCLES_PER_SECOND) / static_cast<double>(master_num_cycles);
    double arm_div = static_cast<double>(MASTER_CYCLES_PER_SECOND) / static_cast<double>(arm_num_cycles);
    double tm0_div = static_cast<double>(MASTER_CYCLES_PER_SECOND) / static_cast<double>(tm0_num_cycles);
    double arm_spd = (arm_div) * 100.0;
    double tm0_spd = (static_cast<double>(timer[0].reload_ticks) / tm0_div) * 100.0;
...
Host  sp_svc    sp_irq    sp_svc    zerofilled area       return address
  GBA   3007FE0h  3007FA0h  3007F00h  [3007E00h..3007FFFh]  Flag[3007FFAh] */
    ...
    // NOTE: the known-good C version (forcebk) did NOT set WAITCNT or enable the
    // prefetch buffer here -- it left them at reset defaults and let the game
    // configure them via normal register writes. Hand-poking them here (esp. the
    // prefetch enable) broke Doom; matching the C behavior fixes it.
}
...
void core::set_audio_ring(audio_output_ring *ring)
{
    ...
    audio.master_cycles_per_audio_sample = static_cast<float>(MASTER_CYCLES_PER_SECOND) / 262144.0f;
}


void core::schedule_audio_events()
{
...
void core::reset()
{
    ...
    // Fast boot removed for GBA: skip_BIOS() leaves the ARM32 core in a state that
    // breaks some games (e.g. Doom). Always boot the real BIOS (gba_bios.bin).
...
void core::setup_lcd(JSM_DISPLAY &d)
{
    ...
    // GBA crystal = 2^24 Hz = 16,777,216 Hz; 228 lines x 1232 cycles/line ... ~59.7275 Hz
