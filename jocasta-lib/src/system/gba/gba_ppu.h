...
struct core {
    ...
    template<bool do_debug> void do_schedule_frame(i64 cur_clock);
    void reschedule_from_state();

private:
    static void vblank(void *ptr, u64 val, u64 clock, u32 jitter);
    template<bool do_debug> static void hblank(void *ptr, u64 key, u64 clock, u32 jitter);

    template<bool do_debug> void do_hblank(bool in_hblank);
    ...
    template<bool do_debug> void calculate_windows(bool in_vblank);
    PPU::WINDOW *get_active_window(u32 x);
    void affine_line_start(BG &bg, i32 *fx, i32 *fy) const;
    void affine_line_end(BG &bg) const;
    template<bool do_debug> void draw_bg_line_affine(u32 bgnum);
    template<bool do_debug> void draw_bg_line_normal(u32 bgnum);
    void output_pixel(u32 x, bool obj_enable, bool bg_enables[4], u16 *line_output);
    template<bool do_debug> void draw_line0(u16 *line_output);
    template<bool do_debug> void draw_line1(u16 *line_output);
    template<bool do_debug> void draw_line2(u16 *line_output);
    void draw_bg_line3();
    void draw_bg_line4();
    void draw_bg_line5();
    template<bool do_debug> void draw_line3(u16 *line_output);
    template<bool do_debug> void draw_line4(u16 *line_output);
    template<bool do_debug> void draw_line5(u16 *line_output);
    template <u8 sz, bool do_debug> u32 read_invalid(u32 addr, u8 access);
    template <u8 sz, bool do_debug> void write_invalid(u32 addr, u8 access, u32 val);
...
public:
    template <u8 sz, bool do_debug, bool peek> static u32 mainbus_read_palette(GBA::core *gba, u32 addr, u8 access);
    template <u8 sz, bool do_debug, bool peek> static u32 mainbus_read_VRAM(GBA::core *gba, u32 addr, u8 access);
    template <u8 sz, bool do_debug, bool peek> static u32 mainbus_read_OAM(GBA::core *gba, u32 addr, u8 access);
    template <u8 sz, bool do_debug> static void mainbus_write_palette(GBA::core *gba, u32 addr, u8 access, u32 val);
    template <u8 sz, bool do_debug> static void mainbus_write_VRAM(GBA::core *gba, u32 addr, u8 access, u32 val);
    template <u8 sz, bool do_debug> static void mainbus_write_OAM(GBA::core *gba, u32 addr, u8 access, u32 val);
    // Only 8-bit rIO access
    template <bool do_debug> static u32 mainbus_read_IO(GBA::core *gba, u32 addr, u8 access);
    template <bool do_debug> static void mainbus_write_IO(GBA::core *gba, u32 addr, u8 access, u32 val);
