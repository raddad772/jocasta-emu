...
static void print_layer_info(core *th, u32 bgnum, debugger_widget_textbox *tb)
{
    if (bgnum < 4) {
        PPU::BG *bg = &th->ppu.pbg[bgnum];
...
static void setup_dbglog(core &th, debugger_interface &dbgr)
{
    ...
    ppu.children.reserve(15);
    ppu.add_node(dv, "VRAM Write", "PPU", SNES_CAT_PPU_VRAM_WRITE, ppu_color);
}

static void setup_image_view_ppu_obj_tiles(core &th, debugger_interface &dbgr) {
...
static void setup_image_view_ppu_tilemaps(core &th, debugger_interface &dbgr)
{
    th.dbg.image_views.ppu_layers = dbgr.make_view(dview_image);
    debugger_view *dview = &th.dbg.image_views.ppu_layers.get();
    image_view *iv = &dview->image;
...
static void readcpumem(void *ptr, u32 addr, void *dest)
{
    ...
    auto *snes = static_cast<core *>(ptr);
    for (u32 i = 0; i < 16; i++) {
