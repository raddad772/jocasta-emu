...
core::core() : r5a22(this, &this->clock.master_cycle_count), apu(this, &clock.master_cycle_count), scheduler(&clock.master_cycle_count), cart(this), ppu(this), mem(this) {
...
u32 read_trace_wdc65816(void *ptr, u32 addr) {
    return static_cast<core *>(ptr)->mem.read_bus_A(addr, 0, 0);
}
...
jsm_system *SNES_new()
{
    ...
    th->scheduler.run.func[0] = &SNES::run_block<false>;
    th->scheduler.run.func[1] = &SNES::run_block<true>;
    th->scheduler.run.ptr = th;

    th->apu.dsp.sample.func = &SNES::sample_audio;
    th->apu.dsp.sample.ptr = th;

    snprintf(th->label, sizeof(th->label), "SNES");

    jsm_debug_read_trace dt;
    dt.read_trace = &SNES::read_trace_wdc65816;
...
void core::schedule_audio_events()
{
    ...
    //scheduler_only_add_abs(static_cast<i64>(audio.next_sample_cycle), 0, th, &sample_audio, nullptr);
}
...
{
    auto *snes = dynamic_cast<core *>(jsm);
...
static void setup_crt(JSM_DISPLAY *d)
{
    ...
    // NTSC crystal 21.477270 MHz, 1364 master cycles/line x 262 lines ... ~60.0985 Hz
...
u32 core::finish_frame()
{
    ...
    double spc_div = static_cast<double>(clock.timing.frame.master_cycles) / static_cast<double>(spc_num_cycles);
    double wdc_div = static_cast<double>(clock.timing.frame.master_cycles) / static_cast<double>(wdc_num_cycles);
