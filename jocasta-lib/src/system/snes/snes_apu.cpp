...
// BRR blocks are 9 bytes.  If next_read_addr is within 8 bytes of the top of
// the 64 KB APU address space the direct-pointer path would overflow; use a
// bounce buffer with address wrapping instead.
static void BRR_decode_at(const u8 *ram, u32 addr, sample &dest, FILTER &filter)
{
    addr &= 0xFFFF;
    if (addr + 9 <= 0x10000) {
        BRR_decode(ram + addr, dest, filter);
    } else {
        u8 tmp[9];
        for (int i = 0; i < 9; i++) tmp[i] = ram[(addr + i) & 0xFFFF];
        BRR_decode(tmp, dest, filter);
    }
}
...
static void BRR_decode(const u8 *buf, sample &dest, FILTER &filter)
{
    ...
    for (u32 i = 0; i < 8; i++) {
        ...
        for (u32 num = 0; num < 2; num++) {
            i32 nibble = static_cast<i32>((data & 0xF0) >> 4);
            data <<= 4;
            nibble = sign_extend<4>(nibble);
            if (scale <= 12) {
...
void ch::write_voice(u8 param, u8 val)
{
    switch(param) {
        case 0:
            io.VOLL = sign_extend<8>(val);
            return;
        case 1:
            io.VOLR = sign_extend<8>(val);
            return;
        case 2: {
...
void ch::keyon()
{
    ...
    BRR_decode_at(apu->cpu.RAM, sample_data.next_read_addr, sample_data, filter);
...
static void writeDSP(void *ptr, u16 addr, u8 val)
{
    ...
    switch (addr) {
        case 0x0c:
            th->io.MVOLL = sign_extend<8>(val);
            return;
        case 0x1C:
            th->io.MVOLR = sign_extend<8>(val);
            return;
        case 0x2C:
            th->io.EVOLL = val;
            return;
...
void core::reset()
{
    cpu.reset();
    dsp.noise.sch_id = 0;
    dsp.noise.sch_still = 0;
    dsp.noise.next_update = 0;
    dsp.scheduling.kon_clear.sch_id = 0;
    dsp.scheduling.kon_clear.still_sched = 0;
    dsp.scheduling.koff_clear.sch_id = 0;
    dsp.scheduling.koff_clear.still_sched = 0;
    for (auto & ch : dsp.channel) {
        ch.pitch.sch_id = 0;
        ch.pitch.sch_still = 0;
        ch.env.sch_id = 0;
        ch.env.sch_still = 0;
    }
}
...
void ch::update_sample()
{
    ...
    if (num > 0 && (apu->dsp.io.PMON & (1 << num))) {
        step = static_cast<u32>((static_cast<i64>(step) * (apu->dsp.channel[num - 1].output.debug + 0x8000)) >> 15);
        step &= 0x3FFF;
    }
    if (ended) return;
    pitch.counter += step;
...
{
    const auto now = static_cast<long double>(snes->clock.master_cycle_count);
    snes->clock.apu.cycle.next = now + snes->clock.apu.cycle.stride;
    snes->clock.apu.sample.next = now + snes->clock.apu.sample.stride;
