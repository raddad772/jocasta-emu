...
void core::reset()
{
    ...
    mode7.b = mode7.c = 0;
    snes->clock.ppu.y = -1;
    snes->clock.ppu.scanline_start = 0;
    snes->clock.ppu.hblank_active = 0;
    snes->clock.ppu.vblank_active = 0;
    hirq.sched_id = 0;
    hirq.still_sched = 0;
    if (display) {
        display->active_draw_buffer = 0;
        display->scan_x = 0;
        display->scan_y = 0;
        cur_output = static_cast<u16 *>(display->output[0]);
        memset(display->output[0], 0, 512 * 224 * 2);
        memset(display->output[1], 0, 512 * 224 * 2);
    }
}
...
void core::write(u32 addr, u32 val, memmap_block *bl) {
    ...
    switch (addr) {
        ...
        case 0x210D: // BG1HOFS
            ...
            latch.mode7 = val;
            mode7.rhoffset = sign_extend<13>(mode7.hoffset);
            return;
        case 0x210E: // BG1VOFS
            ...
            latch.mode7 = val;
            mode7.rvoffset = sign_extend<13>(mode7.voffset);
            return;
        case 0x210F: // BG2HOFS
        ...
        case 0x211F: // M7E
            ...
            mode7.rx = sign_extend<13>(mode7.x);
            latch.mode7 = val;
            return;
        case 0x2120: // M7F
            mode7.y = (val << 8) | latch.mode7;
            mode7.ry = sign_extend<13>(mode7.y);
            latch.mode7 = val;
            return;
...
static inline u32 window_test(u32 one_enable, u32 one, u32 two_enable, u32 two, u32 mask)
{
    if (!one_enable) return two && two_enable;
    if (!two_enable) return one;
    if (mask == 0) return one | two;
    if (mask == 1) return one & two;
    return (one ^ two) == (3 - mask);
}

static inline u32 window_color_enable(u32 mask, u32 value)
{
    if (mask == 0) return 1;
    if (mask == 1) return value;
    if (mask == 2) return !value;
    return 0;
}
...
void core::draw_line()
{
    ...
    u32 ppu_y = snes->clock.ppu.y;
    if ((ppu_y == 0) || (ppu_y > 224)) return;

    u32 screen_y = ppu_y - 1;
    if (screen_y == 0) memset(&snes->dbg_info.line, 0, sizeof(snes->dbg_info.line));
    draw_sprite_line(ppu_y);
    draw_bg_line(0, ppu_y);
    draw_bg_line(1, ppu_y);
    draw_bg_line(2, ppu_y);
    draw_bg_line(3, ppu_y);
    for (u32 i = 0; i < 4; i++) {
        memcpy(snes->dbg_info.line[screen_y].bg[i].px, pbg[i].line, sizeof(pbg[i].line));
    }
    memcpy(snes->dbg_info.line[screen_y].sprite_px, obj.line, sizeof(obj.line));
    assert(screen_y >= 0 && screen_y < 224);
    u16 *line_output = cur_output + (screen_y * 512);
    for (u32 x = 0; x < 256; x++) {
        u32 win1 = (x >= window[0].left) && (x <= window[0].right);
        u32 win2 = (x >= window[1].left) && (x <= window[1].right);
        u32 color_window = window_test(color_math.window.enable[0], win1 ^ color_math.window.invert[0],
                                       color_math.window.enable[1], win2 ^ color_math.window.invert[1],
                                       color_math.window.mask);
        u32 main_color_enable = window_color_enable(color_math.window.main_mask, color_window);
        u32 sub_color_enable = window_color_enable(color_math.window.sub_mask, color_window);
...
#define SUBPRIO 1
        for (u32 bgnum = 0; bgnum < 4; bgnum++) {
            BG *bg = &pbg[bgnum];
            PX *cmp = &bg->line[x];
            u32 bg_window = window_test(bg->window.enable[0], win1 ^ bg->window.invert[0],
                                        bg->window.enable[1], win2 ^ bg->window.invert[1],
                                        bg->window.mask);
            u32 bg_main_enable = bg->main_enable && !(bg->window.main_enable && bg_window);
            u32 bg_sub_enable = bg->sub_enable && !(bg->window.sub_enable && bg_window);
            if (bg_main_enable && cmp->has && (main_px.priority > cmp->priority)) {
            ...
            if (color_math.blend_mode && bg_sub_enable && cmp->has && (sub_px.priority > cmp->priority)) {
        ...
        PX *cmp = &obj.line[x];
        u32 obj_window = window_test(obj.window.enable[0], win1 ^ obj.window.invert[0],
                                     obj.window.enable[1], win2 ^ obj.window.invert[1],
                                     obj.window.mask);
        u32 obj_main_enable = obj.main_enable && !(obj.window.main_enable && obj_window);
        u32 obj_sub_enable = obj.sub_enable && !(obj.window.sub_enable && obj_window);
        if (obj_main_enable && cmp->has && (main_px.priority > cmp->priority)) {
            main_px.color = cmp->color;
            main_px.has = 1;
            main_px.priority = cmp->priority;
            main_px.source = cmp->palette < 4 ? SRC_OBJ_PAL03 : SRC_OBJ_PAL47;
        }
        if (color_math.blend_mode && obj_sub_enable && cmp->has && (sub_px.priority > cmp->priority)) {
            sub_px.color = cmp->color;
            sub_px.has = 1;
            sub_px.priority = cmp->priority;
        ...
        if (main_color_enable && sub_color_enable && color_math.enable[main_px.source]) do_color_math(&main_px, &sub_px);

        u16 c = (!io.force_blank && main_color_enable) ? main_px.color : 0;
...
void hblank(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    if (key == 0) {
        // Draw bg
        // Output bg &  sprite
        if ((snes->clock.ppu.y > 0) && (snes->clock.ppu.y <= 224))
            snes->ppu.draw_line();
    }
...
void hdma_setup(void *ptr, u64 key, u64 clock, u32 jitter)
{
    auto *snes = static_cast<SNES::core *>(ptr);
    for (u32 n = 0; n < 8; n++) {
        snes->r5a22.dma.channels[n].hdma_reset();
    }
    if (!snes->r5a22.hdma_is_enabled()) return;
...
void hdma(void *ptr, u64 key, u64 clock, u32 jitter)
{
    auto *snes = static_cast<SNES::core *>(ptr);
    for (u32 n = 0; n < 8; n++) {
        if (snes->r5a22.dma.channels[n].hdma_is_active()) {
            snes->r5a22.status.hdma_pending = 1;
...
static void schedule_scanline(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    if (snes->r5a22.io.irq_enable) {
        if (!snes->r5a22.io.hirq_enable) assert_hirq(snes, 1, cur, 0);
            ...
            // io.htime is already in master clocks; this is the IRQ unit's counter delay.
            snes->ppu.hirq.sched_id = snes->scheduler.only_add_abs(cur + snes->r5a22.io.htime + 10, 1, snes, &assert_hirq, &snes->ppu.hirq.still_sched);
    }
    snes->r5a22.update_irq();
...
void new_frame(void* ptr, u64 key, u64 cur_clock, u32 jitter)
{
    ...
    snes->clock.ppu.vblank_active = 0;
    snes->ppu.display->active_draw_buffer ^= 1;
    snes->ppu.cur_output = static_cast<u16 *>(snes->ppu.display->output[snes->ppu.display->active_draw_buffer]);
    memset(snes->ppu.cur_output, 0, 512*224*2);
...
{
    const u64 now = snes->clock.master_cycle_count;
    schedule_scanline(snes, 0, now, 0);
    snes->scheduler.only_add_abs_w_tag(now + snes->clock.timing.frame.master_cycles, 0, snes, &new_frame, nullptr, 2);
}
}
