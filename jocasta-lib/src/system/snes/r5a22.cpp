...
void core::update_irq() {
    ...
    u32 old_irq_status;
    if (io.hirq_enable && io.virq_enable) old_irq_status = status.hirq_status && status.virq_status;
    else old_irq_status = status.hirq_status || status.virq_status;
    ...
    if (!old_irq_status && status.irq_line) status.irq_flag = 1;
...
void DMA_CHANNEL::hdma_transfer()
{
    ...
    for (i32 idx = 0; idx < HDMA_LENGTHS[transfer_mode]; idx++) {
        ...
        if (indirect) {
            ...
            addr = (source_bank << 16) | hdma_address;
            hdma_address = (hdma_address + 1) & 0xFFFF;
        }
...
u32 DMA_CHANNEL::hdma_advance()
{
    if (!(hdma_enable && !hdma_completed)) return 0;
    line_counter--;
...
u32 core::dma_run()
{
    ...
    if (status.hdma_pending) {
        status.hdma_pending = 0;
        snes->clock.master_cycle_count += 8;
        for (auto & ch : dma.channels) {
            if (dbg.do_break) return any_going;
            if (ch.hdma_enable && !ch.hdma_completed) ch.hdma_transfer();
        }
        for (auto & ch : dma.channels) {
            if (dbg.do_break) return any_going;
            if (ch.hdma_enable && !ch.hdma_completed) snes->clock.master_cycle_count += ch.hdma_advance();
        }
        ...
        status.dma_running = 0;
        if (any_going) status.dma_running = 1;
        return 0;
    }
