...
void SMSGG::core::set_audio_ring(audio_output_ring *ring)
{
    audio.output_ring = ring;
    // SN76489 clock divisor: one PSG tick per apu_divisor master cycles.
    // Push one stereo pair each PSG tick so the declared sample_rate is honoured.
    audio.master_cycles_per_audio_sample = static_cast<double>(clock.apu_divisor);
}
...
static void setup_audio(std::vector<physical_io_device> &inIOs)
{
    ...
    // PSG clock = 10.738635 MHz crystal / 48 = 223,721 Hz
    chan->sample_rate = (u32)(10738635.0 / 48.0);
...
void SMSGG::core::describe_io()
{
    ...
    io.controllerA.variant = variant;
    ...
    d->display.allocate_output(0, SMSGG_DISPLAY_DRAW_SZ);
    d->display.allocate_output(1, SMSGG_DISPLAY_DRAW_SZ);
...
u32 SMSGG::core::finish_frame()
{
    ...
    while(current_frame == clock.frames_since_restart) {
        scanlines_done++;
        finish_scanline();
        if (::dbg.do_break) return vdp.display->active_draw_buffer;
    }
...
{
    if (variant == jsm::systems::GG || !io.pause_button) return;

    u32 down = io.pause_button->state != 0;
    if (!io.pause_button_down && down)
        cpu.request_NMI();
    io.pause_button_down = down;
}

void SMSGG::core::sample_audio()
{
    if (audio.output_ring && (clock.master_cycles >= static_cast<u64>(audio.next_sample_cycle))) {
        audio.next_sample_cycle += audio.master_cycles_per_audio_sample;
        i16 s = sn76489.mix_sample(false);
        float fs = i16_to_float(s);
        audio.output_ring->push(fs, fs);
    }
}
