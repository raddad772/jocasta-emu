...
{
    crt.y = 0;
    crt.display->active_draw_buffer ^= 1;
    crt.cur_output = static_cast<u8 *>(crt.display->output[crt.display->active_draw_buffer]);
    clock.master_frame++;
}
...
/*            printif(z80.mem, DBGC_Z80 "\nGAL(%06llu)r   %04x   $%02x         TCU:%d" DBGC_RST,
    ...
    if (z80.pins.WR) {
        if (z80.pins.MRQ) {// write ROM/RAM
            if (::dbg.do_debug && (z80.trace.last_cycle != *z80.trace.cycles)) {
