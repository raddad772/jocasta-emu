...
void core::set_audio_ring(audio_output_ring *ring)
{
    ...
    if (audio.master_cycles_per_audio_sample == 0) {
        // 1 master cycle per sample at MASTER_CYCLES_PER_SECOND native rate
...
u32 core::finish_frame()
{
    ...
    return crt.display->active_draw_buffer;
}
...
static void setup_crt(JSM_DISPLAY *d)
{
    ...
    d->fps = 50;
    // removed: d->fps_override_hint = 50;
...
void core::describe_io()
{
    ...
    d->display.allocate_output(0, 384 * 320);
    d->display.allocate_output(1, 384 * 320);
