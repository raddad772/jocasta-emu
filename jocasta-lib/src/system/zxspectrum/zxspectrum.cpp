...
void core::CPU_writemem(u16 addr, u8 val)
{
    ...
    bank.RAM[(addr >> 14)][addr & 0x3FFF] = static_cast<u8>(val);
}
...
static void setup_crt48(JSM_DISPLAY *d)
{
    ...
    // ZX48 ULA clock 7 MHz, 448 clocks/line x 312 lines ... ~50.08 Hz
    d->fps = 7000000.0 / (448.0 * 312.0);

    // total PAL pixels is 448x312x50.08fps for ZX48
...
void setup_crt128(JSM_DISPLAY *d)
{
    ...
    // ZX128 Z80 clock 3.5469 MHz (17.7345 MHz/5), 228 T-states/line x 311 lines ... ~50.02 Hz
    d->fps = (17734500.0 / 5.0) / (228.0 * 311.0);

    // total PAL pixels is 456x311x50.02fps for ZX128
    // so. 352x304 "display" area, = 8-row blank at end
    // 256x192 are the standard draw area
    // 312 lines
 ...
 void core::describe_io()
{
    ...
    d->display.allocate_output(0, 352 * 304);
    d->display.allocate_output(1, 352 * 304);
...
void core::get_framevars(framevars& out)
{
    ...
    out.x = ula.screen_x;
    out.scanline = ula.screen_y < 0 ? 0 : static_cast<u32>(ula.screen_y);
}
...
void core::reset() {
    ...
    bank.ROM = ROM;
    if (variant == spectrum48) {
        bank.RAM[1] = RAM;
        bank.RAM[2] = RAM + 0x4000;
        bank.RAM[3] = RAM + 0x8000;
        bank.display = RAM;
    } else {
        // 128K startup layout: 0x4000=bank 5, 0x8000=bank 2, 0xC000=bank 0.
...
u32 core::finish_frame()
{
    ...
    return ula.display->active_draw_buffer;
}

u32 core::finish_scanline()
{
    i32 current_y = clock.ula_y;
...
void core::CPU_cycle()
{
    ...
    if (cpu.pins.WR) {
        if (cpu.pins.MRQ) {// write ROM/RAM
            if (dbg.do_debug && (cpu.trace.last_cycle != *cpu.trace.cycles)) {
...
{
    BUF* b = &mfs.files[0].buf;
    memcpy(ROM, b->ptr, ROM_size <= b->size ? ROM_size : b->size);
}
