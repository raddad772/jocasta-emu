...
struct core : jsm_system {
    ...
    u32 PAL{};
    gensched_item scheduler_lookup[NUM_GENSCHED * 2]{}; // 2 for debug false/true
    gensched_item scheduler_lookup_debug[NUM_GENSCHED * 2]{}; // 2 for debug false/true
    u16 scheduler_index{};
    ...
    struct {
        ...
        controller_port controller_port_ext{};
        bool SRAM_enabled{};
    } io{};
   ...
   DBG_END
    ...
    struct {
        ...
        // Separate ring buffers for each synthesis chip, populated via audio_rings_ready().
        audio_output_ring *ym_ring{};   // YM2612
        audio_output_ring *psg_ring{};  // SN76489
        u64 cycles{};
        // One-pole IIR low-pass for SN76489 PSG output (~2840 Hz @ ~53 kHz,
        // matching the MD PSG stream filter in Ares).
        float psg_lpf_state{0.0f};
    } audio{};
    ...
    template<bool do_debug> u16 mainbus_read(u32 addr, u8 UDS, u8 LDS, u16 old, bool has_effect);
    template<bool do_debug> void mainbus_write(u32 addr, u8 UDS, u8 LDS, u16 val);
    void z80_interrupt(bool level);
    template<bool do_debug> u8 z80_bus_read(u16 addr, u8 old, bool has_effect);
    u16 read_version_register(u32 mask) const;
    template<bool do_debug> void update_irqs();
...
private:
    ...
    template<bool do_debug> u8 z80_mainbus_read(u32 addr, u8 old, bool has_effect);
    u8 z80_ym2612_read(u32 addr, u8 old, bool has_effect);
    void write_z80_bank_address_register(u32 val);
    template<bool do_debug> void z80_bus_write(u16 addr, u8 val, bool is_m68k);
    SYMDO *get_at_addr(u32 addr);
    void read_opts();
    void populate_opts();
    template<bool do_debug> static void block_step(void *ptr, u64 key, u64 clock, u32 jitter);
    void load_symbols();
    void schedule_first();
