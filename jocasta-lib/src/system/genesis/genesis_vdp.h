...
struct core {
    ...
    template<bool do_debug> void mainbus_write(u32 addr, u16 val, u16 mask);
    template<bool do_debug> void do_cycle();
    u8 z80_read(u32 addr, u8 old, bool has_effect);
    template<bool do_debug>void z80_write(u32 addr, u8 val);
    void reset();
    template<bool do_debug> void vblank(bool new_value);
    void schedule_first();
    ...
    template<bool do_debug> void new_scanline(u64 cur_clock);
    static void gen_z80_interrupt_off(void *ptr, u64 key, u64 clock, u32 jitter);
private:
    [[nodiscard]] bool fifo_empty() const;
    [[nodiscard]] bool fifo_overfull() const;
    [[nodiscard]] bool fifo_full() const;
    void recalc_window();
    template<bool do_debug> void write_vdp_reg(u16 rn, u16 val);
    void print_scroll_info();
    template<bool do_debug> void dma_run_if_ready();
    template<bool do_debug> void write_data_port(u16 val, bool is_cpu);
    void render_sprites();
    void get_line_hscroll(u32 line_num, u32 *planes);
    void get_vscrolls(int column, u32 *planes);
    [[nodiscard]] u16 VRAM_read(u32 addr) const {
    ...
    [[nodiscard]] u16 VRAM_read_byte(u16 addr) const;
    template<bool do_debug> void VRAM_write_byte(u16 addr, u16 val);
    [[nodiscard]] u16 VSRAM_read(u16 addr) const;
    template<bool do_debug> void VSRAM_write(u16 addr, u16 val);
    template<bool do_debug> void VSRAM_write_byte(u16 addr, u16 val);
    [[nodiscard]] u16 CRAM_read(u16 addr) const;
    template<bool do_debug> void CRAM_write(u16 addr, u16 val);
    template<bool do_debug> void CRAM_write_byte(u16 addr, u16 val);
    [[nodiscard]] u16 read_counter() const;
    void init_slot_tables();
    template<bool do_debug> static void handle_iack(void* ptr);
    void set_clock_divisor() const;
    void latch_hcounter();
    template<bool do_debug> void do_vcounter();
    void hblank(u32 new_value);
    void set_sc_array();
    void dma_source_inc();
    template<bool do_debug> void dma_load();
    template<bool do_debug> void dma_fill();
    template<bool do_debug> void dma_copy();
    template<bool do_debug> void write_control_port(u16 val, u16 mask);
    ...
    template<bool do_debug> void output_16();
    template<bool do_debug> void render_left_column();
    template<bool do_debug> void render_16_more();
...
extern template void core::do_cycle<true>();
extern template void core::do_cycle<false>();
