...
#define S(x) Sadd(state, &( x), sizeof( x))
    ...
    S(io.controller_port1.data_lines);
    S(io.controller_port1.control);
    S(io.controller_port2.data_lines);
    S(io.controller_port2.control);
    S(io.controller_port_ext.data_lines);
    S(io.controller_port_ext.control);
...
#define S(x) Sadd(state, &cart. x, sizeof(cart. x))
    ...
    S(header);
    S(kind);
    // SRAM is bundled separately in the .jsst zip; do not serialize here.
#undef S
}
...
#define L(x) Sload(state, & x, sizeof( x))
    ...
    L(io.controller_port1.data_lines);
    L(io.controller_port1.control);
    L(io.controller_port2.data_lines);
    L(io.controller_port2.control);
    L(io.controller_port_ext.data_lines);
    L(io.controller_port_ext.control);
