...
u32 read_trace_z80(void *ptr, u32 addr) {
    ...
    return th->z80_bus_read<false>(addr, th->z80.pins.D, false);
}

u32 read_trace_m68k(void *ptr, u32 addr, u32 UDS, u32 LDS) {
    auto *th = static_cast<core *>(ptr);
    return th->mainbus_read<false>(addr, UDS, LDS, th->io.m68k.open_bus_data, false);
}

void core::audio_rings_ready()
{
    // Scan IOs for our two HID_AUDIO_CHANNEL entries (YM2612 first, PSG second)
    // and grab the ring pointers that the GUI just populated.
    u32 found = 0;
    for (auto &pio : IOs) {
        if (pio.kind != HID_AUDIO_CHANNEL) continue;
        JSM_AUDIO_CHANNEL &chan = pio.audio_channel;
        if (found == 0) {
            audio.ym_ring = chan.ring;
            // YM2612 clock divisor is fixed at init time — safe to latch here.
            audio.master_cycles_per_audio_sample =
                static_cast<double>(clock.ym2612.clock_divisor);
        } else if (found == 1) {
            audio.psg_ring = chan.ring;
        }
        if (++found == 2) break;
    }
}
...
void core::cycle_m68k()
{
    ...
    if (m68k.pins.AS && (!m68k.pins.DTACK) && (!io.m68k.stuck)) {
        if (!m68k.pins.RW) { // read
            io.m68k.open_bus_data = m68k.pins.D = mainbus_read<do_debug>(m68k.pins.Addr, m68k.pins.UDS, m68k.pins.LDS, io.m68k.open_bus_data, 1);
...
void core::create_scheduling_lookup_table()
{
    ...
    for (i32 cycles=16; cycles<24; cycles+=4) { // 4 and 5 are the two possible vdp divisors
        for (i32 tbl_entry = 0; tbl_entry < NUM_GENSCHED; tbl_entry++) {
            ...
            gensched_item *item_n = &scheduler_lookup[lookup_add + tbl_entry];
            gensched_item *item_y = &scheduler_lookup_debug[lookup_add + tbl_entry];

            // Determine which function to use
            if ((z80_cycles <= 0) && (vdp_cycles <= 0)) {
                if (z80_cycles < vdp_cycles) {
                    item_n->callback = &c_z80_vdp_m68k<false>;
                    item_y->callback = &c_z80_vdp_m68k<true>;

                    i32 num = m68k_cycles + z80_cycles;
                    item_n->clk_add_z80 = item_y->clk_add_z80 = num;

                    num = vdp_cycles - z80_cycles;
                    item_n->clk_add_vdp = item_y->clk_add_vdp = num;

                    item_n->clk_add_m68k = item_y->clk_add_m68k = m68k_cycles - (item_n->clk_add_z80 + item_n->clk_add_vdp);
                }
                else {
                    item_n->callback = &c_vdp_z80_m68k<false>;
                    item_y->callback = &c_vdp_z80_m68k<true>;
                ...
                item_n->callback = &c_z80_m68k<false>;
                item_y->callback = &c_z80_m68k<true>;

                i32 num = m68k_cycles + z80_cycles;
                item_n->clk_add_z80 = item_y->clk_add_z80 = num;

                item_n->clk_add_m68k = item_y->clk_add_m68k = m68k_cycles - item_n->clk_add_z80;

                z80_cycles += 15;
            }
            else if (vdp_cycles <= 0) { // just VDP
                item_n->callback = &c_vdp_m68k<false>;
                item_y->callback = &c_vdp_m68k<true>;

                i32 num = m68k_cycles + vdp_cycles;
                item_n->clk_add_vdp = item_y->clk_add_vdp = num;

                item_n->clk_add_m68k = item_y->clk_add_m68k = m68k_cycles - item_n->clk_add_vdp;

                vdp_cycles += cycles;
            }
            else {
                item_y->callback = &c_m68k<false>;
                item_n->callback = &c_m68k<true>;
            }
            ...
            item_y->next_index = ni;
            item_n->next_index = ni;
        }
...
void core::block_step(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    gensched_item *e;
    if constexpr(do_debug) {
        e = &th->scheduler_lookup_debug[lu + th->scheduler_index];
    }
    else {
        e = &th->scheduler_lookup[lu + th->scheduler_index];
    }
    e->callback(th, e);
    u32 ni = e->next_index;
    ...
    vdp(this)
{
    ...
    scheduler.run.func[0] = &block_step<false>;
    scheduler.run.func[1] = &block_step<true>;
...
static void run_ym2612(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    float ym_l = 0.0f, ym_r = 0.0f;
    if (th->ym2612.ext_enable) {
        ym_l = i16_to_float(static_cast<i16>(th->ym2612.mix.left_output));
        ym_r = i16_to_float(static_cast<i16>(th->ym2612.mix.right_output));
    }
    th->audio.ym_ring->push(ym_l, ym_r);
}
...
static void run_psg(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    th->psg.cycle();
    if (th->audio.psg_ring) {
        float raw = static_cast<float>(th->psg.mix_sample(false)) / 32764.0f;
        if (raw < -1.0f) raw = -1.0f;
        if (raw > 1.0f) raw = 1.0f;
        float psg = th->psg.ext_enable ? raw : 0.0f;
        th->audio.psg_ring->push(psg, psg);
    }
...
{
    auto *th = static_cast<core *>(ptr);
    debug_waveform *psg_dw = &th->dbg.waveforms_psg.main.get();
    debug_waveform *ym_dw  = &th->dbg.waveforms_ym2612.main.get();
    bool psg_full = psg_dw->user.buf_pos >= psg_dw->samples_requested;
    bool ym_full  = ym_dw->user.buf_pos  >= ym_dw->samples_requested;
    if (psg_full && ym_full) return; // all buffers full — stop
    if (!psg_full) {
        static_cast<float *>(psg_dw->buf.ptr)[psg_dw->user.buf_pos] = i16_to_float(th->psg.mix_sample(true));
        psg_dw->user.buf_pos++;
    }
    if (!ym_full) {
        static_cast<float *>(ym_dw->buf.ptr)[ym_dw->user.buf_pos] = i16_to_float(th->ym2612.mix.mono_output);
        ym_dw->user.buf_pos++;
    }
...
static void sample_audio_debug_min(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    for (int j = 0; j < 4; j++) {
        debug_waveform *dw = &th->dbg.waveforms_psg.chan[j].get();
        if (dw->user.buf_pos < dw->samples_requested) {
            static_cast<float *>(dw->buf.ptr)[dw->user.buf_pos] = i16_to_float(th->psg.sample_channel(j) * 4);
            dw->user.buf_pos++;
            any_remaining = true;
        }
    }
    for (int j = 0; j < 6; j++) {
        debug_waveform *dw = &th->dbg.waveforms_ym2612.chan[j].get();
        if (dw->user.buf_pos < dw->samples_requested) {
            dw->user.next_sample_cycle += dw->user.cycle_stride;
            static_cast<float *>(dw->buf.ptr)[dw->user.buf_pos] = i16_to_float(th->ym2612.sample_channel(j) << 2);
            dw->user.buf_pos++;
            any_remaining = true;
        }
    }
...
void core::reset_audio_schedule(u64 now)
{
    audio.cycles = 0;
...
void core::schedule_first()
{
    ...
    const u64 ym_divisor = clock.ym2612.clock_divisor > 0 ? static_cast<u64>(clock.ym2612.clock_divisor) : 1;
    const u64 psg_divisor = clock.psg.clock_divisor > 0 ? static_cast<u64>(clock.psg.clock_divisor) : 1;

    scheduler.only_add_abs(static_cast<i64>(now + ym_divisor), 0, this, &run_ym2612, nullptr);
    scheduler.only_add_abs(static_cast<i64>(now + psg_divisor), 0, this, &run_psg, nullptr);
    vdp.schedule_first();
}

void core::reschedule_from_state()
{
    scheduler_index = 0;
    scheduler.clear();

    const u64 now = clock.master_cycle_count;
    reset_audio_schedule(now);

    const u64 ym_divisor = clock.ym2612.clock_divisor > 0 ? static_cast<u64>(clock.ym2612.clock_divisor) : 1;
    const u64 psg_divisor = clock.psg.clock_divisor > 0 ? static_cast<u64>(clock.psg.clock_divisor) : 1;
    scheduler.only_add_abs(static_cast<i64>(now + ym_divisor), 0, this, &run_ym2612, nullptr);
    scheduler.only_add_abs(static_cast<i64>(now + psg_divisor), 0, this, &run_psg, nullptr);
    vdp.reschedule_from_state();
}
...
void core::setup_crt(JSM_DISPLAY &d)
{
    ...
    // Exact fps derived from crystal in genesis_clock (≈59.922 NTSC, ≈49.86 PAL)
    d.fps = clock.timing.second.frames_per;
...
{
    u32 sample_rate = static_cast<u32>(static_cast<double>(clock.timing.second.cycles_per) / static_cast<double>(clock.ym2612.clock_divisor));
    ...
    sample_rate = static_cast<u32>(static_cast<double>(clock.timing.second.cycles_per) / static_cast<double>(clock.psg.clock_divisor));
    chan->sample_rate = sample_rate;
    chan->left = chan->right = 1;
    chan->num = 2;
    chan->low_pass_filter = 3390; // PSG gets a tighter anti-alias filter
    //chan->low_pass_filter = 12000; // PSG gets a tighter anti-alias filter
    chan->mix_volume = 0.3f;   // PSG is quieter than YM on real hardware
}
...
void core::describe_io()
{
    ...
    d->display.allocate_output(0, 1280 * 480 * 2);
    d->display.allocate_output(1, 1280 * 480 * 2);
...
u32 core::finish_frame()
{
    ...
    if (::dbg.do_debug && dbg.waveforms_psg.main.vec != nullptr) {
        ...
        // PSG main waveform (shared timing with YM2612 main via sample_audio_debug_max)
        auto *psg_main = &dbg.waveforms_psg.main.get();
        psg_main->setup(clock.timing.frame.cycles_per);
        psg.ext_enable = psg_main->ch_output_enabled;
        if (psg_main->samples_requested > 0) {
            audio.master_cycles_per_max_sample =
                static_cast<double>(clock.timing.frame.cycles_per) / static_cast<double>(psg_main->samples_requested);
        ...
        // PSG channel waveforms
        for (u32 i = 0; i < 4; i++) {
            auto *cw = &dbg.waveforms_psg.chan[i].get();
            cw->setup(clock.timing.frame.cycles_per);
            if (i < 3) psg.sw[i].ext_enable  = cw->ch_output_enabled;
            else       psg.noise.ext_enable   = cw->ch_output_enabled;
        }

        // YM2612 main waveform (also sampled by sample_audio_debug_max)
        auto *ym_main = &dbg.waveforms_ym2612.main.get();
        ym_main->setup(clock.timing.frame.cycles_per);
        ym2612.ext_enable = ym_main->ch_output_enabled;

        // YM2612 channel waveforms + schedule sample_audio_debug_min
        u32 chan_samples = dbg.waveforms_ym2612.chan[0].get().samples_requested;
        if (chan_samples == 0) chan_samples = dbg.waveforms_psg.chan[0].get().samples_requested;
        if (chan_samples > 0) {
            audio.master_cycles_per_min_sample =
                static_cast<double>(clock.timing.frame.cycles_per) / static_cast<double>(chan_samples);
        ...
        for (u32 i = 0; i < 6; i++) {
            auto *cw = &dbg.waveforms_ym2612.chan[i].get();
            cw->setup(clock.timing.frame.cycles_per);
            ym2612.channel[i].ext_enable = cw->ch_output_enabled;
        }
    }
