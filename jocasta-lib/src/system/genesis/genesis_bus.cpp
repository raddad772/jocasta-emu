...
void core::mainbus_write_a1k(u32 addr, u16 val, u16 mask)
{
    switch(addr) {
        ...
        case 0xA10006: // ext port data
            io.controller_port_ext.write_data(val);
            return;
        case 0xA10008:
        ...
        case 0xA1000C: // ext port control
            io.controller_port_ext.write_control(val);
            return;
...
u16 core::mainbus_read_a1k(u32 addr, u16 old, u16 mask, bool has_effect)
{
    switch(addr) {
        ...
        case 0xA10006:
            return io.controller_port_ext.read_data();
        case 0xA10008:
            return io.controller_port1.read_control();
        case 0xA1000A:
            return io.controller_port2.read_control();
        case 0xA1000C:
            return io.controller_port_ext.read_control();
        case 0xA11100: // Z80 BUSREQ
        /*
         * TT
...
template u16 core::mainbus_read<true>(u32 addr, u8 UDS, u8 LDS, u16 old, bool has_effect);
template u16 core::mainbus_read<false>(u32 addr, u8 UDS, u8 LDS, u16 old, bool has_effect);
...
u16 core::mainbus_read(u32 addr, u8 UDS, u8 LDS, u16 old, bool has_effect)
{
    ...
    if ((addr >= 0xA0'0000) && (addr < 0xA1'0000)) {
        if (UDS) {
            v = z80_bus_read<do_debug>(addr & 0x7FFE, old >> 8, has_effect) << 8;
            if (LDS) v |= (v >> 8);
        }
        else
            v = z80_bus_read<do_debug>((addr & 0x7FFE) | 1, old & 0xFF, has_effect);
        return v;
    }
...
template void core::mainbus_write<true>(u32 addr, u8 UDS, u8 LDS, u16 val);
template void core::mainbus_write<false>(u32 addr, u8 UDS, u8 LDS, u16 val);
...
void core::mainbus_write(u32 addr, u8 UDS, u8 LDS, u16 val) {
    ...
    if ((addr >= 0xA00000) && (addr < 0xA10000)) {
        if (UDS) z80_bus_write<do_debug>(addr & 0x7FFE, val >> 8, true);
        else z80_bus_write<do_debug>((addr & 0x7FFE) | 1, val & 0xFF, true);
        return;
    }
    ...
    if ((addr >= 0xC00000) && (addr < 0xFF0000)) {
        vdp.mainbus_write<do_debug>(addr, val, mask);
        return;
    }
    if (addr >= 0xFF0000) {
        addr = (addr & 0xFFFF) >> 1;
...
u8 core::z80_mainbus_read(u32 addr, u8 old, bool has_effect)
{
    ...
    u8 data = addr & 1 ? mainbus_read<do_debug>(addr & ~1, 0, 1, 0, has_effect) & 0xFF : mainbus_read<do_debug>(addr & ~1, 1, 0, 0, has_effect) >> 8;
    // was TRUE
...
template u8 core::z80_bus_read<true>(u16 addr, u8 old, bool has_effect);
template u8 core::z80_bus_read<false>(u16 addr, u8 old, bool has_effect);
...
u8 core::z80_bus_read(u16 addr, u8 old, bool has_effect)
{
    ...
    if (addr >= 0x8000) {
        return z80_mainbus_read<do_debug>(addr, old, has_effect);
    }
...
void core::z80_bus_write(u16 addr, u8 val, bool is_m68k)
{
    ...
    if ((addr >= 0x7000) && (addr <= 0x7FFF)) {
        vdp.z80_write<do_debug>(addr, val);
        return;
    }
    if (addr >= 0x8000) {
...
template void core::update_irqs<true>();
template void core::update_irqs<false>();
...
void core::cycle_z80()
{
    ...
    if (z80.pins.RD) {
        if (z80.pins.MRQ) {
            z80.pins.D = z80_bus_read<do_debug>(z80.pins.Addr, z80.pins.D, 1);
        }
        ...
        if (z80.pins.MRQ) {
            ...
            z80_bus_write<do_debug>(z80.pins.Addr, z80.pins.D, false);
        }
    }
...
template void core::cycle_z80<true>();
template void core::cycle_z80<false>();
