...
void core::VSRAM_write(u16 addr, u16 val)
{
    ...
    if (addr < 40) VSRAM[addr] = val & 0x7FF;
}
...
void core::handle_iack(void* ptr)
{
    ...
    switch(th->bus->m68k.state.internal_interrupt_level) {
        case 4:
            if (th->irq.vblank.pending && th->irq.vblank.enable) th->irq.vblank.pending = 0;
            else th->irq.hblank.pending = 0;
            break;
        case 6:
            if (th->irq.vblank.enable) th->irq.vblank.pending = 0;
            break;
        default:
            return;
    }
    th->bus->update_irqs<do_debug>();
}
...
void core::do_vcounter()
{
    if ((bus->clock.vdp.vcount >= 0) && (bus->clock.vdp.vcount <= (1+bus->clock.vdp.bottom_rendered_line))) {
        ...
        //printf("\nLN: %d COUNTER:%d", bus->clock.vdp.vcount, irq.hblank.counter);
        if (irq.hblank.counter == 0) {
            //printf("\nHBLANK @line %d", bus->clock.vdp.vcount);
            irq.hblank.pending = 1;
            latch_hcounter();
            bus->update_irqs<do_debug>();
...
void core::new_frame(void* ptr, u64 key, u64 cur_clock, u32 jitter)
{
    ...
    if (::dbg.do_debug) debugger_report_frame(bus->dbg.interface);
    bus->clock.master_frame++;

    //printf("\nNEW FRAME clk:%lld", bus->clock.master_cycle_count);
    bus->clock.vdp.field ^= 1;
    bus->clock.vdp.vcount = -1;
    bus->clock.vdp.vblank_active = 0;
    bus->vdp.display->active_draw_buffer ^= 1;
    bus->clock.current_back_buffer = bus->vdp.display->active_draw_buffer;
    bus->clock.current_front_buffer = bus->vdp.display->active_draw_buffer ^ 1;
    bus->vdp.cur_output = static_cast<u16 *>(bus->vdp.display->output[bus->vdp.display->active_draw_buffer]);
...
void core::new_scanline(u64 cur_clock)
{
    ...
    render_sprites();
    int old_vcount = bus->clock.vdp.vcount;
    bus->clock.vdp.vcount++;
    //printf("\nNEW SCANLINE %d old:%d clk:%lld", bus->clock.vdp.vcount, old_vcount, bus->clock.master_cycle_count);
    sc_slot = 0;
    set_clock_divisor();
    ...
    if (bus->clock.vdp.vcount >= (bus->clock.vdp.bottom_rendered_line+2)) latch_hcounter(); // in vblank, continually reload. TODO: make dependent on vblank
    else do_vcounter<do_debug>();
    set_sc_array();
}
...
void core::schedule_scanline(void *ptr, u64 key, u64 clock, u32 jitter)
{
    ...
    if (::dbg.do_debug) th->new_scanline<true>(cur);
    else th->new_scanline<false>(cur);
}
...
void core::reschedule_from_state()
{
    ...
    bus->clock.current_back_buffer = display->active_draw_buffer;
    bus->clock.current_front_buffer = display->active_draw_buffer ^ 1;
    cur_output = static_cast<u16 *>(display->output[bus->clock.current_back_buffer]);

    const i32 scanlines_per = bus->clock.timing.frame.scanlines_per > 0 ? bus->clock.timing.frame.scanlines_per : 262;
    if ((bus->clock.vdp.vcount < -1) || (bus->clock.vdp.vcount >= scanlines_per)) {
        bus->clock.vdp.vcount = -1;
    }

    if (bus->clock.vdp.bottom_rendered_line <= 0) {
        bus->clock.vdp.bottom_rendered_line = 223;
    }
    if (bus->clock.vdp.bottom_max_rendered_line < static_cast<u32>(bus->clock.vdp.bottom_rendered_line)) {
        bus->clock.vdp.bottom_max_rendered_line = static_cast<u32>(bus->clock.vdp.bottom_rendered_line);
    }
    bus->clock.vdp.vblank_on_line = bus->clock.vdp.bottom_rendered_line + 1;

    constexpr u32 frame_pixels = 1280 * 240;
    if (cur_pixel >= frame_pixels) {
        if ((bus->clock.vdp.vcount >= 0) && (bus->clock.vdp.vcount < 240)) {
            cur_pixel = static_cast<u32>(bus->clock.vdp.vcount) * 1280;
        }
        else {
            cur_pixel = 0;
        }
    }
    if (line.screen_x > 320) {
        line.screen_x = 0;
    }

    const u64 now = bus->clock.master_cycle_count;
    const u64 scanline_cycles = bus->clock.timing.scanline.cycles_per ? bus->clock.timing.scanline.cycles_per : 3420;
    bus->scheduler.only_add_abs_w_tag(static_cast<i64>(now + scanline_cycles), 0, this, &schedule_scanline, nullptr, 1);

    u32 lines_completed = bus->clock.vdp.vcount < 0 ? 0 : static_cast<u32>(bus->clock.vdp.vcount + 1);
    if (lines_completed >= static_cast<u32>(scanlines_per)) {
        lines_completed = 0;
    }
    u64 lines_until_frame = static_cast<u64>(scanlines_per) - lines_completed;
    if (lines_until_frame == 0) {
        lines_until_frame = 1;
    }
    const u64 elapsed_in_frame = static_cast<u64>(lines_completed) * scanline_cycles;
    bus->clock.vdp.frame_start = now >= elapsed_in_frame ? now - elapsed_in_frame : 0;
    bus->scheduler.only_add_abs_w_tag(static_cast<i64>(now + (lines_until_frame * scanline_cycles)), 0, bus, &new_frame, nullptr, 2);
}
...
template void core::write_vdp_reg<true>(u16 rn, u16 val);
template void core::write_vdp_reg<false>(u16 rn, u16 val);
...
void core::dma_load()
{
    ...
    u16 data = bus->mainbus_read<do_debug>(addr & 0xFFFFFF, 1, 1, 0, true);
    ...
    write_data_port<do_debug>(data, false);
    if constexpr(do_debug) {
...
void core::dma_fill()
{
    ...
    switch(command.target) {
        case 1:
            ...
            VRAM_write_byte<do_debug>(command.address, dma.fill_value);
            break;
        case 3:
            //if (dbg.traces.vdp2) DFT("\nVDP DMA FILL CRAM ADDR:%04x DATA:%02x", command.address, dma.fill_value);
            CRAM_write_byte<do_debug>(command.address, dma.fill_value);
            break;
        case 5:
            VSRAM_write_byte<do_debug>(command.address, dma.fill_value);
            break;
        default:
...
void core::dma_copy()
{
    ...
    VRAM_write_byte<do_debug>(command.address, data);
    ...
    if (!dma.locked) {
        ...
        if (dma.enable) {
            ...
            //while(command.pending && !dma.fill_pending) {
                switch(dma.mode) {
                    case 0: case 1: dma_load<do_debug>(); break;
                    case 2: dma_fill<do_debug>(); break;
                    case 3: dma_copy<do_debug>(); break;
                }
            }
        }
        dma.locked = 0;
...
void core::write_control_port(u16 val, u16 mask)
{
    ...
    write_vdp_reg<do_debug>((val >> 8) & 0x1F, val & 0xFF);
}
...
template void core::z80_write<true>(u32 addr, u8 val);
template void core::z80_write<false>(u32 addr, u8 val);
template<bool do_debug>
void core::z80_write(u32 addr, u8 val)
{
    if ((addr >= 0x7F00) && (addr < 0x7F20)) {
        mainbus_write<do_debug>((addr & 0x1E) | 0xC00000, val, 0xFF);
    }
}

template void core::mainbus_write<true>(u32 addr, u16 val, u16 mask);
template void core::mainbus_write<false>(u32 addr, u16 val, u16 mask);
...
void core::mainbus_write(u32 addr, u16 val, u16 mask)
{
    ...
    switch(addr) {
        case 0xC00000: // VDP data port
        case 0xC00002:
            write_data_port<do_debug>(val, true);
            return;
        case 0xC00004: // VDP control
        case 0xC00006:
            write_control_port<do_debug>(val, mask);
            return;
...
void core::get_line_hscroll(u32 line_num, u32 *planes)
{
    ...
    planes[PLANE_A] = VRAM_read(addr);
    planes[PLANE_B] = VRAM_read(addr + 1);
}
...
void core::fetch_slice(u32 nt_base_addr, u32 col, u32 row, slice *slice, u32 plane_num)
{
    ...
    u32 tile_data = VRAM_read(nt_base_addr + (tile_row * foreground_width) + tile_col);
    ...
    tile_data = VRAM_read(nt_base_addr + (tile_row * foreground_width) + tile_col);
...
void core::render_sprites()
{
    ...
    const i32 vc = (bus->clock.vdp.vcount == (bus->clock.timing.frame.scanlines_per-1)) ? 0 : bus->clock.vdp.vcount + 1;
    if (vc > bus->clock.vdp.bottom_rendered_line) return;

    for (auto & s : sprite_line_buf) s = {};
...
void core::render_left_column()
{
    ...
    render_16_more<do_debug>();
}

template void core::do_cycle<true>();
template void core::do_cycle<false>();
