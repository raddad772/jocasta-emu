...
bool IOU::get_joystick_button(u32 idx)
{
    ...
    const auto &bl = joystick_ptr.get().controller.digital_buttons;
    u32 slot = 4 + idx;
    return (slot < static_cast<u32>(bl.size())) && (bl[slot].state != 0);
}
...
void IOU::pixels_text()
{
    ...
    bus->cpu.pins.D = ch;  // video fetch drives open bus
...
void IOU::pixels_lores()
{
    ...
    u32 page_base = io.PAGE2 ? 0x0800u : 0x0400u;
    u32 text_addr = page_base + ((row & 7) << 7) + ((row >> 3) * 40) + col;

    u8 byte = bus->mmu.RAM.ptr[text_addr];
    bus->cpu.pins.D = byte;  // video fetch drives open bus

    // Low nibble = top half of cell (scanlines 0-3), high nibble = bottom (4-7)
    u8 color = (line_in_text < 4) ? (byte & 0x0Fu) : ((byte >> 4) & 0x0Fu);

    for (u32 dot = 0; dot < 7; dot++) {
        if (x == 0) line_output[x++] = color | 0x80u;
        else        line_output[x++] = color;
        line_output[x++] = color;
    }
}
    ...
    if (addr3 == 0xC060) {
        ...
        switch(addr4) {
            ...
            case 5: {
                ...
                // Apple II PREAD loop ~ 9 CPU cycles x 14 master clocks = 126 per count.
                // paddle_value 0-255 maps to ~0-240 counts.
                u32 axis  = addr4 - 4;   // 0=X, 1=Y
                u32 pval  = get_paddle_value(axis);
                u64 thr   = static_cast<u64>(pval) * 126u;
