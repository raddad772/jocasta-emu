...
void core::update_IRQ(u32 device_num, u8 level) {
...
void core::update_NMI(u32 device_num, u8 level) {
    const u32 msk = 1 << device_num;
    u32 old = NMI_F;
    if (level) NMI_F |= msk;
    else NMI_F &= ~msk;
...
void core::load_prg(multi_file_set &mfs) {
...
void core::runtime_sideload(multi_file_set& mfs)
{
    sideload(mfs);
    complete_sideload();
}
...
u32 read_trace_m6502(void *ptr, u32 addr) {
    return static_cast<core *>(ptr)->mem.read_main_bus(addr, 0, 0);
}
...
static void populate_opts(core *th) {
    debugger_widgets_add_button(th->opts, "Load selected file", 1, 0);
}

void core::complete_sideload() {
...
}  // namespace C64  -- the global factory below must NOT be inside it (called by new_system)

jsm_system *Commodore64_new(jsm::systems in_kind)
{
    auto* th = new C64::core(jsm::regions::USA);
    C64::populate_opts(th);
    ...
    dt.read_trace = &C64::read_trace_m6502;
...
void core::setup_audio(std::vector<physical_io_device> &inIOs)
{
...
static void C64IO_load_cart(JSM, multi_file_set &mfs, physical_io_device &which_pio)
{
    ...
    /*auto *C64 = dynamic_cast<core *>(jsm);
