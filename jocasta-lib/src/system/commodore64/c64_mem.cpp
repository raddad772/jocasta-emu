...
u8 MEM::read_IO(u16 addr, u8 old, bool has_effect) {
    switch (addr & 0xDF00) {
        ...
        case 0xDC00: {
            u8 v = bus->cia1.read(addr, old, has_effect);
            switch (addr & 0xF) {
                case 0: // PRA — joystick port 2 on bits 0-4 (wired-AND: active-low)
                    v &= bus->joy2.pin_state();
                    break;
                case 1: // PRB — joystick port 1 on bits 0-4 (wired-AND: active-low)
                    v &= bus->joy1.pin_state();
                    break;
                default: break;
            }
            return v; }
...
void MEM::write_cpu_IO(u16 addr, u8 val) {
    switch(addr) {
        case 0x0000:
            io.cpu.dir = val & 0x3F;
            io.cpu.u = (io.cpu.data & io.cpu.dir) | (~io.cpu.dir & 0x3F);
            remap();
            return;
        case 0x0001:
            io.cpu.data = val & 0x3F;
            io.cpu.u = (io.cpu.data & io.cpu.dir) | (~io.cpu.dir & 0x3F);
            remap();
            return;
    }
...
void MEM::first_map() {
    ...
    map_reads(0x0000, 0x0100, this, read_zp);;
