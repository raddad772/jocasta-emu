...
void TIA::m_step(u32 num, u32 clocks)
{
    while (clocks--) {
        ...
        if (m[num].starting && (m[num].start_counter-- == 0)) {
            ...
            m[num].width_counter = static_cast<i32>(m_width(num));
        }
...
void TIA::p_step(u32 num, u32 clocks)
{
    ...
    while (clocks--) {
        ...
        if (P[num].starting && (P[num].start_counter-- == 0)) {
            ...
            P[num].width_counter = static_cast<i32>(p_width(num));
        }
...
void TIA::write(u16 addr, u8 *data)
{
    // b-f, 1b-24
    u32 val = *data;
    i32 msx = static_cast<i32>(hcounter) - 68;
#define DELAY(num, howlong) case num: WQ_add(num, val, howlong); return
    switch(addr & 0x3F) {
        ...
        case 0x25: // VDELP0 vertical delay player 0
            P[0].delay = static_cast<i32>(val) & 1;
            return;
        case 0x26: // VDELP1 vertical delay player 1
            P[1].delay = static_cast<i32>(val) & 1;
            return;
        case 0x27: // VDELBL vertical delay ball
            ball.delay = (static_cast<i32>(val) & 1);
            return;
...
u32 TIA::get_player_pixel(u32 msx, u32 player_num)
{
    ...
    if ((static_cast<i32>(msx) - player_x) >= 80) return 0;

    // We haven't yet gotten to the left edge of the player X...
    if (static_cast<i32>(msx) < player_x) return 0;

    u32 pm_num_size = P[player_num].size;
    u32 offset = static_cast<u32>(static_cast<i32>(msx) - player_x);
...
u32 TIA::get_ball_pixel(u32 msx)
{
    ...
    if (static_cast<i32>(msx) < ball.counter) return 0; // We aren't at left edge of ball yet

    u32 ball_size = BALL_SIZES[io.CTRLPF.ball_size];
    return static_cast<i32>(msx) < (ball.counter + static_cast<i32>(ball_size));
}
...
u32 TIA::get_missile_pixel(u32 msx, u32 missile_num)
{
    ...
    if (static_cast<i32>(msx) < m[missile_num].counter) return 0; // We aren't at left edge of missile yet

    u32 missile_size = BALL_SIZES[(m[missile_num].size)];
    return static_cast<i32>(msx) < (m[missile_num].counter + static_cast<i32>(missile_size));
}
...
void TIA::run_cycle()
{
    ...
    hcounter = pcounter;
    i32 screen_x = static_cast<i32>(pcounter) - 68;
    hblank = (pcounter < 68) || (io.hmoved && (pcounter < 76));
    if (screen_x >= 0)
        x = static_cast<u32>(screen_x);
    ...
    if (IN_DISPLAY && screen_x >= 0 && screen_x < 160) {
        ...
        if (hblank || io.vblank) {
            ...
            if ((screen_x & 3) == 0) {
                u32 pf_x = static_cast<u32>(screen_x) >> 2;
