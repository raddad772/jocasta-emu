...
struct HID_digital_button {
    ...
    u32 state{};
    u32 virtual_state{}; // set by virtual controller UI; ORed into state each frame
...
struct JSM_KEYBOARD {
    JKEYS key_defs[100];
    u32 key_states[100];
    u32 virtual_key_states[100]; // set by virtual keyboard UI; ORed into key_states each frame
    u32 num_keys;
};
...
struct JSM_DISPLAY {
    ...
    u32 active_draw_buffer{};

    u32 cur_output_width{}, cur_output_height{};
    //u32 last_displayed;
    u16 *cur_output{};
    ...
    void allocate_output(u32 index, size_t size) {
        if (index >= 2) return;
        if (output[index] != nullptr) free(output[index]);
        output[index] = size ? malloc(size) : nullptr;
        output_size[index] = output[index] ? size : 0;
    }

    void clear_output(u32 index) {
        if (index >= 2 || output[index] == nullptr || output_size[index] == 0) return;
        memset(output[index], 0, output_size[index]);
    }
    ...
    ~JSM_DISPLAY() {
        if (output[0] != nullptr) { free(output[0]); output[0] = nullptr; output_size[0] = 0; }
        if (output[1] != nullptr) { free(output[1]); output[1] = nullptr; output_size[1] = 0; }
...
struct JSM_CHASSIS {
    ...
    // Find a button by its common_id (DBCID).  Returns nullptr if not present.
    HID_digital_button* find_button(JKEYS id) {
        for (auto& b : digital_buttons)
            if (b.common_id == id) return &b;
        return nullptr;
    }
    const HID_digital_button* find_button(JKEYS id) const {
        for (auto& b : digital_buttons)
            if (b.common_id == id) return &b;
        return nullptr;
    }
    // Convenience: return the button's state (0 if not found).
    u32 button_state(JKEYS id) const {
        const HID_digital_button* b = find_button(id);
        return b ? b->state : 0u;
    }
};
...
struct JSM_AUDIO_CHANNEL {
    ...
    u32 num{};
    u32 low_pass_filter{};
    // Per-stream mixing weight applied by the GUI before summing to output.
    // Default 1.0 — correct for cores with a single stream.
    // Multi-stream cores (e.g. Genesis YM2612 + SN76489) set different values.
    float mix_volume{1.0f};
    // Populated by the GUI after allocating the ring.
    // Cores read this pointer in audio_rings_ready() to obtain their push target.
...
enum rtc_time_source { RTC_SOURCE_WALLCLOCK = 0, RTC_SOURCE_USER = 1 };

struct JSM_RTC {
    bool present{false};
    bool has_date{true};
    bool wall_valid{false};
    rtc_time_source source{RTC_SOURCE_WALLCLOCK};
    bool reseed{false};
    rtc_datetime wall{};
    rtc_datetime user{};
    rtc_datetime game{};
...
struct JSM_DISC_DRIVE {
    ...
    bool motor_on{};  // drive motor spinning — controls activity light
};
