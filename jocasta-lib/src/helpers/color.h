...
static inline u32 neogeo_to_screen(u32 color)
{
    // red is 14, 11-8
    // green is 13, 7-4
    // blue is 12, 3-0
    // they all share 15
    u32 common = (color >> 15) & 1;
    u32 b = ((color >> 6) & 0x3C) | common;
    u32 g = ((color >> 2) & 0x3C) | common;
    u32 r = ((color << 2) & 0x3C) | common;
    r |= (color >> 13) & 2;
    g |= (color >> 12) & 2;
    b |= (color >> 11) & 2;
    if (color & 0x10000) {
    ...
    g <<= 2;
    b <<= 2;
    // We've now got 6 bits for each component, not bad
...
static inline u32 nds_coeff5(u32 color, u32 co)
{
    i32 c = (i32)color;
    i32 b = (c >> 12) & 0x3F;
    i32 g = (c >> 6) & 0x3F;
...
static inline u32 nds_alpha_3d(u32 target_a, u32 target_b, u32 eva, u32 evb)
{
    u32 c = nds_coeff5(target_a, eva) + nds_coeff5(target_b, evb);
    u32 b = (c >> 16) & 0xFF;
    u32 g = (c >> 8) & 0xFF;
    u32 r = c & 0xFF;
    if (b > 0x3F) b = 0x3F;
