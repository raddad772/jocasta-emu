...
scheduler_event* scheduler_t::alloc_event(i64 timecode, u64 key, scheduler_event* next, u64 id) {
    ...
    if (to_delete.num) {
        ...
        se = static_cast<scheduler_event *>(malloc(sizeof(scheduler_event)));
        if (!se) printf("\nWARN BAD SE PTR!");
    }
...
template u64 scheduler_t::bind_or_run<true>(scheduler_event *e, void *ptr, scheduler_callback func, scheduler_callback func_debug, i64 timecode, u64 key, u32 *still_sched);
template u64 scheduler_t::bind_or_run<false>(scheduler_event *e, void *ptr, scheduler_callback func, scheduler_callback func_debug, i64 timecode, u64 key, u32 *still_sched);

template<bool do_debug>
u64 scheduler_t::bind_or_run(scheduler_event *e, void *ptr, scheduler_callback func, scheduler_callback func_debug, i64 timecode, u64 key, u32 *still_sched)
{
    if (!e) {
        if (still_sched) *still_sched = 0;
        if constexpr (do_debug) func_debug(ptr, key, timecode, 0);
        else func(ptr, key, timecode, 0);
        return 0;
    }

    if (still_sched) *still_sched = 1;
    e->bound_func.ptr = ptr;
    e->bound_func.func[0] = func;
    e->bound_func.func[1] = func_debug;
...
template u64 scheduler_t::add_or_run_abs<false>(i64 timecode, u64 key, void *ptr, scheduler_callback callback, scheduler_callback callback_debug, u32 *still_sched);
template u64 scheduler_t::add_or_run_abs<true>(i64 timecode, u64 key, void *ptr, scheduler_callback callback, scheduler_callback callback_debug, u32 *still_sched);

template<bool do_debug>
u64 scheduler_t::add_or_run_abs(i64 timecode, u64 key, void *ptr, scheduler_callback callback, scheduler_callback callback_debug, u32 *still_sched)
{
    scheduler_event *e = add_abs(timecode, key, 1);
    return bind_or_run<do_debug>(e, ptr, callback, callback_debug, timecode, key, still_sched);
}
...
u64 scheduler_t::only_add_abs_w_tag(i64 timecode, u64 key, void *ptr, scheduler_callback callback, scheduler_callback callback_debug, u32 *still_sched, u32 tag)
{
    scheduler_event *e = add_abs(timecode, key, 0);
    u64 id = bind_or_run<false>(e, ptr, callback, callback_debug, timecode, key, still_sched);
    e->tag = tag;
    //printf("\n%lld %lld", id, key);
    return id;
}
...
u64 scheduler_t::only_add_abs(i64 timecode, u64 key, void *ptr, scheduler_callback callback, scheduler_callback callback_debug, u32 *still_sched)
{
    scheduler_event *e = add_abs(timecode, key, 0);
    // bind or run will only run if it gets a pointer *e
    u64 id = bind_or_run<false>(e, ptr, callback, callback_debug, timecode, key, still_sched);
    return id;
}


u64 scheduler_t::add_after_first_event(void *ptr, scheduler_callback callback, scheduler_callback callback_debug, u32 *still_sched)
{
    if (first_event == nullptr) {
        return only_add_abs(static_cast<i64>(current_time()) + 1, 0, ptr, callback, callback_debug, still_sched);
    }
    const i64 tc = first_event->timecode;
    *clock = static_cast<u64>(tc);

    scheduler_event *insert_after = first_event;
    while (insert_after->next != nullptr && insert_after->next->timecode == tc)
        insert_after = insert_after->next;

    u64 id = id_counter++;
    scheduler_event *e = alloc_event(tc, 0, insert_after->next, id);
    insert_after->next = e;
    e->bound_func.ptr = ptr;
    e->bound_func.func[0] = callback;
    e->bound_func.func[1] = callback_debug;
    e->still_sched = still_sched;
    if (still_sched) *still_sched = 1;
    return id;
}
...
u64 scheduler_t::add_next(u64 key, void *ptr, scheduler_callback callback, u32 *still_sched)
{
    ...
    first_event->bound_func.func[0] = callback;
    first_event->bound_func.func[1] = callback;
    first_event->still_sched = still_sched;
    return id;
}

u64 scheduler_t::add_next(u64 key, void *ptr, scheduler_callback callback, scheduler_callback callback_debug, u32 *still_sched)
{
    u64 id = id_counter++;
    first_event = alloc_event(0, key, first_event, id);
    if (still_sched) *still_sched = 1;
    first_event->bound_func.ptr = ptr;
    first_event->bound_func.func[0] = callback;
    first_event->bound_func.func[1] = callback_debug;
    first_event->still_sched = still_sched;
    return id;
}
...
void scheduler_t::run_for_cycles(u64 howmany)
{
    ...
    while(!dbg.do_break) {
        ...
        while(loop_start_clock >= e->timecode) {
            ...
            e->next = nullptr;
            e->bound_func.func[1](e->bound_func.ptr, e->key, current_time(), static_cast<u32>(jitter));
        ...
        // Now...Run cycles!
        i64 num_cycles_to_run = e->timecode - loop_start_clock;
        ...
        in_event = false;
        run.func[1](run.ptr, num_cycles_to_run, *clock, 0);
...
template void scheduler_t::run_for_cycles_tg16<false>(u64 howmany);
template void scheduler_t::run_for_cycles_tg16<true>(u64 howmany);
...
void scheduler_t::run_for_cycles_tg16(u64 howmany)
{
    ...
    while(!dbg.do_break && *clock < exit_at) {
        ...
        e->next = nullptr;
        *clock = e->timecode;
        if constexpr(do_debug) e->bound_func.func[1](e->bound_func.ptr, e->key, current_time(), 0);
        else e->bound_func.func[0](e->bound_func.ptr, e->key, current_time(), 0);
...
template void scheduler_t::run_til_tag_tg16<false>(u32 tag);
template void scheduler_t::run_til_tag_tg16<true>(u32 tag);
...
void scheduler_t::run_til_tag_tg16(u32 tag)
{
    ...
    while(!dbg.do_break) {
        ...
        *clock = e->timecode;
        ...
        if constexpr(do_debug) e->bound_func.func[1](e->bound_func.ptr, e->key, e->timecode, 0);
        else e->bound_func.func[0](e->bound_func.ptr, e->key, e->timecode, 0);
        u32 etag = e->tag;
...
template void scheduler_t::run_til_tag<true>(u32 tag);
template void scheduler_t::run_til_tag<false>(u32 tag);
...
void scheduler_t::run_til_tag(u32 tag)
{
    ...
    while(!dbg.do_break) {
        ...
        while(loop_start_clock >= e->timecode) {
            ...
            if (e->still_sched) *e->still_sched = 0; // Set it now, so it can be reset if needed during function execution
            e->next = nullptr;
            e->bound_func.func[do_debug](e->bound_func.ptr, e->key, current_time(), static_cast<u32>(jitter));
            u32 etag = e->tag;
        ...
        if (num_cycles_to_run > max_block_size) num_cycles_to_run = max_block_size;
        in_event = false;
        run.func[do_debug](run.ptr, num_cycles_to_run, *clock, 0);
    }
    if (dbg.do_break) {
        cycles_left_to_run = 0;
    }
