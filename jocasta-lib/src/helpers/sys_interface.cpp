...
void populate_core_options(jsm::systems which, std::vector<jsm_core_option>& out)
{
    ...
    [[maybe_unused]] auto add_bool = [&](const char* key, const char* label, i32 default_val = 0) {
        jsm_core_option opt;
        snprintf(opt.key,   sizeof(opt.key),   "%s", key);
        snprintf(opt.label, sizeof(opt.label), "%s", label);
    ...
    switch (which) {
        ...
        case jsm::systems::GBA: {
            ...
            ci.add_choice("Cached interpreter (experimental, may be worse)", 1);
            ci.value = 0; // default: interpreter (cached interp is experimental / buggy)
            break;
        }
