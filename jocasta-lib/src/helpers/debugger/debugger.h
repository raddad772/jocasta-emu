...
struct debug_waveform {
    ...
    inline void setup(float cycles_per_frame) {
        if (samples_requested == 0) return;
        samples_rendered    = samples_requested;
        user.cycle_stride   = cycles_per_frame / static_cast<float>(samples_requested);
        user.buf_pos        = 0;
    }
};
...
namespace debug::waveform2 {
    ...
    struct wf {
        ...
        inline void setup(double cycles_per_frame) {
            if (samples_requested == 0) return;
            samples_rendered  = samples_requested;
            user.cycle_stride = cycles_per_frame / static_cast<double>(samples_requested);
            user.buf_pos      = 0;
            rendering_buf     = (rendering_buf + 1) % 3;

            size_t sample_count = static_cast<size_t>(samples_requested) * (stereo ? 2 : 1);
            size_t byte_count = std::min(buf[rendering_buf].size, sample_count * sizeof(float));
            if (byte_count) memset(buf[rendering_buf].ptr, 0, byte_count);
        }
    };
...
struct debugger_view {
    ...
    struct {
        ...
        struct { u32 enabled{}; i32 line_num{}; } on_line{};
    } update{};
