...
{
    if ((ptr == nullptr) || (allocated_len <= 0)) {
        assert(1==0);
        return;
    }
    if (pos < 0) pos = 0;
    if (pos >= allocated_len) pos = allocated_len - 1;
    cur = ptr + pos;
}
...
int jsm_string::vsprintf(const char* format, va_list va)
{
    ...
    if (cur < ptr) cur = ptr;
    if (cur >= (ptr + allocated_len)) cur = ptr + allocated_len - 1;

    size_t remaining = static_cast<size_t>((ptr + allocated_len) - cur);
    int num = vsnprintf(cur, remaining, format, va);
    ...
    if (static_cast<size_t>(num) >= remaining) {
        cur = ptr + allocated_len - 1;
        *cur = 0;
        return num;
    }
