...
static constexpr u32 make_rgba(u32 r, u32 g, u32 b)
{
    return 0xFF000000u | (b << 16) | (g << 8) | r;
}

static u32 present_buffer_for_pixel(const jsm_present_context *ctx, u32 y, u32 x, u32 default_buffer)
{
    if (!ctx || !ctx->partial) return default_buffer & 1;
    if (y < ctx->split_y) return ctx->draw_buffer & 1;
    if (y > ctx->split_y) return ctx->complete_buffer & 1;
    if (ctx->split_x >= 0 && x < static_cast<u32>(ctx->split_x)) return ctx->draw_buffer & 1;
    return ctx->complete_buffer & 1;
}

template<typename T>
static T *present_buffer(physical_io_device &device, const jsm_present_context *ctx, u32 y, u32 x, u32 default_buffer)
{
    return static_cast<T *>(device.display.output[present_buffer_for_pixel(ctx, y, x, default_buffer)]);
}

template<typename T>
static T *present_metadata_buffer(physical_io_device &device, const jsm_present_context *ctx, u32 y, u32 x, u32 default_buffer)
{
    return static_cast<T *>(device.display.output_debug_metadata[present_buffer_for_pixel(ctx, y, x, default_buffer)]);
}
...
void mac512k_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, const jsm_present_context *ctx)
{
    u32* imgdata = (u32 *)out_buf;
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;
    for (u32 ry = 0; ry < 342; ry++) {
        ...
        for (u32 rx = 0; rx < 512; rx++) {
            ...
            u8* output = present_buffer<u8>(device, ctx, y, x, default_buffer);
...
void apple2_present(physical_io_device &device, void *out_buf, u32 x_offset, u32 y_offset, u32 out_width, u32 out_height, const jsm_present_context *ctx)
{
    u32* img32 = calc_start_pos(out_buf, x_offset, y_offset, out_width);
    u32 stride = calc_stride(out_width, 560);
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;

    u32 color_mode = ctx ? ctx->apple2_color_mode : 0;
    bool respect_burst = ctx ? ctx->apple2_respect_burst : true;

    static constexpr u32 ntsc_colors4[4] = {
        make_rgba(0xBF, 0x3F, 0xBF),
        make_rgba(0x00, 0x40, 0xE0),
        make_rgba(0x00, 0xC0, 0x00),
        make_rgba(0xE0, 0x60, 0x00),
    };

    for (u32 y = 0; y < 192; y++) {
        u8 *appleo = present_buffer<u8>(device, ctx, y, 0, default_buffer);
        u8 *line = appleo + (y * 560);
        bool text_row = (line[0] & 0x80u) != 0;
        bool force_mono = (color_mode == 0) || (text_row && respect_burst && color_mode == 2);

        for (u32 lx = 0; lx < 280; lx++) {
            u32 x = lx << 1;
            u8 pval = line[x] & 0x7F;
            bool pixel_on = pval & 1u;
            bool left_on = (lx > 0) && (line[x - 2] & 1u);
            bool right_on = (lx < 279) && (line[x + 2] & 1u);
            u32 color;

            if (force_mono) {
                color = pixel_on ? 0xFFFFFFFFu : 0xFF000000u;
            } else if (color_mode == 1) {
                color = pixel_on ? 0xFF33FF33u : 0xFF000000u;
            } else if (pixel_on) {
                u8 pal = (pval >> 1) & 1u;
                u32 phase = ((lx & 1u) * 2u + pal) & 3u;
                color = (left_on || right_on) ? 0xFFFFFFFFu : ntsc_colors4[phase];
            } else if (left_on && right_on) {
                u8 pal = (line[x - 2] >> 1) & 1u;
                u32 phase = ((((lx - 1) & 1u) * 2u) + pal) & 3u;
                color = ntsc_colors4[phase];
            } else {
                color = 0xFF000000u;
            }
...
void galaksija_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, const jsm_present_context *ctx)
{
...
void cosmac_vip_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, const jsm_present_context *ctx) {
...
void zx_spectrum_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, const jsm_present_context *ctx)
{
    u32* imgdata = (u32 *)out_buf;
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;
...
void atari2600_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, const jsm_present_context *ctx)
{
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;
    for (u32 y = 0; y < 192; y++) {
        for (u32 x = 0; x < 160; x++) {
            u8* ibuf = present_buffer<u8>(device, ctx, y, x, default_buffer);
            u8* iptr = (u8 *)ibuf + ((y * 160) + x);
            u8* optr = static_cast<u8 *>(out_buf) + (((y * out_width) + x) * 4);

            // TIA colors are CCCC LLL after dropping bit 0, but this table is stored as
            // luminance rows of 16 hues.
            u32 tiacolor = *iptr & 0x7F;
            u32 col = TIA_palette[((tiacolor & 7) << 4) | (tiacolor >> 3)];
            optr[0] = (col >> 16) & 0xFF;
            optr[1] = (col >> 8) & 0xFF;
            optr[2] = col & 0xFF;
            optr[3] = 255;
...
void DMG_present(physical_io_device &device, void *out_buf, u32 x_offset, u32 y_offset, u32 out_width, u32 out_height, bool is_event_view_present, const jsm_present_context *ctx)
{
    ...
    for (u32 y = 0; y < 144; y++) {
        ...
        for (u32 x = 0; x < 160; x++) {
            u16* ibuf = present_buffer<u16>(device, ctx, y, x, default_buffer);
            u16* mdbuf = present_metadata_buffer<u16>(device, ctx, y, x, default_buffer);
...
void GBC_present(physical_io_device &device, void *out_buf, u32 x_offset, u32 y_offset, u32 out_width, u32 out_height, bool is_event_view_present, const jsm_present_context *ctx)
{
    if (is_event_view_present) memset(out_buf, 0, out_width*out_height*4);
    //u16* ptr = ibuf;
...
void NES_present(physical_io_device &device, void *out_buf, u32 x_offset, u32 y_offset, u32 out_width, u32 out_height, const jsm_present_context *ctx)
{
    u32* img32 = calc_start_pos(out_buf, x_offset, y_offset, out_width);
    ...
    for (u32 nes_y = 0; nes_y < 240; nes_y++) {
        ...
        for (u32 nes_x = 0; nes_x < 256; nes_x++) {
            ...
            u16* neso = present_buffer<u16>(device, ctx, nes_y, nes_x, default_buffer);
...
void c64_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32 is_event_view_present, const jsm_present_context *ctx) {
    ...
    for (u32 ry = 0; ry < ysize; ry++) {
        ...
        for (u32 rx = 0; rx < xsize; rx++) {
            u32 x = rx;
            u32 di = ((y * xsize) + x);
            u8 *c64o = present_buffer<u8>(device, ctx, y, x, default_buffer);
            u8 c = c64o[di];
...
void neogeo_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32 is_event_view_present, const jsm_present_context *ctx) {
    // always 320x224
    u32 *img32 = static_cast<u32 *>(out_buf);
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;

    for (u32 ry = 0; ry < 224; ry++) {
        u32 *line_out = img32 + (ry * out_width);
        for (u32 rx = 0; rx < 320; rx++) {
            u32 *ngo = present_buffer<u32>(device, ctx, ry, rx, default_buffer);
            u32 *line_in = ngo + (ry * 320);
            u32 c = line_in[rx];
            line_out[rx] = neogeo_to_screen(c);
        }
    }
}

void ngp_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32, jsm::systems kind) {
    // Always 160x152. The KxGE writes one finished hardware pixel per dot into the u16 line
    // buffer; how to expand it to RGBA depends on the graphics engine:
    //   - NEOGEO_POCKET_COLOR (K2GE): a 12-bit color-palette value, R/G/B 4 bits each.
    //   - NEOGEO_POCKET       (K1GE): a 3-bit gray shade (0 = lightest .. 7 = darkest).
    u32 *img32 = static_cast<u32 *>(out_buf);
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;
    const u16 *kgeo = static_cast<const u16 *>(device.display.output[default_buffer]);
    const bool color = (kind == jsm::systems::NEOGEO_POCKET_COLOR);

    for (u32 y = 0; y < KXGE::DISP_HEIGHT; y++) {
        u32 *line_out = img32 + (y * out_width);
        const u16 *line_in = kgeo + (y * KXGE::DISP_WIDTH);
        for (u32 x = 0; x < KXGE::DISP_WIDTH; x++) {
            u16 v = line_in[x];
            u32 r, g, b;
            if (color) {
                r = (v >> 0) & 0x0F;
                ...
                u32 s = v & 7;
                r = g = b = ((7 - s) * 255) / 7; // shade 0 = lightest (rear-most), 7 = darkest
            }
            line_out[x] = make_rgba(r, g, b);
        }
    }
}

void PS1_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32 is_event_view_present, u32 &outcols, u32 &outrows) {
    u32 *ps1o = static_cast<u32 *>(device.display.output[device.display.active_draw_buffer ^ 1]);
    u32 *img32 = static_cast<u32 *>(out_buf);

    outcols = device.display.cur_output_width;
    outrows = device.display.cur_output_height;

    for (u32 ry = 0; ry < outrows; ry++) {
        u32 y = ry;
        u32 *line_out_ptr = img32 + (ry * out_width);

        for (u32 rx = 0; rx < outcols; rx++) {
            u32 x = rx;
            u32 di = ((y * 640) + x);
            u32 c = ps1o[di];
            //ps1o++;
...
void NDS_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32 is_event_view_present, const jsm_present_context *ctx)
{
    u32 *img32 = static_cast<u32 *>(out_buf);
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;

    for (u32 ry = 0; ry < 192; ry++) {
        u32 y = ry;
        u32 *line_out_ptr = img32 + (ry * out_width);
        for (u32 rx = 0; rx < 256; rx++) {
            u32 x = rx;
            u32 di = ((y * 256) + x);
            u32 *gbao = present_buffer<u32>(device, ctx, y, x, default_buffer);
            u32 c = gbao[di];
...
void GBA_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32 is_event_view_present, const jsm_present_context *ctx)
{
    ...
    for (u32 ry = 0; ry < 160; ry++) {
        ...
        for (u32 rx = 0; rx < 240; rx++) {
            ...
            u16 *gbao = present_buffer<u16>(device, ctx, y, x, default_buffer);
...
void tg16_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32 is_event_view_present, events_view *evp, const jsm_present_context *ctx)
{
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;
    ...
    for (u32 ry = 0; ry < 242; ry++) {
        ...
        for (u32 rx = 0; rx < HUC6260::CYCLE_PER_LINE; rx++) {
            ...
            u16 *tg16o = present_buffer<u16>(device, ctx, y, rx, default_buffer);
            u32 color = tg16o[di];
            if (color & 0x8000) {
...
void snes_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32 is_event_view_present, const jsm_present_context *ctx)
{
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;
    ...
    for (u32 ry = 0; ry < 224; ry++) {
        ...
        for (u32 rx = 0; rx < 512; rx++) {
            ...
            u16 *sneso = present_buffer<u16>(device, ctx, y, x, default_buffer);
            u32 color = sneso[di];
...
void CASIO_PV1000_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, const jsm_present_context *ctx) {
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;
    //u16 *geno = (u16 *)device.display.output[0];
    u32 w = out_width;//256 - (overscan_left + overscan_right);
    u32 *out32 = static_cast<u32 *>(out_buf);

    for (u32 y = 0; y < 192; y++) {
        u32 *out_line = out32 + (w * y);
        for (u32 x = 0; x < 256; x++) {
            u8 *pvo = present_buffer<u8>(device, ctx, y, x, default_buffer);
            u8 *in_line = pvo + (256 * y);
            u32 color = 0xFF000000;
            u8 b = in_line[x];
            if (b & 1) color |= 0x000000FF;
            if (b & 2) color |= 0x0000FF00;
            if (b & 4) color |= 0x00FF0000;
            out_line[x] = color;
        }
    }
...
void genesis_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32 is_event_view_present, const jsm_present_context *ctx)
{
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;
    //u16 *geno = (u16 *)device.display.output[0];
    u32 w = out_width;//256 - (overscan_left + overscan_right);
    ...
    for (u32 ry = 0; ry < 224; /*data.bottom_rendered_line; */ ry++) {
        ...
        for (u32 rx = 0; rx < 1280; rx++) {
            ...
            u16 *geno = present_buffer<u16>(device, ctx, y, x, default_buffer);
            u32 color = geno[di];
            u32 mode = (color >> 10) & 3;
            color &= 0x1FF;
...
void SMS_present(physical_io_device &device, void *out_buf, u32 x_offset, u32 y_offset, u32 out_width, u32 out_height, const jsm_present_context *ctx)
{
    u32* img32 = calc_start_pos(out_buf, x_offset, y_offset, out_width);
    u32 stride = calc_stride(out_width, 256);
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;
    ...
    for (u32 y = 0; y < 192; /*data.bottom_rendered_line; */ y++) {
        for (u32 x = 0; x < 256; x++) {
            ...
            u16 *smso = present_buffer<u16>(device, ctx, y, x, default_buffer);
            u32 color = smso[di];
            u32 r, g, b;
            b = ((color >> 4) & 3) * 0x55;
            g = ((color >> 2) & 3) * 0x55;
...
void GG_present(physical_io_device &device, void *out_buf, u32 x_offset, u32 y_offset, u32 out_width, u32 out_height, const jsm_present_context *ctx) {
    u32 *img32 = calc_start_pos(out_buf, x_offset, y_offset, out_width);
    u32 stride = calc_stride(out_width, 160);
    const u32 default_buffer = device.display.active_draw_buffer ^ 1;

    // 160x144
    for (u32 y = 0; y < 144; /*data.bottom_rendered_line; */ y++) {
        for (u32 x = 0; x < 160; x++) {
            u32 di = ((y + 24) * 256) + (x + 48);
...
void DC_present(physical_io_device &device, void *out_buf, u32 out_width, u32 out_height, u32 &outcols, u32 &outrows)
{
    u32 *dco = (u32 *)device.display.output[0];
    outcols = device.display.cur_output_width;
    outrows = device.display.cur_output_height;
...
void jsm_present(jsm_system *jsm, jsm::systems which, physical_io_device &display, void *out_buf, u32 x_offset, u32 y_offset, u32 out_width, u32 out_height, events_view *ev, u32 &outcols, u32 &outrows, bool &updated_uv, const jsm_present_context *ctx)
{
    u32 is_event_view_present = ev != nullptr;
    const jsm_present_context *present_ctx = is_event_view_present ? nullptr : ctx;
    switch(which) {
        case jsm::systems::NEOGEO_AES:
        case jsm::systems::NEOGEO_MVS:
            neogeo_present(display, out_buf, out_width, out_height, is_event_view_present, present_ctx);
            break;
        case jsm::systems::NEOGEO_POCKET:
        case jsm::systems::NEOGEO_POCKET_COLOR:
            ngp_present(display, out_buf, out_width, out_height, is_event_view_present, which);
            break;
        case jsm::systems::PS1:
            updated_uv = true;
            PS1_present(display, out_buf, out_width, out_height, is_event_view_present, outcols, outrows);
            break;
        case jsm::systems::GBA:
            GBA_present(display, out_buf, out_width, out_height, is_event_view_present, present_ctx);
            break;
        case jsm::systems::NDS:
            NDS_present(display, out_buf, out_width, out_height, is_event_view_present, present_ctx);
            break;
        case jsm::systems::COMMODORE64:
            c64_present(display, out_buf, out_width, out_height, is_event_view_present, present_ctx);
            break;
        case jsm::systems::SNES:
            snes_present(display, out_buf, out_width, out_height, is_event_view_present, present_ctx);
            break;
        case jsm::systems::TURBOGRAFX16:
        case jsm::systems::TURBOGRAFX16_ARCADE_CD:
        case jsm::systems::TURBOGRAFX16_CD:
            tg16_present(display, out_buf, out_width, out_height, is_event_view_present, ev, present_ctx);
            break;
        case jsm::systems::GENESIS_USA:
        case jsm::systems::GENESIS_JAP:
        case jsm::systems::MEGADRIVE_PAL:
            genesis_present(display, out_buf, out_width, out_height, is_event_view_present, present_ctx);
            break;
        case jsm::systems::DMG:
            DMG_present(display, out_buf, x_offset, y_offset, out_width, out_height, is_event_view_present, present_ctx);
            break;
        case jsm::systems::GBC:
            GBC_present(display, out_buf, x_offset, y_offset, out_width, out_height, is_event_view_present, present_ctx);
            break;
        case jsm::systems::NES:
            NES_present(display, out_buf, x_offset, y_offset, out_width, out_height, present_ctx);
            break;
        case jsm::systems::CASIO_PV1000:
            CASIO_PV1000_present(display, out_buf, out_width, out_height, present_ctx);
            break;
        case jsm::systems::MAC128K:
        case jsm::systems::MAC512K:
        case jsm::systems::MACPLUS_1MB:
            mac512k_present(display, out_buf, out_width, out_height, present_ctx);
            break;
        case jsm::systems::SG1000:
        case jsm::systems::SMS1:
        case jsm::systems::SMS2:
            SMS_present(display, out_buf, x_offset, y_offset, out_width, out_height, present_ctx);
            break;
        case jsm::systems::GG:
            GG_present(display, out_buf, x_offset, y_offset, out_width, out_height, present_ctx);
            break;
        case jsm::systems::DREAMCAST:
            updated_uv = true;
            DC_present(display, out_buf, out_width, out_height, outcols, outrows);
            break;
        case jsm::systems::ATARI2600:
            atari2600_present(display, out_buf, out_width, out_height, present_ctx);
            break;
        case jsm::systems::APPLEIIe:
            apple2_present(display, out_buf, x_offset, y_offset, out_width, out_height, present_ctx);
            break;
        case jsm::systems::GALAKSIJA:
            galaksija_present(display, out_buf, out_width, out_height, present_ctx);
            break;
        case jsm::systems::COSMAC_VIP_2k:
        case jsm::systems::COSMAC_VIP_4k:
            cosmac_vip_present(display, out_buf,out_width, out_height, present_ctx);
            break;
        case jsm::systems::ZX_SPECTRUM_48K:
        case jsm::systems::ZX_SPECTRUM_128K:
            zx_spectrum_present(display, out_buf, out_width, out_height, present_ctx);
            break;
        default:
            printf("\nUNKNOWN PRESENT!");
            break;
