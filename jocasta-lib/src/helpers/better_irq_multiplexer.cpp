...
void IRQ_multiplexer_b::trigger(u32 num)
{
    irqs[num].IF = 1;
    IF |= (1ull << num);
}
...
void IRQ_multiplexer_b::set_sensitivity(u32 num, IRQ_multiplexer_b_kind kind)
{
    assert(num < MAX_IRQS_MULTIPLEXED);
    irqs[num].kind = kind;
    // Clear this line's latch + remembered input so the new sensitivity starts clean
    // (avoids a stale edge-latched request surviving a level<->edge mode change).
    irqs[num].IF = 0;
    irqs[num].input = 0;
    IF &= ~(1ull << num);
}
